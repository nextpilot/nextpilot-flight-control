...
parser.add_argument("--all-ports", action="store_true", help="Generate output for all known ports (params file only)")
parser.add_argument("--constrained-flash", action="store_true", help="Reduce verbosity in ROMFS scripts to reduce flash size")
parser.add_argument("--rc-dir", type=str, action="store", help="ROMFS output directory", default=None)
parser.add_argument("--params-file", type=str, action="store", help="Parameter output file", default=None)
...
rc_serial_template = "template/rc.serial.jinja"
rc_serial_port_template = "template/rc.serial_port.jinja"
serial_params_output_file = args.params_file
serial_params_template = "template/serial_params.c.jinja"
...
if rc_serial_output_dir is None and serial_params_output_file is None:
    raise Exception("At least one of --rc-dir or --params-file " "(e.g. serial_params.c) needs to be specified")
...
if ethernet_supported:
    ethernet_configuration.append({"tag": "ETH", "label": "Ethernet", "index": 1000})
...
jinja_env = Environment(loader=FileSystemLoader(os.path.dirname(os.path.realpath(__file__))))
...
if rc_serial_output_dir is not None:
    ...
    if len(serial_devices) == 0:
        ...
        with open(rc_serial_output_file, "w") as fid:
            fid.write(template.render(serial_devices=serial_devices, commands=commands, constrained_flash=constrained_flash))

        if verbose:
            print("Generating {:}".format(rc_serial_port_output_file))
