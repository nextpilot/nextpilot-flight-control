...
parser.add_argument("--config-files", type=str, nargs="*", default=[], help="YAML module config file(s)")
parser.add_argument("--params-file", type=str, action="store", help="Parameter output file")
parser.add_argument("--timer-config", type=str, action="store", help="board-specific timer_config.cpp file")
parser.add_argument("--ethernet", action="store_true", help="Ethernet support")
parser.add_argument("--board", type=str, action="store", help="board name, e.g. ")
parser.add_argument(
    "--board-with-io",
    dest="board_with_io",
    action="store_true",
...
output_functions_file = os.path.join(root_dir, "apps/libraries/controller/mixer_module/output_functions.yaml")
    ...
    for parameters_section in parameters_section_list:
        ...
        for param_name in definitions:
            ...
            for param in param_list:
                ...
                for tag in ["decimal", "increment", "category", "volatile", "min", "max", "unit", "reboot_required"]:
                    if tag in param:
...
""".format(
                        short_descr=param["description"]["short"].replace("\n", "\n * "),
                        ...
                        "${i}", str(i + instance_start_label)
                    )
    return ret


def get_actuator_output_params(yaml_config, output_functions, timer_config_file, verbose):
    ...
    while group_idx < len(output_groups):
        ...
        if "generator" in group:
            if group["generator"] == "pwm":
                ...
                channel_labels = [process_channel_label(module_name, label, no_prefix) for label in group["channel_labels"]]
        ...
        all_param_prefixes[param_prefix].append((instance_start, instance_start_label, num_channels, channel_label))

        # function param
        param = {
            "description": {
                "short": channel_label + " ${i} Output Function",
                "long": """Select what should be output on {:} ${{i}}.
...
""".format(
        ...
        disarmed_description = """This is the output value that is set when not armed.

Note that non-motor outputs might already be active in prearm state if COM_PREARM_MODE is set.
"""
        minimum_description = """Minimum output value (when not disarmed).
"""
        maximum_description = """Maxmimum output value (when not disarmed).
"""
        failsafe_description = """This is the output value that is set when in failsafe mode.

When set to -1 (default), the value depends on the function (see {:}).
""".format(
            param_prefix + "_FUNC${i}"
        )
        ...
        for key, label, param_suffix, description in standard_params_array:
            if key in standard_params:
                ...
                if standard_params[key]["max"] >= 1 << 16:
                    raise Exception(
                        "maximum value for {:} expected <= {:} (got {:})".format(key, 1 << 16, standard_params[key]["max"])
                    )
                ...
                param = {
                    "description": {"short": channel_label + " ${i} " + label + " Value", "long": description},
                    "type": "int32",
                    "instance_start": instance_start,
                    "instance_start_label": instance_start_label,
                    "num_instances": num_channels,
    ...
    for param_prefix in all_param_prefixes:
        ...
        param = {
            "description": {
                ...
                "long": """Allows to reverse the output range for each channel.
...
""".format(
    ...
    if verbose:
        print("adding actuator params: {:}".format(all_params))
    return all_params
...
for yaml_file in args.config_files:
    ...
    try:
        actuator_output_params = get_actuator_output_params(yaml_config, output_functions, timer_config_file, verbose)
...
if verbose:
    print("Generating {:}".format(params_output_file))
