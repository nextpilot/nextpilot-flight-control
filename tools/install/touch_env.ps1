...
if ($args[0] -eq "--gitee") {
    Write-Output "Using gitee service."
...
if (!(Test-Path -Path $env_dir)) {
    ...
    Write-Output 'source "$PKGS_DIR/packages/Kconfig"' | Out-File -FilePath $env_dir/packages/Kconfig -Encoding ASCII
    git clone $ENV_URL $env_dir/tools/scripts
    Write-Output '$env:path="$HOME\.env\tools\scripts;$env:path"' > $env_dir/env.ps1
    Write-Output '$env:pathext=".PS1;$env:pathext"' >> $env_dir/env.ps1
} else {
    Write-Output ".env folder has exsited. Jump this step."
}
