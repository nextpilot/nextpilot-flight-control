...
public:
    ...
    const px4_spi_bus_t &bus() const {
        return px4_spi_buses[_index];
    }

    spi_drdy_gpio_t DRDYGPIO() const {
        return px4_spi_buses[_index].devices[_bus_device_index].drdy_gpio;
    }

    uint32_t devid() const {
        return px4_spi_buses[_index].devices[_bus_device_index].devid;
    }
    ...
    bool external() const {
        return px4_spi_bus_external(bus());
    }

    int busDeviceIndex() const {
        return _bus_device_index;
    }
