...
/**
 * @file shutdown.cpp
 * Implementation of the API declared in shutdown.h.
 */

#define LOG_TAG "shutdown"

// #include <platform_common/config.h>
...
int shutdown_lock() {
    ...
    if (ret == 0) {
        ...
        indicate_external_reset_lockout(LockoutComponent::SystemShutdownLock, true);
...
int shutdown_unlock() {
    ...
    if (ret == 0) {
        if (shutdown_lock_counter > 0) {
            ...
            if (shutdown_lock_counter == 0) {
                indicate_external_reset_lockout(LockoutComponent::SystemShutdownLock, false);
            }

        } else {
            LOG_E("unmatched number of shutdown_unlock() calls");
        }
...
int unregister_shutdown_hook(shutdown_hook_t hook) {
    pthread_mutex_lock(&shutdown_mutex);
...
static void shutdown_worker(void *arg) {
    LOG_D("shutdown worker (%i)", shutdown_counter);
    bool done = true;
