...
class GPS : public ModuleCommand<GPS>, public device::Device {
public:
...
#   define B921600 921600
    ...
    default:
        LOG_E("ERR: unknown baudrate: %d", baud);
        return -EINVAL;
    }
...
void GPS::initializeCommunicationDump() {
    ...
    if (!_dump_from_device || !_dump_to_device) {
        LOG_E("failed to allocated dump data");
        return;
    }
...
void GPS::run() {
    ...
    while (!should_exit()) {
        ...
        if (_serial_fd < 0) {
            ...
            if (_serial_fd < 0) {
                LOG_E("failed to open %s err: %d", _port, errno);
                px4_sleep(1);
                continue;
            }
            ...
            if (_interface == GPSHelper::Interface::SPI) {
                ...
                if (status_value < 0) {
                    LOG_E("SPI_IOC_WR_MAX_SPEED_HZ failed for %s (%d)", _port, errno);
                }

                status_value = ::ioctl(_serial_fd, SPI_IOC_RD_MAX_SPEED_HZ, &spi_speed);

                if (status_value < 0) {
                    LOG_E("SPI_IOC_RD_MAX_SPEED_HZ failed for %s (%d)", _port, errno);
                }
            }
...
int GPS::print_status() {
    ...
    case gps_driver_mode_t::ASHTECH:
        LOG_I("protocol: ASHTECH");
        break;

    case gps_driver_mode_t::EMLIDREACH:
        LOG_I("protocol: EMLIDREACH");
        break;

    case gps_driver_mode_t::FEMTOMES:
        LOG_I("protocol: FEMTOMES");
        break;
    ...
    LOG_I("status: %s, port: %s, baudrate: %d", _healthy ? "OK" : "NOT OK", _port, _baudrate);
    LOG_I("sat info: %s", (_p_report_sat_info != nullptr) ? "enabled" : "disabled");
    LOG_I("rate reading: \t\t%6i B/s", _rate_reading);

    if (_report_gps_pos.timestamp != 0) {
        if (_helper) {
            LOG_I("rate position: \t\t%6.2f Hz", (double)_helper->getPositionUpdateRate());
            LOG_I("rate velocity: \t\t%6.2f Hz", (double)_helper->getVelocityUpdateRate());
        }

        LOG_I("rate publication:\t\t%6.2f Hz", (double)_rate);
        LOG_I("rate RTCM injection:\t%6.2f Hz", (double)_rate_rtcm_injection);
...
void GPS::reset_if_scheduled() {
    ...
    if (restart_type != GPSRestartType::None) {
        ...
        if (res == -1) {
            LOG_I("Reset is not supported on this device.");

        } else if (res < 0) {
            LOG_I("Reset failed.");

        } else {
            LOG_I("Reset succeeded.");
        }
    }
}
    ...
    if (_instance == Instance::Main || _is_gps_main_advertised.load()) {
        ...
        if (_report_gps_pos.spoofing_state != _spoofing_state) {
            if (_report_gps_pos.spoofing_state > sensor_gps_s::SPOOFING_STATE_NONE) {
                LOG_W("GPS spoofing detected! (state: %d)", _report_gps_pos.spoofing_state);
            }
        ...
        if (_report_gps_pos.jamming_state != _jamming_state) {
            if (_report_gps_pos.jamming_state > sensor_gps_s::JAMMING_STATE_WARNING) {
                LOG_W("GPS jamming detected! (state: %d) (indicator: %d)", _report_gps_pos.jamming_state,
...
GPS *GPS::instantiate(int argc, char *argv[], Instance instance) {
    ...
    while ((ch = px4_getopt(argc, argv, "b:d:e:g:i:j:p:", &myoptind, &myoptarg)) != EOF) {
        ...
        case 'i':
            if (!strcmp(myoptarg, "spi")) {
                ...
                LOG_E("unknown interface: %s", myoptarg);
                error_flag = true;
            }
            break;
        ...
        case 'j':
            if (!strcmp(myoptarg, "spi")) {
                ...
                LOG_E("unknown interface for secondary: %s", myoptarg);
                error_flag = true;
            }
            break;
        ...
        case 'p':
            if (!strcmp(myoptarg, "ubx")) {
                ...
                LOG_E("unknown protocol: %s", myoptarg);
                error_flag = true;
            }
            break;
    ...
    if (instance == Instance::Main) {
        if (device_name && (access(device_name, R_OK | W_OK) == 0)) {
            ...
            LOG_E("invalid device (-d) %s", device_name ? device_name : "");
        }
        ...
        if (gps && device_name_secondary) {
            ...
            if (i == 400) {
                LOG_E("Timed out while waiting for thread to start");
            }
        }
        ...
        if (device_name_secondary && (access(device_name_secondary, R_OK | W_OK) == 0)) {
            ...
            LOG_E("invalid secondary device (-g) %s", device_name_secondary ? device_name_secondary : "");
        }
    }
