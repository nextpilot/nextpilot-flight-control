...
int GPSDriverAshtech::handleMessage(int len) {
    ...
    if ((memcmp(_rx_buffer + 3, "ZDA,", 3) == 0) && (uiCalcComma == 6)) {
		...
		*/
        ...
        _gps_position->lon  = static_cast<int>((static_cast<int>(lon * 0.01) + (lon * 0.01 - static_cast<int>(lon * 0.01)) * 100.0 / 60.0) * 10000000);
        _gps_position->alt  = static_cast<int>(alt * 1000);
        ...
        if (coordinatesFound < 3) {
            ...
            if (_configure_done && _output_mode == OutputMode::RTCM && _board == AshtechBoard::trimble_mb_two && !_correction_output_activated) {
...
void GPSDriverAshtech::sendSurveyInStatusUpdate(bool active, bool valid, double latitude, double longitude, float altitude) {
    SurveyInStatus status{};
