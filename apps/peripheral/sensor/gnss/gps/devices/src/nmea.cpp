...
int GPSDriverNMEA::handleMessage(int len) {
    ...
    if ((memcmp(_rx_buffer + 3, "ZDA,", 4) == 0) && (uiCalcComma == 6)) {
		...
		*/
        ...
        if (bufptr && *(++bufptr) != ',') {
            heading_deg = strtof(bufptr, &endp);
            bufptr      = endp;
		...
		*/
        ...
        if (bufptr && *(++bufptr) != ',') {
            all_page_num = strtol(bufptr, &endp, 10);
            bufptr       = endp;
        }

        if (bufptr && *(++bufptr) != ',') {
            this_page_num = strtol(bufptr, &endp, 10);
            bufptr        = endp;
        }
...
int GPSDriverNMEA::configure(unsigned &baudrate, const GPSConfig &config) {
    ...
    for (unsigned int baud_i = 0; !_POS_received
                                  && baud_i < sizeof(baudrates_to_try) / sizeof(baudrates_to_try[0]);
         baud_i++) {
