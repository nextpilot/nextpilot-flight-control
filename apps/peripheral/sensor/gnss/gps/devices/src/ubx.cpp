...
#define UBX_TRACE_SVINFO(...)    \
    { /*GPS_INFO(__VA_ARGS__);*/ \
    } // NAV-SVINFO processing (debug use only, will cause rx buffer overflows)
...
int GPSDriverUBX::configure(unsigned &baudrate, const GPSConfig &config) {
    ...
    uint16_t out_proto_mask = _output_mode == OutputMode::GPS ? UBX_TX_CFG_PRT_PROTO_UBX : (UBX_TX_CFG_PRT_PROTO_UBX | UBX_TX_CFG_PRT_PROTO_RTCM);

    uint16_t in_proto_mask = (_output_mode == OutputMode::GPS || _output_mode == OutputMode::GPSAndRTCM) ? (UBX_TX_CFG_PRT_PROTO_UBX | UBX_TX_CFG_PRT_PROTO_RTCM) : UBX_TX_CFG_PRT_PROTO_UBX;
...
GPSDriverUBX::payloadRxInit() {
    ...
    case UBX_MSG_MON_RF:
        if (_rx_payload_length < sizeof(ubx_payload_rx_mon_rf_t) || (_rx_payload_length - 4) % sizeof(ubx_payload_rx_mon_rf_t::ubx_payload_rx_mon_rf_block_t) != 0) {
            _rx_state = UBX_RXMSG_ERROR_LENGTH;
...
GPSDriverUBX::payloadRxAddNavSat(const uint8_t b) {
    ...
    if (_rx_payload_index < sizeof(ubx_payload_rx_nav_sat_part1_t)) {
        ...
        if (_rx_payload_index < sizeof(ubx_payload_rx_nav_sat_part1_t) + _satellite_info->count * sizeof(ubx_payload_rx_nav_sat_part2_t)) {
            // Still room in _satellite_info: fill Part 2 buffer
            unsigned buf_index = (_rx_payload_index - sizeof(ubx_payload_rx_nav_sat_part1_t)) % sizeof(ubx_payload_rx_nav_sat_part2_t);
            p_buf[buf_index]   = b;

            if (buf_index == sizeof(ubx_payload_rx_nav_sat_part2_t) - 1) {
                // Part 2 complete: decode Part 2 buffer
                unsigned sat_index = (_rx_payload_index - sizeof(ubx_payload_rx_nav_sat_part1_t)) / sizeof(ubx_payload_rx_nav_sat_part2_t);
    ...
    if (_rx_payload_index < sizeof(ubx_payload_rx_mon_ver_part1_t)) {
        ...
        if (_rx_payload_index == sizeof(ubx_payload_rx_mon_ver_part1_t)) {
            ...
            if (strncmp((const char *)_buf.payload_rx_mon_ver_part1.hwVersion, "00040005",
                        sizeof(_buf.payload_rx_mon_ver_part1.hwVersion))
                == 0) {
...
GPSDriverUBX::payloadRxDone() {
    ...
    case UBX_MSG_NAV_STATUS:
        ...
        _gps_position->spoofing_state = (_buf.payload_rx_nav_status.flags2 & UBX_RX_NAV_STATUS_SPOOFDETSTATE_MASK) >> UBX_RX_NAV_STATUS_SPOOFDETSTATE_SHIFT;
    ...
    case UBX_MSG_NAV_RELPOSNED:
...

        {
            ...
            gps_rel.position_length = (_buf.payload_rx_nav_relposned.relPosLength
                                       + _buf.payload_rx_nav_relposned.relPosHPLength * 1e-2f)
                                    * 1e-2f;
...
int GPSDriverUBX::reset(GPSRestartType restart_type) {
