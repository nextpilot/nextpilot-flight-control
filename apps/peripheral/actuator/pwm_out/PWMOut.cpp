    ...
    if (_pwm_initialized) {
        for (int timer = 0; timer < MAX_IO_TIMERS; ++timer) {
            if (_timer_rates[timer] >= 0) {
                LOG_RAW("Timer %i: rate: %3i", timer, _timer_rates[timer]);
                uint32_t channels = _pwm_mask & up_pwm_servo_get_rate_group(timer);
...
static int pwm_out_main(int argc, char *argv[]) {
    return PWMOut::main(argc, argv);
}

MSH_CMD_EXPORT_ALIAS(pwm_out_main, pwm_out, pwm out);

static int pwm_out_start() {
    const char *argv[] = {"pwm_out", "start"};
    int         argc   = sizeof(argv) / sizeof(argv[0]);

    return PWMOut::main(argc, (char **)argv);
}

INIT_APP_EXPORT(pwm_out_start);
