...
const airframe_config_t *airframe_get_config() {
    static airframe_config_t config{};
    static param_t           sys_autostart_handle    = PARAM_INVALID;
    static param_t           sys_autoconfig_handle   = PARAM_INVALID;
    static param_t           sys_hitl_handle         = PARAM_INVALID;
    static param_t           sys_vehicle_type_handle = PARAM_INVALID;
    static param_t           sys_mc_est_group_handle = PARAM_INVALID;

    if (sys_autostart_handle == PARAM_INVALID) {
        sys_autostart_handle = param_find("SYS_AUTOSTART");
    }
    if (sys_autoconfig_handle == PARAM_INVALID) {
        sys_autoconfig_handle = param_find("SYS_AUTOCONFIG");
    }
    if (sys_hitl_handle == PARAM_INVALID) {
        sys_hitl_handle = param_find("SYS_HITL");
    }
    if (sys_vehicle_type_handle == PARAM_INVALID) {
        sys_vehicle_type_handle = param_find("SYS_VEHICLE_TYPE");
    }

    if (sys_mc_est_group_handle == PARAM_INVALID) {
        sys_mc_est_group_handle = param_find("SYS_MC_EST_GROUP");
    }

    param_get(sys_autostart_handle, &config.sys_autostart);
    param_get(sys_autoconfig_handle, &config.sys_autoconfig);
    param_get(sys_hitl_handle, &config.sys_hitl);
    param_get(sys_vehicle_type_handle, &config.sys_vehicle_type);
    param_get(sys_mc_est_group_handle, &config.sys_mc_est_group);
...
airframe_info_t *airframe_get_info() {
    ...
    for (int i = 0; i < airframe_get_count(); i++) {
        if (__airframe_table__[i].id == config->sys_autostart) {
...
static int reset_airframe(int argc, char *argv[]) {
    ...
    param_set(param_find("SYS_AUTOCONFIG"), &sys_autoconfig);
...
static int change_airframe(int argc, char *argv[]) {
    ...
    if (id == sys_autostart) {
        ...
        param_set(param_find("SYS_AUTOSTART"), &id);
        param_set(param_find("SYS_AUTOCONFIG"), &sys_autoconfig);
...
static int airframe_param_init() {
    ...
    if ((sys_autostart < 1100) || (sys_autostart > 1104)) {
        ...
        sys_autostart  = 1102;
        sys_autoconfig = 1;
        LOG_W("airframe wrong, change to 1102");
    }
    ...
    LOG_I("start id: %d, hitl: %d, vehicle: %d, est: %d",
          config->sys_autostart,
          config->sys_hitl,
          config->sys_vehicle_type,
          config->sys_mc_est_group);
