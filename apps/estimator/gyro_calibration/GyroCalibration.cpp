...
void GyroCalibration::Run() {
    ...
    if (hrt_elapsed_time(&_last_calibration_update) > 5_s) {
        ...
        for (int gyro = 0; gyro < _sensor_gyro_subs.size(); gyro++) {
            if ((_gyro_calibration[gyro].device_id() != 0) && _gyro_mean[gyro].valid() && (_gyro_mean[gyro].count() > 100)) {
                ...
                if ((change_exceeds_stddev || variance_significantly_better || !_gyro_calibration[gyro].calibrated()) && _gyro_calibration[gyro].set_offset(new_offset)) {
