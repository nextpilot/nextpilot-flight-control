...
class EKF2 final : public ModuleParams, public WorkItemScheduled {
public:
    EKF2() = delete;
...
private:
    ...
    DEFINE_PARAMETERS(
        (ParamExtInt<params_id::EKF2_PREDICT_US>)_param_ekf2_predict_us,
        ...
        (ParamExtFloat<params_id::EKF2_GPS_P_NOISE>)
        ...
        (ParamExtFloat<params_id::EKF2_ARSP_THR>)
...
#if defined(CONFIG_EKF2_SIDESLIP)
        (ParamExtFloat<params_id::EKF2_BETA_GATE>)
            _param_ekf2_beta_gate,                                         ///< synthetic sideslip innovation consistency gate size (STD)
        (ParamExtFloat<params_id::EKF2_BETA_NOISE>)_param_ekf2_beta_noise, ///< synthetic sideslip noise (rad)

        (ParamExtInt<params_id::EKF2_FUSE_BETA>)
        ...
        (ParamInt<params_id::EKF2_AID_MASK>)
            _param_ekf2_aid_mask,                                      ///< bitmasked integer that selects which of the GPS and optical flow aiding sources will be used
        (ParamExtInt<params_id::EKF2_HGT_REF>)_param_ekf2_hgt_ref,     ///< selects the primary source for height data
        (ParamExtInt<params_id::EKF2_BARO_CTRL>)_param_ekf2_baro_ctrl, ///< barometer control selection
        (ParamExtInt<params_id::EKF2_GPS_CTRL>)_param_ekf2_gps_ctrl,   ///< GPS control selection

        (ParamExtInt<params_id::EKF2_NOAID_TOUT>)
        ...
        (ParamExtInt<params_id::EKF2_EV_QMIN>)_param_ekf2_ev_qmin,
        (ParamExtFloat<params_id::EKF2_EVP_NOISE>)
            _param_ekf2_evp_noise, ///< default position observation noise for exernal vision measurements (m)
        (ParamExtFloat<params_id::EKF2_EVV_NOISE>)
        ...
        (ParamExtFloat<params_id::EKF2_IMU_POS_Y>)_param_ekf2_imu_pos_y, ///< Y position of IMU in body frame (m)
        (ParamExtFloat<params_id::EKF2_IMU_POS_Z>)_param_ekf2_imu_pos_z, ///< Z position of IMU in body frame (m)
        (ParamExtFloat<params_id::EKF2_GPS_POS_X>)_param_ekf2_gps_pos_x, ///< X position of GPS antenna in body frame (m)
        (ParamExtFloat<params_id::EKF2_GPS_POS_Y>)_param_ekf2_gps_pos_y, ///< Y position of GPS antenna in body frame (m)
        (ParamExtFloat<params_id::EKF2_GPS_POS_Z>)_param_ekf2_gps_pos_z, ///< Z position of GPS antenna in body frame (m)

        // output predictor filter time constants
        (ParamFloat<params_id::EKF2_TAU_VEL>)
            _param_ekf2_tau_vel, ///< time constant used by the output velocity complementary filter (sec)
        (ParamFloat<params_id::EKF2_TAU_POS>)
        ...
        (ParamExtInt<params_id::EKF2_DRAG_CTRL>)_param_ekf2_drag_ctrl, ///< drag fusion selection
        // Multi-rotor drag specific force fusion
        (ParamExtFloat<params_id::EKF2_DRAG_NOISE>)
            _param_ekf2_drag_noise,                                  ///< observation noise variance for drag specific force measurements (m/sec**2)**2
        (ParamExtFloat<params_id::EKF2_BCOEF_X>)_param_ekf2_bcoef_x, ///< ballistic coefficient along the X-axis (kg/m**2)
        (ParamExtFloat<params_id::EKF2_BCOEF_Y>)_param_ekf2_bcoef_y, ///< ballistic coefficient along the Y-axis (kg/m**2)
        (ParamExtFloat<params_id::EKF2_MCOEF>)_param_ekf2_mcoef,     ///< propeller momentum drag coefficient (1/s)
