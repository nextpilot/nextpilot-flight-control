...
void sympyMagXInnovVarHxAndKx(float q0, float q1, float q2, float q3, float magN, float magE, float magD,
    ...
    innov_var = (HKX10 * HKX20 - HKX11 * HKX18 + HKX12 * HKX22 + HKX13 * HKX16 - HKX14 * HKX19 + HKX15 * HKX21 + HKX17 * HKX6 + HKX23 + R_MAG);
...
void sympyMagYInnovVarHyAndKy(float q0, float q1, float q2, float q3, float magN, float magE, float magD,
    ...
    innov_var         = (HKY10 * HKY20 - HKY11 * HKY18 + HKY12 * HKY22 + HKY13 * HKY16 + HKY14 * HKY21 - HKY15 * HKY19 + HKY17 * HKY8 + HKY23 + R_MAG);
...
void sympyMagZInnovVarHzAndKz(float q0, float q1, float q2, float q3, float magN, float magE, float magD,
    ...
    innov_var         = (HKZ10 * HKZ20 - HKZ11 * HKZ18 + HKZ12 * HKZ21 + HKZ13 * HKZ16 - HKZ14 * HKZ19 + HKZ15 * HKZ22 + HKZ17 * HKZ9 + HKZ23 + R_MAG);
...
TEST(Mag3DFusionGenerated, SympyVsSymforce) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        EXPECT_LT(report.max_diff_fraction, 1e-5f) << "i = " << i << "Airspeed Hfusion max diff fraction = " << report.max_diff_fraction << " location index = " << report.max_row << " sympy = " << report.max_v1 << " symforce = " << report.max_v2;

        report = computeDiffRatioVector24f(Kfusion_sympy, Kfusion_symforce);
        EXPECT_LT(report.max_diff_fraction, 1e-5f) << "i = " << i << "Airspeed Kfusion max diff fraction = " << report.max_diff_fraction << " location index = " << report.max_row << " sympy = " << report.max_v1 << " symforce = " << report.max_v2;
