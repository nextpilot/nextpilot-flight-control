...
TEST(OptFlowFusionGenerated, SympyVsSymforce) {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        EXPECT_LT(report.max_diff_fraction, 1e-5f) << "i = " << i << "Hfusion max diff fraction = " << report.max_diff_fraction << " location index = " << report.max_row << " sympy = " << report.max_v1 << " symforce = " << report.max_v2;

        report = computeDiffRatioVector24f(Kfusion_sympy, Kfusion_symforce);
        EXPECT_LT(report.max_diff_fraction, 1e-5f) << "i = " << i << "Kfusion max diff fraction = " << report.max_diff_fraction << " location index = " << report.max_row << " sympy = " << report.max_v1 << " symforce = " << report.max_v2;
