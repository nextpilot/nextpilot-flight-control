...
TEST(GnssYawFusionGenerated, SympyVsSymforce) {
    ...
    EXPECT_LT(report.max_diff_fraction, 1e-5f) << "H max diff fraction = " << report.max_diff_fraction << " location index = " << report.max_row << " sympy = " << report.max_v1 << " symforce = " << report.max_v2;

    report = computeDiffRatioVector24f(K_sympy, K_symforce);
    EXPECT_LT(report.max_diff_fraction, 1e-5f) << "K max diff fraction = " << report.max_diff_fraction << " location index = " << report.max_row << " sympy = " << report.max_v1 << " symforce = " << report.max_v2;
