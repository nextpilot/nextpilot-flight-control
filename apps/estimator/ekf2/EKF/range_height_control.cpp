...
void Ekf::controlRangeHeightFusion() {
    ...
    if (_range_buffer) {
        ...
        if (_range_sensor.isDataHealthy()) {
            ...
            if (_control_status.flags.in_air) {
                const bool horizontal_motion = _control_status.flags.fixed_wing || (sq(_state.vel(0)) + sq(_state.vel(1)) > fmaxf(P(4, 4) + P(5, 5), 0.1f));
            ...
            if (!_control_status.flags.in_air && _range_sensor.isRegularlySendingData() && _range_sensor.isDataReady()) {
    ...
    if (rng_data_ready && _range_sensor.getSampleAddress()) {
        ...
        const bool continuing_conditions_passing = ((_params.rng_ctrl == RngCtrl::ENABLED) || do_conditional_range_aid) && measurement_valid && _range_sensor.isDataHealthy();

        const bool starting_conditions_passing = continuing_conditions_passing && isNewestSampleRecent(_time_last_range_buffer_push, 2 * RNG_MAX_INTERVAL) && _range_sensor.isRegularlySendingData();
