...
bool EKF2::multi_init(int imu, int mag) {
    ...
    if ((status_instance >= 0) && changed_instance && (_attitude_pub.get_instance() == status_instance) && (_local_position_pub.get_instance() == status_instance) && (_global_position_pub.get_instance() == status_instance)) {
...
void EKF2::VerifyParams() {
    ...
    if ((_param_ekf2_hgt_ref.get() == HeightSensor::EV) && !(_param_ekf2_ev_ctrl.get() & static_cast<int32_t>(EvCtrl::VPOS))) {
    ...
    if ((_param_ekf2_aid_mask.get() & SensorFusionMask::DEPRECATED_USE_EXT_VIS_VEL) || (_param_ekf2_aid_mask.get() & SensorFusionMask::DEPRECATED_USE_EXT_VIS_POS) || (_param_ekf2_aid_mask.get() & SensorFusionMask::DEPRECATED_USE_EXT_VIS_YAW)) {
        ...
        if ((_param_ekf2_aid_mask.get() & SensorFusionMask::DEPRECATED_USE_EXT_VIS_POS)) {
            _param_ekf2_ev_ctrl.set(_param_ekf2_ev_ctrl.get() | static_cast<int32_t>(EvCtrl::HPOS) | static_cast<int32_t>(EvCtrl::VPOS));
        }
...
void EKF2::PublishLocalPosition(const hrt_abstime &timestamp) {
    ...
    lpos.dead_reckoning = _ekf.control_status_flags().inertial_dead_reckoning || _ekf.control_status_flags().wind_dead_reckoning;
...
void EKF2::PublishSensorBias(const hrt_abstime &timestamp) {
    ...
    if ((gyro_bias - _last_gyro_bias_published).longerThan(0.001f) || (accel_bias - _last_accel_bias_published).longerThan(0.001f) || (mag_bias - _last_mag_bias_published).longerThan(0.001f) || (timestamp >= _last_sensor_bias_published + 1_s)) {
...
void EKF2::UpdateAirspeedSample(ekf2_timestamps_s &ekf2_timestamps) {
    ...
    if (_airspeed_validated_sub.updated()) {
        ...
        if (_airspeed_validated_sub.update(&airspeed_validated)) {
            ...
            if (PX4_ISFINITE(airspeed_validated.true_airspeed_m_s) && PX4_ISFINITE(airspeed_validated.calibrated_airspeed_m_s) && (airspeed_validated.calibrated_airspeed_m_s > 0.f) && (airspeed_validated.selected_airspeed_index > 0)) {
...
bool EKF2::UpdateExtVisionSample(ekf2_timestamps_s &ekf2_timestamps) {
    ...
    if (_ev_odom_sub.update(&ev_odom)) {
        ...
        const bool     non_zero                   = (fabsf(ev_odom_q(0)) > 0.f) || (fabsf(ev_odom_q(1)) > 0.f) || (fabsf(ev_odom_q(2)) > 0.f) || (fabsf(ev_odom_q(3)) > 0.f);
        const float    eps                        = 1e-5f;
        const bool     no_element_larger_than_one = (fabsf(ev_odom_q(0)) <= 1.f + eps) && (fabsf(ev_odom_q(1)) <= 1.f + eps) && (fabsf(ev_odom_q(2)) <= 1.f + eps) && (fabsf(ev_odom_q(3)) <= 1.f + eps);
        ...
        if (_distance_sensor_subs.advertised()) {
            for (unsigned i = 0; i < _distance_sensor_subs.size(); i++) {
                if (_distance_sensor_subs[i].update(&distance_sensor)) {
                    ...
                    if ((distance_sensor.timestamp != 0) && (distance_sensor.timestamp > timestamp_stale) && (distance_sensor.orientation == distance_sensor_s::ROTATION_DOWNWARD_FACING)) {
...
void EKF2::UpdateSystemFlagsSample(ekf2_timestamps_s &ekf2_timestamps) {
    ...
    if (_status_sub.updated() || _vehicle_land_detected_sub.updated()) {
        ...
        if (_vehicle_land_detected_sub.copy(&vehicle_land_detected) && (ekf2_timestamps.timestamp < vehicle_land_detected.timestamp + 3_s)) {
...
void EKF2::UpdateCalibration(const hrt_abstime &timestamp, InFlightCalibration &cal, const matrix::Vector3f &bias,
    ...
    const bool valid = bias_valid && (bias_variance.max() < max_var_allowed) && (bias_variance.max() < max_var_ratio * bias_variance.min());
...
void EKF2::UpdateAccelCalibration(const hrt_abstime &timestamp) {
    // the EKF is operating in the correct mode and there are no filter faults
    const bool bias_valid = (_param_ekf2_imu_ctrl.get() & static_cast<int32_t>(ImuCtrl::AccelBias)) && _ekf.control_status_flags().tilt_align && (_ekf.fault_status().value == 0) && !_ekf.fault_status_flags().bad_acc_bias && !_ekf.fault_status_flags().bad_acc_clipping && !_ekf.fault_status_flags().bad_acc_vertical && !_ekf.warning_event_flags().invalid_accel_bias_cov_reset;
...
void EKF2::UpdateGyroCalibration(const hrt_abstime &timestamp) {
    // the EKF is operating in the correct mode and there are no filter faults
    const bool bias_valid = (_param_ekf2_imu_ctrl.get() & static_cast<int32_t>(ImuCtrl::GyroBias)) && _ekf.control_status_flags().tilt_align && (_ekf.fault_status().value == 0);
...
void EKF2::UpdateMagCalibration(const hrt_abstime &timestamp) {
    const bool bias_valid = (_ekf.control_status_flags().mag_hdg || _ekf.control_status_flags().mag_3D) && !_ekf.control_status_flags().mag_fault && !_ekf.control_status_flags().mag_field_disturbed;
...
int EKF2::instantiate(int argc, char *argv[]) {
    bool success     = false;
    ...
    if (multi_mode && !replay_mode) {
        ...
        while ((multi_instances_allocated < multi_instances) && (vehicle_status_sub.get().arming_state != vehicle_status_s::ARMING_STATE_ARMED) && ((hrt_elapsed_time(&time_started) < 30_s) || (vehicle_status_sub.get().hil_state == vehicle_status_s::HIL_STATE_ON))) {
...

    {
        ...
        EKF2 *ekf2_inst = new EKF2(false, wq_configurations::INS0, replay_mode);
