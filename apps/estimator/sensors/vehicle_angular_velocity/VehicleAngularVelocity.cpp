...
bool VehicleAngularVelocity::UpdateSampleRate() {
    ...
    if (PX4_ISFINITE(sample_rate_hz) && (sample_rate_hz > 10) && (sample_rate_hz < 10'000) && PX4_ISFINITE(publish_rate_hz) && (publish_rate_hz > 0)) {
        // check if sample rate error is greater than 1%
        const bool sample_rate_changed = (fabsf(sample_rate_hz - _filter_sample_rate_hz) / sample_rate_hz) > 0.01f;

        if (_update_sample_rate || sample_rate_changed || (_filter_sample_rate_hz <= FLT_EPSILON) || !PX4_ISFINITE(_filter_sample_rate_hz)) {
...
void VehicleAngularVelocity::ResetFilters(const hrt_abstime &time_now_us) {
    if ((_filter_sample_rate_hz > 0) && PX4_ISFINITE(_filter_sample_rate_hz)) {
        ...
        for (int axis = 0; axis < 3; axis++) {
            ...
            if ((_param_imu_dgyro_cutoff.get() > 0.f) && (_lp_filter_acceleration[axis].setCutoffFreq(_filter_sample_rate_hz, _param_imu_dgyro_cutoff.get()))) {
...
bool VehicleAngularVelocity::SensorSelectionUpdate(const hrt_abstime &time_now_us, bool force) {
    if (_sensor_selection_sub.updated() || (_selected_sensor_device_id == 0) || force) {
        ...
        for (uint8_t i = 0; i < MAX_SENSOR_COUNT; i++) {
            ...
            if (imu_status.advertised() && (imu_status.get().timestamp != 0) && (time_now_us < imu_status.get().timestamp + 1_s) && (imu_status.get().gyro_device_id != 0)) {
        ...
        if ((_selected_sensor_device_id != device_id) || force) {
            ...
            for (uint8_t i = 0; i < MAX_SENSOR_COUNT; i++) {
                ...
                if (sensor_gyro_fifo_sub.advertised() && (sensor_gyro_fifo_sub.get().timestamp != 0) && (sensor_gyro_fifo_sub.get().device_id != 0) && (time_now_us < sensor_gyro_fifo_sub.get().timestamp + 1_s)) {
            ...
            for (uint8_t i = 0; i < MAX_SENSOR_COUNT; i++) {
                ...
                if (sensor_gyro_sub.advertised() && (sensor_gyro_sub.get().timestamp != 0) && (sensor_gyro_sub.get().device_id != 0) && (time_now_us < sensor_gyro_sub.get().timestamp + 1_s)) {
