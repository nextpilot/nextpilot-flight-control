        ...
        if (_magnetometer_bias_estimate_sub.copy(&mag_bias_est)) {
            ...
            for (int mag_index = 0; mag_index < MAX_SENSOR_COUNT; mag_index++) {
                if (mag_bias_est.valid[mag_index] && (mag_bias_est.timestamp > _last_calibration_update)) {
                    ...
                    if (_param_sens_mag_autocal.get() && !_armed && mag_bias_est.stable[mag_index] && (_calibration[mag_index].device_id() != 0) && !_calibration[mag_index].calibrated()) {
...
void VehicleMagnetometer::UpdatePowerCompensation() {
    if (_mag_comp_type != MagCompensationType::Disabled) {
        ...
        } else if ((_mag_comp_type == MagCompensationType::Current_inst0) || (_mag_comp_type == MagCompensationType::Current_inst1)) {
...
void VehicleMagnetometer::Run() {
    ...
    for (int uorb_index = 0; uorb_index < MAX_SENSOR_COUNT; uorb_index++) {
        ...
        if (!_advertised[uorb_index]) {
            ...
            if ((_last_publication_timestamp[uorb_index] == 0) || (time_now_us > _last_publication_timestamp[uorb_index] + 1_s)) {
            ...
            if (updated[instance] && (_data_sum_count[instance] > 0)) {
                ...
                if (timestamp_sample >= _last_publication_timestamp[instance] + interval_us) {
                    ...
                    if (!multi_mode && publish) {
                        publish = (_selected_sensor_sub_index >= 0) && (instance == _selected_sensor_sub_index) && (_voter.get_sensor_state(_selected_sensor_sub_index) == DataValidator::ERROR_FLAG_NO_ERROR);
                    }
