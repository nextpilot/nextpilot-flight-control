...
void VehicleAcceleration::CheckAndUpdateFilters() {
    ...
    for (uint8_t i = 0; i < MAX_SENSOR_COUNT; i++) {
        ...
        if (imu_status.advertised() && (imu_status.get().timestamp != 0) && (imu_status.get().accel_device_id != 0) && (imu_status.get().accel_device_id == _calibration.device_id()) && PX4_ISFINITE(sample_rate_hz) && (sample_rate_hz > 0)) {
    ...
    if (_sensor_selection_sub.updated() || (_calibration.device_id() == 0) || force) {
        ...
        if ((sensor_selection.accel_device_id != 0) && (_calibration.device_id() != sensor_selection.accel_device_id)) {
            for (uint8_t i = 0; i < MAX_SENSOR_COUNT; i++) {
                ...
                if (sensor_accel_sub.advertised() && (sensor_accel_sub.get().timestamp != 0) && (device_id != 0) && (device_id == sensor_selection.accel_device_id)) {
