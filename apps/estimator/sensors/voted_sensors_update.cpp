...
void VotedSensorsUpdate::parametersUpdate() {
    ...
    for (uint8_t uorb_index = 0; uorb_index < MAX_SENSOR_COUNT; uorb_index++) {
        ...
        if (imu.advertised() && (imu.get().timestamp != 0) && (imu.get().accel_device_id != 0) && (imu.get().gyro_device_id != 0)) {
...
void VotedSensorsUpdate::imuPoll(struct sensor_combined_s &raw) {
    ...
    for (int uorb_index = 0; uorb_index < MAX_SENSOR_COUNT; uorb_index++) {
        ...
        if ((_accel.priority[uorb_index] > 0) && (_gyro.priority[uorb_index] > 0) && _vehicle_imu_sub[uorb_index].update(&imu_report)) {
    ...
    if ((accel_best_index >= 0) && (accel_best_index < MAX_SENSOR_COUNT) && (_accel_device_id[accel_best_index] != 0) && (gyro_best_index >= 0) && (gyro_best_index < MAX_SENSOR_COUNT) && (_gyro_device_id[gyro_best_index] != 0)) {
