...
void VehicleIMU::Run() {
    ...
    while (_sensor_gyro_sub.updated() || _sensor_accel_sub.updated()) {
        ...
        if (_intervals_configured && !consume_all_gyro && _sensor_gyro_sub.updated() && _gyro_publish_latency_mean_us.valid() && _gyro_mean_interval_us.valid() && (_gyro_publish_latency_mean_us.mean() > _gyro_mean_interval_us.mean())) {
        ...
        while (_sensor_accel_sub.updated() && (!_accel_integrator.integral_ready() || !_intervals_configured || _data_gap || (_accel_timestamp_sample_last < (_gyro_timestamp_sample_last - 0.5f * _accel_interval_us)))) {
            if (UpdateAccel()) {
                updated = true;
            }
        }
    ...
    if (_param_sens_imu_autocal.get() && !parameters_updated) {
        if ((_armed || !_accel_calibration.calibrated() || !_gyro_calibration.calibrated()) && (now_us > _in_flight_calibration_check_timestamp_last + 1_s)) {
...
bool VehicleIMU::Publish() {
    ...
    if (_accel_integrator.reset(delta_velocity, imu.delta_velocity_dt) && _gyro_integrator.reset(delta_angle, imu.delta_angle_dt)) {
        if (_accel_calibration.enabled() && _gyro_calibration.enabled()) {
            ...
            if (_raw_accel_mean.valid() && _raw_gyro_mean.valid() && _accel_mean_interval_us.valid() && _gyro_mean_interval_us.valid() && (_publish_status || status_publish_interval_exceeded)) {
                // Accel
                {
...
void VehicleIMU::UpdateAccelVibrationMetrics(const Vector3f &acceleration) {
    ...
    _status.accel_vibration_metric = 0.99f * _status.accel_vibration_metric + 0.01f * Vector3f(acceleration - _acceleration_prev).norm();
...
void VehicleIMU::UpdateGyroVibrationMetrics(const Vector3f &angular_velocity) {
    ...
    _status.gyro_vibration_metric = 0.99f * _status.gyro_vibration_metric + 0.01f * Vector3f(angular_velocity - _angular_velocity_prev).norm();
...
void VehicleIMU::SensorCalibrationUpdate() {
    for (int i = 0; i < _estimator_sensor_bias_subs.size(); i++) {
        ...
        if (_estimator_sensor_bias_subs[i].update(&estimator_sensor_bias) && (hrt_elapsed_time(&estimator_sensor_bias.timestamp) < 1_s)) {
            // find corresponding accel bias
            if (estimator_sensor_bias.accel_bias_valid && estimator_sensor_bias.accel_bias_stable && (estimator_sensor_bias.accel_device_id != 0) && (estimator_sensor_bias.accel_device_id == _accel_calibration.device_id())) {
            ...
            if (estimator_sensor_bias.gyro_bias_valid && estimator_sensor_bias.gyro_bias_stable && (estimator_sensor_bias.gyro_device_id != 0) && (estimator_sensor_bias.gyro_device_id == _gyro_calibration.device_id())) {
