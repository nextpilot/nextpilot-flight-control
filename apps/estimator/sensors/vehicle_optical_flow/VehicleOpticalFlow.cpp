...
void VehicleOpticalFlow::Run() {
    ...
    if (_sensor_flow_sub.update(&sensor_optical_flow)) {
        ...
        if ((sensor_optical_flow.timestamp_sample >= _flow_timestamp_sample_last + integration_gap_threshold_us) || (_accumulated_count > 0 && (sensor_optical_flow.quality > 0) && _quality_sum == 0)) {
        ...
        if (publish) {
            ...
            // SENS_FLOW_MAXR
            if (PX4_ISFINITE(sensor_optical_flow.max_flow_rate) && (sensor_optical_flow.max_flow_rate <= _param_sens_flow_maxr.get())) {
            ...
            // SENS_FLOW_MINHGT
            if (PX4_ISFINITE(sensor_optical_flow.min_ground_distance) && (sensor_optical_flow.min_ground_distance >= _param_sens_flow_minhgt.get())) {
            ...
            // SENS_FLOW_MAXHGT
            if (PX4_ISFINITE(sensor_optical_flow.max_ground_distance) && (sensor_optical_flow.max_ground_distance <= _param_sens_flow_maxhgt.get())) {
...
void VehicleOpticalFlow::UpdateDistanceSensor() {
    ...
    if (_distance_sensor_selected >= 0 && _distance_sensor_subs[_distance_sensor_selected].update(&distance_sensor)) {
        ...
        if (distance_sensor.orientation == distance_sensor_s::ROTATION_DOWNWARD_FACING) {
            if ((distance_sensor.current_distance >= distance_sensor.min_distance) && (distance_sensor.current_distance <= distance_sensor.max_distance)) {
                rangeSample sample;
...
void VehicleOpticalFlow::UpdateSensorGyro() {
    if (_sensor_selection_sub.updated()) {
        ...
        for (uint8_t i = 0; i < MAX_SENSOR_COUNT; i++) {
            ...
            if (sensor_gyro_sub.advertised() && (sensor_gyro_sub.get().timestamp != 0) && (sensor_gyro_sub.get().device_id != 0) && (hrt_elapsed_time(&sensor_gyro_sub.get().timestamp) < 1_s)) {
