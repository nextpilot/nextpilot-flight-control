...
void MavlinkParametersManager::send() {
    ...
    while ((i++ < max_num_to_send) && (_mavlink->get_free_tx_buf() >= get_size()) && !_mavlink->radio_status_critical() && send_params()) {
    }
}
...
bool MavlinkParametersManager::send_uavcan() {
    ...
    if (_uavcan_parameter_value_sub.update(&value)) {
        ...
        if ((_uavcan_open_request_list != nullptr) && (value.param_index == _uavcan_open_request_list->req.param_index) && (value.node_id == _uavcan_open_request_list->req.node_id)) {
