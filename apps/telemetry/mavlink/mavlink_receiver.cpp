...
void MavlinkReceiver::handle_message_optical_flow_rad(mavlink_message_t *msg) {
    ...
    device_id.devid_s.bus_index = _mavlink->get_instance_id();
...
void MavlinkReceiver::handle_message_odometry(mavlink_message_t *msg) {
    ...
    if (PX4_ISFINITE(odom_in.rollspeed) && PX4_ISFINITE(odom_in.pitchspeed) && PX4_ISFINITE(odom_in.yawspeed)) {
...
void MavlinkReceiver::update_message_statistics(const mavlink_message_t &message) {
    ...
    if (_received_msg_stats) {
        ...
        for (int stat_slot = 0; stat_slot < MAX_MSG_STAT_SLOTS; stat_slot++) {
            if ((_received_msg_stats[stat_slot].msg_id == message.msgid) && (_received_msg_stats[stat_slot].system_id == message.sysid) && (_received_msg_stats[stat_slot].component_id == message.compid)) {
            ...
            if (comp_stat.received_messages > 0) {
                ...
                if (_message_statistics_enabled && _received_msg_stats) {
                    for (int i = 0; i < MAX_MSG_STAT_SLOTS; i++) {
                        ...
                        if ((msg_stat.system_id == comp_stat.system_id) && (msg_stat.component_id == comp_stat.component_id) && (msg_stat.last_time_received_ms != 0) && (now_ms - msg_stat.last_time_received_ms < 10'000)) {
