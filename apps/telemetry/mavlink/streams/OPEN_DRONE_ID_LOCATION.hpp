...
public:
    static MavlinkStream *new_instance(Mavlink *mavlink) {
        return new MavlinkStreamOpenDroneIdLocation(mavlink);
    }
...
private:
    explicit MavlinkStreamOpenDroneIdLocation(Mavlink *mavlink) :
    ...
    bool send() override {
        ...
        if (_vehicle_land_detected_sub.advertised()) {
            ...
            if (_vehicle_land_detected_sub.copy(&vehicle_land_detected) && (hrt_elapsed_time(&vehicle_land_detected.timestamp) < 10_s)) {
        ...
        if (_vehicle_gps_position_sub.advertised()) {
            ...
            if (_vehicle_gps_position_sub.copy(&vehicle_gps_position) && (hrt_elapsed_time(&vehicle_gps_position.timestamp) < 10_s)) {
        ...
        if (_home_position_sub.advertised() && _vehicle_local_position_sub.updated()) {
            ...
            if (_home_position_sub.copy(&home_position) && _vehicle_local_position_sub.copy(&vehicle_local_position) && (hrt_elapsed_time(&vehicle_local_position.timestamp) < 1_s)) {
