...
private:
    explicit MavlinkStreamUTMGlobalPosition(Mavlink *mavlink) :
    ...
    bool send() override {
        ...
        if (_global_pos_sub.update(&global_pos)) {
            ...
            if (vehicle_status.timestamp > 0 && land_detected.timestamp > 0 && vehicle_status.arming_state == vehicle_status_s::ARMING_STATE_ARMED) {
