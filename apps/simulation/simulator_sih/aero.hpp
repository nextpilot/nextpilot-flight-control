...
private:
    ...
    float fCL(float a) {
        return 0.25f * (1.0f + sqrtf(_fte)) * (1.0f + sqrtf(_fte)) * (_kp * sinf(a) * cosf(a) * cosf(a) + _fle * _fle * KV * fabsf(sinf(a)) * sinf(a) * cosf(a));
    }

    float fCM(float a) {
        return -0.25f * (1.0f + sqrtf(_fte)) * (1.0f + sqrtf(_fte)) * 0.0625f * (-1.0f + 6.0f * sqrtf(_fte) - 5.0f * _fte) * _kp * sinf(a) * cosf(a) + 0.17f * _fle * _fle * KV * fabsf(sinf(a)) * sinf(a);
    }
