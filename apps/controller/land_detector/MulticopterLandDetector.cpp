...
bool MulticopterLandDetector::_get_ground_contact_state() {
    ...
    if (_flag_control_climb_rate_enabled) {
        ...
        if (_trajectory_setpoint_sub.update(&trajectory_setpoint)) {
            ...
            _in_descend = PX4_ISFINITE(trajectory_setpoint.velocity[2]) && (trajectory_setpoint.velocity[2] >= 1.1f * _param_lndmc_z_vel_max.get());
        }
    ...
    return !_armed ||
           (_close_to_ground_or_skipped_check && ground_contact && !_horizontal_movement && !_vertical_movement);
}
...
bool MulticopterLandDetector::_get_maybe_landed_state() {
    ...
    return !_armed ||
           (minimum_thrust_now && !_freefall_hysteresis.get_state() && !_rotational_movement && ((vertical_estimate && _ground_contact_hysteresis.get_state()) || (!vertical_estimate && _minimum_thrust_8s_hysteresis.get_state())));
}
