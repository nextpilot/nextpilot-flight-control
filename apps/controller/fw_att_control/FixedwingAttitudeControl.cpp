    ...
    if (_vcontrol_mode.flag_control_manual_enabled && _in_fw_or_transition_wo_tailsitter_transition) {
        ...
        if (_manual_control_setpoint_sub.copy(&_manual_control_setpoint)) {
            if (!_vcontrol_mode.flag_control_climb_rate_enabled & _vcontrol_mode.flag_control_attitude_enabled) {
                ...
                _att_sp.pitch_body = -_manual_control_setpoint.pitch * radians(_param_fw_man_p_max.get()) + radians(_param_fw_psp_off.get());
...
float FixedwingAttitudeControl::get_airspeed_constrained() {
    ...
    const bool airspeed_valid = PX4_ISFINITE(_airspeed_validated_sub.get().calibrated_airspeed_m_s) && (hrt_elapsed_time(&_airspeed_validated_sub.get().timestamp) < 1_s);
    ...
    if ((_param_fw_arsp_mode.get() == 0) && airspeed_valid) {
        ...
        if (_vehicle_status.is_vtol && _vehicle_status.vehicle_type == vehicle_status_s::VEHICLE_TYPE_ROTARY_WING && !_vehicle_status.in_transition_mode) {
...
void FixedwingAttitudeControl::Run() {
    ...
    if (_att_sub.updated() || (hrt_elapsed_time(&_last_run) > 20_ms)) {
        ...
        const bool is_in_transition_except_tailsitter = _vehicle_status.in_transition_mode && !_vehicle_status.is_vtol_tailsitter;
        ...
        if (_vcontrol_mode.flag_control_rates_enabled) {
             ...
             */
            if (_att_sp.reset_integral || _landed || !_in_fw_or_transition_wo_tailsitter_transition) {
            ...
            if (_vcontrol_mode.flag_control_manual_enabled) {
                ...
                wheel_u = wheel_control ? _wheel_ctrl.control_bodyrate(dt, control_input) + _att_sp.yaw_sp_move_rate : 0.f;
            }
