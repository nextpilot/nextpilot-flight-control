...
int FixedwingPositionControl::init() {
...
int FixedwingPositionControl::parameters_update() {
    ...
    if (_param_fw_airspd_trim.get() < _param_fw_airspd_min.get() || _param_fw_airspd_trim.get() > _param_fw_airspd_max.get()) {
        /* EVENT
         * @description
         * - <param>FW_AIRSPD_MAX</param>: {1:.1}
         * - <param>FW_AIRSPD_MIN</param>: {2:.1}
...
void FixedwingPositionControl::vehicle_command_poll() {
    ...
    while (_vehicle_command_sub.update(&vehicle_command)) {
        if (vehicle_command.command == vehicle_command_s::VEHICLE_CMD_DO_GO_AROUND) {
            ...
            if (_control_mode.flag_control_auto_enabled && _position_setpoint_current_valid && (_pos_sp_triplet.current.type == position_setpoint_s::SETPOINT_TYPE_LAND)) {
            ...
            if ((static_cast<uint8_t>(vehicle_command.param1 + .5f) == vehicle_command_s::SPEED_TYPE_AIRSPEED)) {
                if (vehicle_command.param2 > FLT_EPSILON) { // param2 is an equivalent airspeed setpoint
                    ...
                    } else if (_control_mode_current == FW_POSCTRL_MODE_MANUAL_ALTITUDE || _control_mode_current == FW_POSCTRL_MODE_MANUAL_POSITION) {
...
void FixedwingPositionControl::airspeed_poll() {
    ...
    if ((_param_fw_arsp_mode.get() == 0) && _airspeed_validated_sub.update(&airspeed_validated)) {
        ...
        if (PX4_ISFINITE(airspeed_validated.calibrated_airspeed_m_s) && PX4_ISFINITE(airspeed_validated.true_airspeed_m_s) && (airspeed_validated.calibrated_airspeed_m_s > 0.0f)) {
            airspeed_valid = true;
...
float FixedwingPositionControl::adapt_airspeed_setpoint(const float control_interval, float calibrated_airspeed_setpoint,
...
void FixedwingPositionControl::tecs_status_publish(float alt_sp, float equivalent_airspeed_sp,
...
void FixedwingPositionControl::updateLandingAbortStatus(const uint8_t new_abort_status) {
    ...
    if (!_flare_states.flaring || new_abort_status == position_controller_landing_status_s::ABORTED_BY_OPERATOR) {
        ...
        _landing_abort_status = (new_abort_status >= position_controller_landing_status_s::UNKNOWN_ABORT_CRITERION) ? position_controller_landing_status_s::UNKNOWN_ABORT_CRITERION : new_abort_status;
...
void FixedwingPositionControl::updateManualTakeoffStatus() {
    if (!_completed_manual_takeoff) {
        const bool at_controllable_airspeed = _airspeed > _param_fw_airspd_min.get() || !_airspeed_valid;
        const bool is_hovering              = _vehicle_status.vehicle_type == vehicle_status_s::VEHICLE_TYPE_ROTARY_WING && _control_mode.flag_armed;
        _completed_manual_takeoff           = (!_landed && at_controllable_airspeed) || is_hovering;
    }
}

void FixedwingPositionControl::set_control_mode_current(const hrt_abstime &now) {
    ...
    if (((_control_mode.flag_control_auto_enabled && _control_mode.flag_control_position_enabled) || _control_mode.flag_control_offboard_enabled) && (_position_setpoint_current_valid || _pos_sp_triplet.current.type == position_setpoint_s::SETPOINT_TYPE_IDLE)) {
    ...
    } else if (_control_mode.flag_control_auto_enabled && _control_mode.flag_control_climb_rate_enabled && _control_mode.flag_armed // only enter this modes if armed, as pure failsafe modes
               && !_control_mode.flag_control_position_enabled) {
        // failsafe modes engaged if position estimate is invalidated

        if (commanded_position_control_mode != FW_POSCTRL_MODE_AUTO_ALTITUDE && commanded_position_control_mode != FW_POSCTRL_MODE_AUTO_CLIMBRATE) {
...
void FixedwingPositionControl::control_auto(const float control_interval, const Vector2d &curr_pos,
    ...
    if (position_sp_type == position_setpoint_s::SETPOINT_TYPE_LOITER || current_sp.type == position_setpoint_s::SETPOINT_TYPE_LOITER) {
...
FixedwingPositionControl::handle_setpoint_type(const position_setpoint_s &pos_sp_curr) {
    ...
    if (pos_sp_curr.type == position_setpoint_s::SETPOINT_TYPE_POSITION || pos_sp_curr.type == position_setpoint_s::SETPOINT_TYPE_LOITER) {
        float dist_xy = -1.f;
        float dist_z  = -1.f;
        ...
        if (pos_sp_curr.type == position_setpoint_s::SETPOINT_TYPE_POSITION) {
            ...
            if ((!_vehicle_status.in_transition_mode) && (dist >= 0.f) && (dist_z > _param_nav_fw_alt_rad.get()) && (dist_xy < acc_rad || _position_sp_type == position_setpoint_s::SETPOINT_TYPE_LOITER)) {
...
void FixedwingPositionControl::control_auto_position(const float control_interval, const Vector2d &curr_pos,
    ...
    if (_position_setpoint_previous_valid && ((pos_sp_prev.type == position_setpoint_s::SETPOINT_TYPE_POSITION) || (pos_sp_prev.type == position_setpoint_s::SETPOINT_TYPE_LOITER))) {
    ...
    if (_control_mode.flag_control_offboard_enabled && PX4_ISFINITE(pos_sp_curr.vx) && PX4_ISFINITE(pos_sp_curr.vy)) {
        ...
        float            curvature = PX4_ISFINITE(_pos_sp_triplet.current.loiter_radius) ? 1 / _pos_sp_triplet.current.loiter_radius : 0.0f;
...
void FixedwingPositionControl::control_auto_loiter(const float control_interval, const Vector2d &curr_pos,
    ...
    if (pos_sp_next.type == position_setpoint_s::SETPOINT_TYPE_LAND && _position_setpoint_next_valid && close_to_circle && _param_fw_lnd_earlycfg.get()) {
...
void FixedwingPositionControl::control_auto_takeoff(const hrt_abstime &now, const float control_interval,
    ...
    const float takeoff_airspeed = (_param_fw_tko_airspd.get() > FLT_EPSILON) ? _param_fw_tko_airspd.get() : _param_fw_airspd_min.get();
    ...
    if (_runway_takeoff.runwayTakeoffEnabled()) {
        ...
        if (!_skipping_takeoff_detection && _param_fw_laun_detcn_on.get() && _launchDetector.getLaunchDetected() < launch_detection_status_s::STATE_FLYING) {
            if (_control_mode.flag_armed) {
                /* Perform launch detection */
        ...
        if (!_launch_detected && _launchDetector.getLaunchDetected() > launch_detection_status_s::STATE_WAITING_FOR_LAUNCH && _param_fw_laun_detcn_on.get()) {
        ...
        if (_launchDetector.getLaunchDetected() > launch_detection_status_s::STATE_WAITING_FOR_LAUNCH && _param_fw_laun_detcn_on.get()) {
            ...
            const float max_takeoff_throttle = (_launchDetector.getLaunchDetected() < launch_detection_status_s::STATE_FLYING) ? _param_fw_thr_idle.get() : _param_fw_thr_max.get();
...
void FixedwingPositionControl::control_auto_landing_straight(const hrt_abstime &now, const float control_interval,
                                                             const Vector2f &ground_speed, const position_setpoint_s &pos_sp_prev, const position_setpoint_s &pos_sp_curr) {
    // first handle non-position things like airspeed and tecs settings
    const float airspeed_land         = (_param_fw_lnd_airspd.get() > FLT_EPSILON) ? _param_fw_lnd_airspd.get() : _param_fw_airspd_min.get();
    float       adjusted_min_airspeed = _param_fw_airspd_min.get();
    ...
    const float glide_slope_reference_alt            = (_param_fw_lnd_useter.get() == TerrainEstimateUseOnLanding::kFollowTerrainRelativeLandingGlideSlope) ? terrain_alt : pos_sp_curr.alt;
    ...
    if ((_current_altitude < terrain_alt + flare_rel_alt) || _flare_states.flaring) {
        ...
        // tune up the lateral position control guidance when on the ground
        const float ground_roll_npfg_period = flare_ramp_interpolator * _param_rwto_npfg_period.get() + (1.0f - flare_ramp_interpolator) * _param_npfg_period.get();
        ...
        const float height_rate_setpoint = flare_ramp_interpolator_sqrt * (-_param_fw_lnd_fl_sink.get()) + (1.0f - flare_ramp_interpolator_sqrt) * _flare_states.initial_height_rate_setpoint;

        float pitch_min_rad = flare_ramp_interpolator_sqrt * radians(_param_fw_lnd_fl_pmin.get()) + (1.0f - flare_ramp_interpolator_sqrt) * radians(_param_fw_p_lim_min.get());
        float pitch_max_rad = flare_ramp_interpolator_sqrt * radians(_param_fw_lnd_fl_pmax.get()) + (1.0f - flare_ramp_interpolator_sqrt) * radians(_param_fw_p_lim_max.get());
        ...
        if (_param_fw_lnd_td_time.get() > FLT_EPSILON) {
            ...
            const float touchdown_interpolator = math::constrain((seconds_since_flare_start - touchdown_time) / POST_TOUCHDOWN_CLAMP_TIME,
                                                                 0.0f,
                                                                 1.0f);
        ...
        const float throttle_max = flare_ramp_interpolator_sqrt * _param_fw_thr_idle.get() + (1.0f - flare_ramp_interpolator_sqrt) * _param_fw_thr_max.get();
        ...
        // control
        const float blended_throttle = flare_ramp_interpolator * get_tecs_thrust() + (1.0f - flare_ramp_interpolator) * _flare_states.initial_throttle_setpoint;
...
void FixedwingPositionControl::control_auto_landing_circular(const hrt_abstime &now, const float control_interval,
                                                             const Vector2f &ground_speed, const position_setpoint_s &pos_sp_curr) {
    // first handle non-position things like airspeed and tecs settings
    const float airspeed_land         = (_param_fw_lnd_airspd.get() > FLT_EPSILON) ? _param_fw_lnd_airspd.get() : _param_fw_airspd_min.get();
    float       adjusted_min_airspeed = _param_fw_airspd_min.get();
...
void FixedwingPositionControl::control_manual_altitude(const float control_interval, const Vector2d &curr_pos,
    ...
    // just passed launch
    const float min_pitch = (_completed_manual_takeoff) ? radians(_param_fw_p_lim_min.get()) : MIN_PITCH_DURING_MANUAL_TAKEOFF;
...
void FixedwingPositionControl::control_manual_position(const float control_interval, const Vector2d &curr_pos,
    ...
    if (fabsf(_manual_control_setpoint.roll) < HDG_HOLD_MAN_INPUT_THRESH && fabsf(_manual_control_setpoint.yaw) < HDG_HOLD_MAN_INPUT_THRESH) {
    ...
    if (!_yaw_lock_engaged || fabsf(_manual_control_setpoint.roll) >= HDG_HOLD_MAN_INPUT_THRESH || fabsf(_manual_control_setpoint.yaw) >= HDG_HOLD_MAN_INPUT_THRESH) {
        _hdg_hold_enabled = false;
        _yaw_lock_engaged = false;
...
void FixedwingPositionControl::Run() {
    ...
    if (_local_pos_sub.update(&_local_pos)) {
        ...
        if (_control_mode.flag_control_manual_enabled) {
            ...
            if (_control_mode.flag_control_altitude_enabled && _control_mode.flag_control_velocity_enabled && _local_pos.vxy_reset_counter != _pos_reset_counter) {
        ...
        if (!_global_local_proj_ref.isInitialized() || (_global_local_proj_ref.getProjectionReferenceTimestamp() != _local_pos.ref_timestamp) || (_local_pos.vxy_reset_counter != _pos_reset_counter)) {
        ...
        if (_control_mode.flag_control_offboard_enabled) {
            ...
            if (_trajectory_setpoint_sub.update(&trajectory_setpoint)) {
                ...
                if (Vector3f(trajectory_setpoint.velocity).isAllFinite()) {
                    ...
                    if (Vector3f(trajectory_setpoint.acceleration).isAllFinite()) {
                        ...
                        Vector2f acceleration_normal          = acceleration_sp_2d - acceleration_sp_2d.dot(normalized_velocity_sp_2d) * normalized_velocity_sp_2d;
                        float    direction                    = -normalized_velocity_sp_2d.cross(acceleration_normal.normalized());
                        _pos_sp_triplet.current.loiter_radius = direction * velocity_sp_2d.norm() * velocity_sp_2d.norm() / acceleration_normal.norm();
            ...
            if (_pos_sp_triplet_sub.update(&_pos_sp_triplet)) {
                _position_setpoint_previous_valid = PX4_ISFINITE(_pos_sp_triplet.previous.lat) && PX4_ISFINITE(_pos_sp_triplet.previous.lon) && PX4_ISFINITE(_pos_sp_triplet.previous.alt);

                _position_setpoint_current_valid = PX4_ISFINITE(_pos_sp_triplet.current.lat) && PX4_ISFINITE(_pos_sp_triplet.current.lon) && PX4_ISFINITE(_pos_sp_triplet.current.alt);

                _position_setpoint_next_valid = PX4_ISFINITE(_pos_sp_triplet.next.lat) && PX4_ISFINITE(_pos_sp_triplet.next.lon) && PX4_ISFINITE(_pos_sp_triplet.next.alt);
        ...
        if (_control_mode_current != FW_POSCTRL_MODE_AUTO_LANDING_STRAIGHT && _control_mode_current != FW_POSCTRL_MODE_AUTO_LANDING_CIRCULAR) {
            reset_landing_state();
        }
        ...
        if (_control_mode_current != FW_POSCTRL_MODE_OTHER) {
            ...
            if (_control_mode.flag_control_position_enabled || _control_mode.flag_control_velocity_enabled || _control_mode.flag_control_acceleration_enabled || _control_mode.flag_control_altitude_enabled || _control_mode.flag_control_climb_rate_enabled) {
        ...
        if (_new_landing_gear_position != old_landing_gear_position && _new_landing_gear_position != landing_gear_s::GEAR_KEEP) {
        ...
        if (_control_mode.flag_control_auto_enabled && _vehicle_status.vehicle_type == vehicle_status_s::VEHICLE_TYPE_FIXED_WING) {
...
float FixedwingPositionControl::calculateTrimThrottle(float throttle_min,
    ...
    const float slope_below_trim = (_param_fw_thr_trim.get() - _param_fw_thr_aspd_min.get()) / (_param_fw_airspd_trim.get() - _param_fw_airspd_min.get());
    const float slope_above_trim = (_param_fw_thr_aspd_max.get() - _param_fw_thr_trim.get()) / (_param_fw_airspd_max.get() - _param_fw_airspd_trim.get());

    if (PX4_ISFINITE(airspeed_sp) && PX4_ISFINITE(slope_below_trim) && _param_fw_thr_aspd_min.get() > FLT_EPSILON && airspeed_sp < _param_fw_airspd_trim.get()) {
        throttle_trim = _param_fw_thr_trim.get() - slope_below_trim * (_param_fw_airspd_trim.get() - airspeed_sp);

    } else if (PX4_ISFINITE(airspeed_sp) && PX4_ISFINITE(slope_above_trim) && _param_fw_thr_aspd_max.get() > FLT_EPSILON && airspeed_sp > _param_fw_airspd_trim.get()) {
...
float FixedwingPositionControl::constrainRollNearGround(const float roll_setpoint, const float altitude,
...
void FixedwingPositionControl::initializeAutoLanding(const hrt_abstime &now, const position_setpoint_s &pos_sp_prev,
...
FixedwingPositionControl::calculateTouchdownPosition(const float control_interval, const Vector2f &local_land_position) {
    if (fabsf(_manual_control_setpoint.yaw) > MANUAL_TOUCHDOWN_NUDGE_INPUT_DEADZONE && _param_fw_lnd_nudge.get() > LandingNudgingOption::kNudgingDisabled && !_flare_states.flaring) {
        ...
        const float signed_deadzone_threshold  = MANUAL_TOUCHDOWN_NUDGE_INPUT_DEADZONE * math::signNoZero(_manual_control_setpoint.yaw);
        _lateral_touchdown_position_offset    += (_manual_control_setpoint.yaw - signed_deadzone_threshold) * MAX_TOUCHDOWN_POSITION_NUDGE_RATE * control_interval;
...
float FixedwingPositionControl::getLandingTerrainAltitudeEstimate(const hrt_abstime &now, const float land_point_altitude,
...
void FixedwingPositionControl::navigateWaypoints(const Vector2f &start_waypoint, const Vector2f &end_waypoint,
    ...
    if ((start_waypoint_to_end_waypoint.dot(start_waypoint_to_vehicle) < -FLT_EPSILON) && (start_waypoint_to_vehicle.norm() > _npfg.switchDistance(500.0f))) {
