...
void RTL::on_inactive() {
    ...
    if ((hrt_absolute_time() - _destination_check_time) > 1_s) {
        ...
        const bool global_position_recently_updated = global_position.timestamp > 0 && hrt_elapsed_time(&global_position.timestamp) < 10_s;
...
void RTL::find_RTL_destination() {
    ...
    const bool vtol_in_rw_mode = _navigator->get_vstatus()->is_vtol && _navigator->get_vstatus()->vehicle_type == vehicle_status_s::VEHICLE_TYPE_ROTARY_WING;
    ...
    if (_param_rtl_cone_half_angle_deg.get() > 0 && _navigator->get_vstatus()->vehicle_type == vehicle_status_s::VEHICLE_TYPE_ROTARY_WING) {
...
void RTL::advance_rtl() {
    ...
    const bool vtol_in_fw_mode = _navigator->get_vstatus()->is_vtol && _navigator->get_vstatus()->vehicle_type == vehicle_status_s::VEHICLE_TYPE_FIXED_WING;
...
void RTL::calcRtlTimeEstimate(const RTLState rtl_state, rtl_time_estimate_s &rtl_time_estimate) {
    ...
    rtl_time_estimate.safe_time_estimate = _param_rtl_time_factor.get() * rtl_time_estimate.time_estimate + _param_rtl_time_margin.get();
}
