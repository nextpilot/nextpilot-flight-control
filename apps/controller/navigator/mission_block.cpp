...
bool MissionBlock::is_mission_item_reached_or_completed() {
    ...
    if (!_navigator->get_land_detected()->landed && !_waypoint_position_reached) {
        ...
        if ((_mission_item.nav_cmd == NAV_CMD_TAKEOFF || _mission_item.nav_cmd == NAV_CMD_VTOL_TAKEOFF) && _navigator->get_vstatus()->vehicle_type == vehicle_status_s::VEHICLE_TYPE_ROTARY_WING) {
        ...
        } else if (_mission_item.nav_cmd == NAV_CMD_TAKEOFF && _navigator->get_vstatus()->vehicle_type == vehicle_status_s::VEHICLE_TYPE_FIXED_WING) {
        ...
        } else if (_mission_item.nav_cmd == NAV_CMD_TAKEOFF && _navigator->get_vstatus()->vehicle_type == vehicle_status_s::VEHICLE_TYPE_ROVER) {
            ...
            if (dist >= 0.0f && dist_xy <= (_navigator->get_acceptance_radius() + mission_item_loiter_radius_abs) && dist_z <= _navigator->get_altitude_acceptance_radius()) {
                _waypoint_position_reached = true;
            }
            ...
            } else if (dist >= 0.f && dist_xy <= (_navigator->get_acceptance_radius() + mission_item_loiter_radius_abs) && dist_z <= _navigator->get_altitude_acceptance_radius()) {
            ...
            if (_navigator->get_vstatus()->vehicle_type == vehicle_status_s::VEHICLE_TYPE_ROTARY_WING && PX4_ISFINITE(_mission_item.acceptance_radius) && _mission_item.acceptance_radius > FLT_EPSILON) {
            ...
            if (_mission_item.vtol_back_transition && _navigator->get_vstatus()->vehicle_type == vehicle_status_s::VEHICLE_TYPE_FIXED_WING) {
    ...
    if (_waypoint_position_reached && !_waypoint_yaw_reached) {
        if (_navigator->get_vstatus()->vehicle_type == vehicle_status_s::VEHICLE_TYPE_ROTARY_WING && PX4_ISFINITE(_navigator->get_yaw_acceptance(_mission_item.yaw)) && _navigator->get_local_position()->heading_good_for_control) {
            const float yaw_err = wrap_pi(_mission_item.yaw - _navigator->get_local_position()->heading);

            /* accept yaw if reached or if timeout is set in which case we ignore not forced headings */
            if (fabsf(yaw_err) < _navigator->get_yaw_threshold() || (_navigator->get_yaw_timeout() >= FLT_EPSILON && !_mission_item.force_heading)) {
                _waypoint_yaw_reached = true;
            }
    ...
    if (_waypoint_position_reached && _waypoint_yaw_reached) {
        ...
        if (time_inside_reached) {
            ...
            const bool enforce_exit_course = _navigator->get_vstatus()->vehicle_type != vehicle_status_s::VEHICLE_TYPE_ROTARY_WING && next_sp.valid && curr_sp_new->type == position_setpoint_s::SETPOINT_TYPE_LOITER && (_mission_item.force_heading || _mission_item.nav_cmd == NAV_CMD_WAYPOINT);
...
float MissionBlock::get_time_inside(const mission_item_s &item) const {
    if ((item.nav_cmd == NAV_CMD_WAYPOINT && _navigator->get_vstatus()->vehicle_type == vehicle_status_s::VEHICLE_TYPE_ROTARY_WING) ||
...
bool MissionBlock::item_has_timeout(const mission_item_s &item) {
...
bool MissionBlock::item_contains_gate(const mission_item_s &item) {
    return item.nav_cmd == NAV_CMD_CONDITION_GATE;
}

bool MissionBlock::item_contains_marker(const mission_item_s &item) {
...
bool MissionBlock::mission_item_to_position_setpoint(const mission_item_s &item, position_setpoint_s *sp) {
    ...
    case NAV_CMD_VTOL_TAKEOFF:
        ...
        if ((_navigator->get_vstatus()->arming_state == vehicle_status_s::ARMING_STATE_ARMED) && !_navigator->get_land_detected()->landed && !_navigator->get_land_detected()->maybe_landed) {
