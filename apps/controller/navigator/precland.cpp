...
bool PrecLand::check_state_conditions(PrecLandState state) {
    ...
    case PrecLandState::HorizontalApproach:
        ...
        if (_state == PrecLandState::HorizontalApproach) {
            if (fabsf(_target_pose.x_abs - vehicle_local_position->x) < _param_pld_hacc_rad.get() && fabsf(_target_pose.y_abs - vehicle_local_position->y) < _param_pld_hacc_rad.get()) {
    ...
    case PrecLandState::DescendAboveTarget:
        ...
        if (_state == PrecLandState::DescendAboveTarget) {
            ...
            return _target_pose_updated && _target_pose.abs_pos_valid && fabsf(_target_pose.x_abs - vehicle_local_position->x) < _param_pld_hacc_rad.get() && fabsf(_target_pose.y_abs - vehicle_local_position->y) < _param_pld_hacc_rad.get();
        }

    case PrecLandState::FinalApproach:
        return _target_pose_valid && _target_pose.abs_pos_valid && (_target_pose.z_abs - vehicle_local_position->z) < _param_pld_fappr_alt.get();
