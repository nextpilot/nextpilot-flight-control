...
void McAutotuneAttitudeControl::Run() {
    ...
    if (!_vehicle_torque_setpoint_sub.copy(&vehicle_torque_setpoint) || !_vehicle_angular_velocity_sub.copy(&angular_velocity)) {
        return;
    }
    ...
    if (hrt_elapsed_time(&_last_publish) > _publishing_dt_hrt || _last_publish == 0) {
        ...
        const float desired_rise_time = ((_state == state::yaw) || (_state == state::yaw_pause)) ? 0.2f : _param_mc_at_rise_time.get();
...
void McAutotuneAttitudeControl::updateStateMachine(hrt_abstime now) {
    ...
    case state::roll:
        if (areAllSmallerThan(_sys_id.getVariances(), converged_thr) && ((now - _state_start_time) > 5_s)) {
            copyGains(0);
    ...
    case state::test:
        ...
        } else if ((now - _state_start_time) < 4_s && (now - _state_start_time) > 1_s && _control_power.longerThan(0.1f)) {
            _state = state::fail;
            revertParamGains();
            _state_start_time = now;
        }
    ...
    if (_state != state::wait_for_disarm && _state != state::idle && (((now - _state_start_time) > 20_s) || (fabsf(manual_control_setpoint.roll) > 0.05f) || (fabsf(manual_control_setpoint.pitch) > 0.05f))) {
        _state            = state::fail;
        _state_start_time = now;
    }
}
...
bool McAutotuneAttitudeControl::areAllSmallerThan(const Vector<float, 5> &vect, float threshold) const {
    return (vect(0) < threshold) && (vect(1) < threshold) && (vect(2) < threshold) && (vect(3) < threshold) && (vect(4) < threshold);
}
...
bool McAutotuneAttitudeControl::areGainsGood() const {
    const bool are_positive = _rate_k.min() > 0.f && _rate_i.min() > 0.f && _rate_d.min() >= 0.f && _att_p.min() > 0.f;

    const bool are_small_enough = _rate_k.max() < 0.5f && _rate_i.max() < 10.f && _rate_d.max() < 0.1f && _att_p.max() < 12.f;
