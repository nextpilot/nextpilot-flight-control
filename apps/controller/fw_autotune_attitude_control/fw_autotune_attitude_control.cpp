...
void FwAutotuneAttitudeControl::Run() {
    ...
    if ((_state == state::idle && !_aux_switch_en) || !_vehicle_torque_setpoint_sub.updated()) {
        return;
    }
...
void FwAutotuneAttitudeControl::updateStateMachine(hrt_abstime now) {
    ...
    case state::fail:
        ...
        if ((now - _state_start_time) > 2_s) {
            ...
            mavlink_log_info(&_mavlink_log_pub, "Autotune returned to idle");
            _state = state::idle;
    ...
    if (_state != state::wait_for_disarm && _state != state::idle && _state != state::fail && _state != state::complete) {
        if (now - _state_start_time > 20_s || (_param_fw_at_man_aux.get() && !_aux_switch_en) || _start_flight_mode != _nav_state) {
            // orb_advert_t mavlink_log_pub = nullptr;
...
bool FwAutotuneAttitudeControl::areGainsGood() const {
    ...
    for (int i = 0; i < 3; i++) {
        if (((i == 0) && (_param_fw_at_axes.get() & Axes::roll)) || ((i == 1) && (_param_fw_at_axes.get() & Axes::pitch)) || ((i == 2) && (_param_fw_at_axes.get() & Axes::yaw))) {
