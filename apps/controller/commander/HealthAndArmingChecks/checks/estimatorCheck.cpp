...
#define LOG_TAG "estimator_check"
...
void EstimatorChecks::checkEstimatorStatus(const Context &context, Report &reporter,
    ...
    if ((_param_com_arm_mag_str.get() >= 1) && (!context.isArmed() && estimator_status.pre_flt_fail_mag_field_disturbed)) {
...
void EstimatorChecks::checkEstimatorStatusFlags(const Context &context, Report &reporter,
    ...
    if (_estimator_status_flags_sub.copy(&estimator_status_flags)) {
        bool dead_reckoning = estimator_status_flags.cs_wind_dead_reckoning || estimator_status_flags.cs_inertial_dead_reckoning;
    ...
    if (context.status().vehicle_type == vehicle_status_s::VEHICLE_TYPE_ROTARY_WING) {
        if (!context.isArmed()) {
            ...
            if (!_nav_test_passed) {
                ...
                const bool innovation_pass = (estimator_status.vel_test_ratio < 1.f) && (estimator_status.pos_test_ratio < 1.f) && (estimator_status.vel_test_ratio > FLT_EPSILON) && (estimator_status.pos_test_ratio > FLT_EPSILON);
...
void EstimatorChecks::lowPositionAccuracy(const Context &context, Report &reporter,
    ...
    const bool local_position_valid_but_low_accuracy = !reporter.failsafeFlags().local_position_invalid && (_param_com_low_eph.get() > FLT_EPSILON && lpos.eph > _param_com_low_eph.get());
