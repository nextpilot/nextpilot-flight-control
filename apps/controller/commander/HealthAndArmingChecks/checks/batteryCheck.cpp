...
void BatteryChecks::checkAndReport(const Context &context, Report &reporter) {
            ...
            if (battery.faults > 0) {
                ...
                for (uint8_t fault_index = 0; fault_index <= static_cast<uint8_t>(battery_fault_reason_t::_max); fault_index++) {
                    if (battery.faults & (1 << fault_index)) {
                         ...
                         */
                        reporter.healthFailure<uint8_t, battery_fault_reason_t, events::px4::enums::suggested_action_t, uint32_t>(NavModes::All, health_component_t::battery, events::ID("check_battery_fault"), {events::Log::Emergency, events::LogInternal::Warning},
    ...
    if (reporter.failsafeFlags().battery_warning > battery_status_s::BATTERY_WARNING_NONE && reporter.failsafeFlags().battery_warning < battery_status_s::BATTERY_WARNING_FAILED) {
    ...
    reporter.failsafeFlags().battery_unhealthy =
        ...
        || battery_has_fault || reporter.failsafeFlags().battery_warning == battery_status_s::BATTERY_WARNING_FAILED;
...
void BatteryChecks::rtlEstimateCheck(const Context &context, Report &reporter, float worst_battery_time_s) {
    ...
    reporter.failsafeFlags().battery_low_remaining_time = _rtl_time_estimate_sub.copy(&rtl_time_estimate) && (hrt_absolute_time() - rtl_time_estimate.timestamp) < 2_s && rtl_time_estimate.valid && context.isArmed() && PX4_ISFINITE(worst_battery_time_s) && rtl_time_estimate.safe_time_estimate >= worst_battery_time_s;
