...
void MagnetometerChecks::checkAndReport(const Context &context, Report &reporter) {
    ...
    for (int instance = 0; instance < _sensor_mag_sub.size(); instance++) {
        ...
        if (exists) {
            ...
            is_valid = _sensor_mag_sub[instance].copy(&mag_data) && (mag_data.device_id != 0) && (mag_data.timestamp != 0) && (hrt_elapsed_time(&mag_data.timestamp) < 1_s);
