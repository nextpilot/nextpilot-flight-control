...
void FailureDetector::updateImbalancedPropStatus() {
    ...
    if (imu_status.accel_device_id != _selected_accel_device_id) {
        for (unsigned i = 0; i < ORB_MULTI_MAX_INSTANCES; i++) {
            ...
            if (_vehicle_imu_status_sub.copy(&imu_status) && (imu_status.accel_device_id == _selected_accel_device_id)) {
                // instance found
                break;
            }
        }
    ...
    if (updated) {
        if (_vehicle_imu_status_sub.copy(&imu_status)) {
            if ((imu_status.accel_device_id != 0) && (imu_status.accel_device_id == _selected_accel_device_id)) {
    ...
    if (vehicle_status.arming_state == vehicle_status_s::ARMING_STATE_ARMED) {
        ...
        for (int esc_status_idx = 0; esc_status_idx < limited_esc_count; esc_status_idx++) {
            ...
            if (_motor_failure_escs_have_current) {
                ...
                if (_motor_failure_undercurrent_start_time[i_esc] != 0 && (time_now - _motor_failure_undercurrent_start_time[i_esc]) > _param_fd_motor_time_thres.get() * 1_ms && (_motor_failure_esc_under_current_mask & (1 << i_esc)) == 0) {
                    // Set flag
