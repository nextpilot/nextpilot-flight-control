...
static int command_start() {
    static const char *argv[] = {"commander", "start"};
...
transition_result_t Commander::disarm(arm_disarm_reason_t calling_reason, bool forced) {
    if (!forced) {
        const bool landed                = (_vehicle_land_detected.landed || _vehicle_land_detected.maybe_landed || is_ground_vehicle(_vehicle_status));
        const bool mc_manual_thrust_mode = _vehicle_status.vehicle_type == vehicle_status_s::VEHICLE_TYPE_ROTARY_WING && _vehicle_control_mode.flag_control_manual_enabled && !_vehicle_control_mode.flag_control_climb_rate_enabled;
        const bool commanded_by_rc       = (calling_reason == arm_disarm_reason_t::rc_stick) || (calling_reason == arm_disarm_reason_t::rc_switch) || (calling_reason == arm_disarm_reason_t::rc_button);
...
bool Commander::handle_command(const vehicle_command_s &cmd) {
    ...
    case vehicle_command_s::VEHICLE_CMD_COMPONENT_ARM_DISARM: {
        ...
        if (arming_action != vehicle_command_s::ARMING_ACTION_ARM && arming_action != vehicle_command_s::ARMING_ACTION_DISARM) {
            ...
            if (!forced && cmd_from_io && (cmd.source_system == _vehicle_status.system_id) && (cmd.source_component == _vehicle_status.component_id) && (arming_action == vehicle_command_s::ARMING_ACTION_ARM)) {
            ...
            if (arming_res == TRANSITION_DENIED) {
                ...
                if ((arming_action == vehicle_command_s::ARMING_ACTION_ARM) && (arming_res == TRANSITION_CHANGED) && (hrt_absolute_time() > (_boot_timestamp + INAIR_RESTART_HOLDOFF_INTERVAL)) && (_param_com_home_en.get())) {
    ...
    case vehicle_command_s::VEHICLE_CMD_MISSION_START: {
        ...
        if (!_failsafe_flags.auto_mission_missing) {
            ...
            if (PX4_ISFINITE(cmd.param1) && (cmd.param1 >= -1) && (cmd.param1 < _mission_result_sub.get().seq_total)) {
                ...
                if (_user_mode_intention.change(vehicle_status_s::NAVIGATION_STATE_AUTO_MISSION) && (TRANSITION_DENIED != arm(arm_disarm_reason_t::mission_start))) {
                    cmd_result = vehicle_command_ack_s::VEHICLE_CMD_RESULT_ACCEPTED;
    ...
    case vehicle_command_s::VEHICLE_CMD_PREFLIGHT_CALIBRATION: {
        if (_arm_state_machine.isArmed() || _arm_state_machine.isShutdown() || _worker_thread.isBusy()) {
            ...
            if (TRANSITION_DENIED == _arm_state_machine.arming_state_transition(_vehicle_status, vehicle_status_s::ARMING_STATE_INIT, _actuator_armed, _health_and_arming_checks, false /* fRunPreArmChecks */, &_mavlink_log_pub, (cmd.from_external ? arm_disarm_reason_t::command_external : arm_disarm_reason_t::command_internal))) {
                answer_command(cmd, vehicle_command_ack_s::VEHICLE_CMD_RESULT_DENIED);
                break;
            }
...
void Commander::executeActionRequest(const action_request_s &action_request) {
    ...
    if (_vehicle_status.rc_calibration_in_progress && (action_request.source == action_request_s::SOURCE_RC_STICK_GESTURE || action_request.source == action_request_s::SOURCE_RC_SWITCH || action_request.source == action_request_s::SOURCE_RC_BUTTON || action_request.source == action_request_s::SOURCE_RC_MODE_SLOT)) {
        return;
    }
...
void Commander::updateParameters() {
    ...
    const bool is_rotary = is_rotary_wing(_vehicle_status) || (is_vtol(_vehicle_status) && _vtol_vehicle_status.vehicle_vtol_state != vtol_vehicle_status_s::VEHICLE_VTOL_STATE_FW);
...
void Commander::checkForMissionUpdate() {
    if (_mission_result_sub.updated()) {
        ...
        if (_arm_state_machine.isArmed() && !_vehicle_land_detected.landed && (mission_result.timestamp >= _vehicle_status.nav_state_timestamp) && mission_result.finished) {
            if (_vehicle_status.nav_state == vehicle_status_s::NAVIGATION_STATE_AUTO_TAKEOFF || _vehicle_status.nav_state == vehicle_status_s::NAVIGATION_STATE_AUTO_VTOL_TAKEOFF) {
...
void Commander::vtolStatusUpdate() {
    ...
    if (_vtol_vehicle_status_sub.update(&_vtol_vehicle_status) && is_vtol(_vehicle_status)) {
        ...
        if (_vehicle_status.in_transition_to_fw != (_vtol_vehicle_status.vehicle_vtol_state == vtol_vehicle_status_s::VEHICLE_VTOL_STATE_TRANSITION_TO_FW)) {
            _vehicle_status.in_transition_to_fw = (_vtol_vehicle_status.vehicle_vtol_state == vtol_vehicle_status_s::VEHICLE_VTOL_STATE_TRANSITION_TO_FW);
            _status_changed                     = true;
        }
    }
}
...
void Commander::handleAutoDisarm() {
    ...
    if (_arm_state_machine.isArmed()) {
        ...
        if (_param_com_disarm_land.get() > 0 || _param_com_disarm_preflight.get() > 0) {
            const bool landed_amid_mission = (_vehicle_status.nav_state == vehicle_status_s::NAVIGATION_STATE_AUTO_MISSION) && !_mission_result_sub.get().finished;
...
void Commander::checkAndInformReadyForTakeoff() {
    ...
    if (_vehicle_status.vehicle_type == vehicle_status_s::VEHICLE_TYPE_ROTARY_WING || _vehicle_status.vehicle_type == vehicle_status_s::VEHICLE_TYPE_FIXED_WING) {
...
void Commander::dataLinkCheck() {
    ...
    if ((_datalink_last_heartbeat_onboard_controller > 0) && (hrt_elapsed_time(&_datalink_last_heartbeat_onboard_controller) > (_param_com_obc_loss_t.get() * 1_s)) && !_onboard_controller_lost) {
    ...
    // OpenDroneID system
    if ((hrt_elapsed_time(&_datalink_last_heartbeat_open_drone_id_system) > 3_s) && !_open_drone_id_system_lost) {
...
void Commander::manualControlCheck() {
    ...
    if (manual_control_updated && manual_control_setpoint.valid) {
        ...
        if (_arm_state_machine.isArmed()) {
            ...
            if (manual_control_setpoint.sticks_moving && !_vehicle_control_mode.flag_control_manual_enabled && (_vehicle_status.vehicle_type == vehicle_status_s::VEHICLE_TYPE_ROTARY_WING)) {
