...
public:
    FailsafeTester(ModuleParams *parent) :
        FailsafeBase(parent) {
    }
...
protected:
    void checkStateAndMode(const hrt_abstime &time_us, const State &state,
        ...
        _last_state_test = checkFailsafe(_caller_id_test, _last_state_test, status_flags.fd_motor_failure && status_flags.fd_critical_failure, ActionOptions(Action::Terminate).cannotBeDeferred());
    }
