...
void Failsafe::checkStateAndMode(const hrt_abstime &time_us, const State &state,
    ...
    const bool ignore_link_failsafe = state.user_intended_mode == vehicle_status_s::NAVIGATION_STATE_AUTO_VTOL_TAKEOFF && in_forward_flight && !state.mission_finished;
    ...
    if (state.user_intended_mode == vehicle_status_s::NAVIGATION_STATE_AUTO_MISSION) {
        ...
        if ((_param_com_rc_in_mode.get() == int32_t(offboard_loss_failsafe_mode::Land_mode) || rc_loss_ignored_mission) && _param_nav_dll_act.get() == int32_t(gcs_connection_loss_failsafe_mode::Disabled) && state.mission_finished) {
    ...
    // Failure detector
    if ((_armed_time != 0) && (time_us < _armed_time + static_cast<hrt_abstime>(_param_com_spoolup_time.get() * 1_s))) {
        CHECK_FAILSAFE(status_flags, fd_esc_arming_failure, ActionOptions(Action::Disarm).cannotBeDeferred());
    }

    if ((_armed_time != 0) && (time_us < _armed_time + static_cast<hrt_abstime>((_param_com_lkdown_tko.get() + _param_com_spoolup_time.get()) * 1_s))) {
...
FailsafeBase::Action Failsafe::checkModeFallback(const failsafe_flags_s &status_flags,
    ...
    case position_control_navigation_loss_response::Altitude_Manual: // AltCtrl/Manual
        ...
        // PosCtrl -> AltCtrl
        if (user_intended_mode == vehicle_status_s::NAVIGATION_STATE_POSCTL && !modeCanRun(status_flags, user_intended_mode)) {
            action             = Action::FallbackAltCtrl;
            user_intended_mode = vehicle_status_s::NAVIGATION_STATE_ALTCTL;
        }

        // AltCtrl -> Stabilized
        if (user_intended_mode == vehicle_status_s::NAVIGATION_STATE_ALTCTL && !modeCanRun(status_flags, user_intended_mode)) {
            action             = Action::FallbackStab;
            user_intended_mode = vehicle_status_s::NAVIGATION_STATE_STAB;
        }
...
uint8_t Failsafe::modifyUserIntendedMode(Action previous_action, Action current_action,
    ...
    if ((int)previous_action > (int)Action::Warn && modeFromAction(current_action, user_intended_mode) == vehicle_status_s::NAVIGATION_STATE_ORBIT) {
