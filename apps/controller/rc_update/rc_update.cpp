...
void RCUpdate::Run() {
    ...
    if (_input_rc_sub.update(&input_rc)) {
        ...
        if (!input_rc.rc_lost) {
            ...
            if ((_input_source_previous != input_rc.input_source) && (_input_source_previous != input_rc_s::RC_INPUT_SOURCE_UNKNOWN)) {
        ...
        if (input_source_stable && channel_count_stable && !_rc_signal_lost_hysteresis.get_state()) {
            if ((input_rc.timestamp_last_signal > _last_timestamp_signal) && (input_rc.timestamp_last_signal < _last_timestamp_signal + VALID_DATA_MIN_INTERVAL_US)) {
...
void RCUpdate::UpdateManualSwitches(const hrt_abstime &timestamp_sample) {
    ...
    if ((switches == _manual_switches_previous) && (switches.timestamp_sample < _manual_switches_previous.timestamp_sample + VALID_DATA_MIN_INTERVAL_US)) {
...
extern "C" __EXPORT int rc_update_start() {
    const char *argv[] = {"rc_update", "start"};
