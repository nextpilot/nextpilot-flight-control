...
void FlightModeManager::start_flight_task() {
    ...
    // Orbit
    if ((_vehicle_status_sub.get().nav_state == vehicle_status_s::NAVIGATION_STATE_ORBIT) && !_command_failed) {
        found_some_task       = true;
        FlightTaskError error = FlightTaskError::InvalidTask;
...
void FlightModeManager::handleCommand() {
    ...
    while (_vehicle_command_sub.update(&command)) {
        ...
        if (_current_task.task) {
            ...
            if ((command.command == vehicle_command_s::VEHICLE_CMD_DO_CHANGE_SPEED) && (static_cast<uint8_t>(command.param1 + .5f) == vehicle_command_s::SPEED_TYPE_GROUNDSPEED) && (command.param2 > 0.f)) {
