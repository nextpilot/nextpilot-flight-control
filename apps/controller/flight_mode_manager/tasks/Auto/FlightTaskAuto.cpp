...
bool FlightTaskAuto::_evaluateTriplets() {
    ...
    if (PX4_ISFINITE(cruise_speed_from_triplet) && (_sub_triplet_setpoint.get().current.timestamp > _time_last_cruise_speed_override)) {
    ...
    if (!PX4_ISFINITE(_sub_triplet_setpoint.get().current.lat) || !PX4_ISFINITE(_sub_triplet_setpoint.get().current.lon)) {
    ...
    if (_param_com_obs_avoid.get() && _sub_vehicle_status.get().vehicle_type == vehicle_status_s::VEHICLE_TYPE_ROTARY_WING) {
    ...
    if (_weathervane.isActive()) {
        ...
        } else if ((_type != WaypointType::takeoff || _sub_triplet_setpoint.get().current.disable_weather_vane) && _sub_triplet_setpoint.get().current.yaw_valid) {
