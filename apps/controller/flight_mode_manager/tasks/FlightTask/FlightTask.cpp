...
void FlightTask::_evaluateVehicleLocalPosition() {
    ...
    if ((_time_stamp_current - _sub_vehicle_local_position.get().timestamp) < _timeout) {
        ...
        // distance to bottom
        if (_sub_vehicle_local_position.get().dist_bottom_valid && PX4_ISFINITE(_sub_vehicle_local_position.get().dist_bottom)) {
        ...
        if (_sub_vehicle_local_position.get().xy_global && _sub_vehicle_local_position.get().z_global) {
            if (!_geo_projection.isInitialized() || (_geo_projection.getProjectionReferenceTimestamp() != _sub_vehicle_local_position.get().ref_timestamp)) {
