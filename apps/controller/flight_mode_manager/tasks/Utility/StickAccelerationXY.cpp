...
void StickAccelerationXY::generateSetpoints(Vector2f stick_xy, const float yaw, const float yaw_sp, const Vector3f &pos,
    ...
    // to discretization
    if (((_acceleration_setpoint.norm_squared() < FLT_EPSILON) || (sign(_acceleration_setpoint_prev(0)) != sign(_acceleration_setpoint(0))) || (sign(_acceleration_setpoint_prev(1)) != sign(_acceleration_setpoint(1)))) && (_velocity_setpoint.norm_squared() < (drag.norm_squared() * dt * dt))) {
        drag.setZero();
