...
NextPilot采用RT-Thread国产实时嵌入式系统，启动流程是在RT-Thread启动基础上增加了自动驾驶相关的业务逻辑。

NextPilot所有模块都是独立运行的，并采用[RT-Thread自动初始化机制](#rt-thread-自动初始化机制)进行启动，在同一个初始化阶段里面`启动函数`的**执行顺序一般由编译顺序确定**。
