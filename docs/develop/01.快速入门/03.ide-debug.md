...
NextPilot支持 Keil5 MDK 和 VS Code 两种集成开发环境，两者都是先通过scons工具生成相应的 Keil 工程和 VS Code 配置，然后再使用 IDE 进行编译、调试、烧录等。

> 注意：飞行仿真sitl/qemu只支持VS Code+Gcc，不支持Keil；普通STM32程序支持Vscode和Keil，但是推荐Keil。
