...
This build guide shows how to add a flight controller system using Auterion Skynode evaluation kit or [Pixhawk 6C](../01.基本概念/pixhawk6c.md) and setup PX4.
...
- Auterion Skynode evaluation kit
- [Pixhawk 6C](../01.基本概念/pixhawk6c.md)
...
- GPS F9P (included in Skynode eval. kit(../21.周边设备/04.传 感 器/01.卫星导航/rtk_gps_holybro_h-rtk-f9p.md)
- GPS M9N (cheaper alternative to F9P(../21.周边设备/04.传 感 器/01.卫星导航/rtk_gps_holybro_h-rtk-m8p.md)
...
- Lidar Lightware lw20-c (included in Skynode eval. kit(../21.周边设备/04.传 感 器/sfxx_lidar.md) (Optional)
...
- [Radio (RC) System](../21.周边设备/01.遥 控 器/index.md) of your preference
...
If no lidar is mounted you should disable using fixed-wing actuation in hover to accelerate forward (set [VT_FWD_THRUST_EN](../19.高级调参/parameter_reference.md#VT_FWD_THRUST_EN) to `0` instead of `1`).
:::
...
If you wish connect the actuators to different outputs you will need to modify the Actuator Output mappings (see [Actuator Configuration](../07.标准配置/actuators.md)).
...
1. Select the [Airframe](../07.标准配置/02.select-airframe.md) tab
1. Select [Generic Standard VTOL](../09.机型调参/01.airframe_reference.md#vtol_standard_vtol_generic_standard_vtol) from the _Standard VTOL_ group, and then and click **Apply and Restart**.
...
1. Select the [Parameters](../19.高级调参/parameters.md#finding-updating-parameters) tab and then click on **Tools** in the top right corner.
1. Select **Load from file** and then choose the `loong.params` file you just downloaded.
1. Reboot the vehicle.

### Sensor Selection

- If the Lidar Lightware lw20-c (included in Skynode eval. kit(../21.周边设备/04.传 感 器/sfxx_lidar.md) is used, [SENS_EN_SF1XX](../19.高级调参/parameter_reference.md#SENS_EN_SF1XX) needs to be set to 6 (SF/LW/20c).
- Make that the correct airspeed sensor is selected.
  If you use the recommended [SDP33 airspeed sensor](https://www.dualrc.com/parts/airspeed-sensor-sdp33) no changes will be needed as [SENS_EN_SDP3X](../19.高级调参/parameter_reference.md#SENS_EN_SDP3X) is enabled (set to `1`) in the parameter file.

### Sensor Calibration

First make sure to set the [correct orientation of the flight controller](../07.标准配置/05.flight_controller_orientation.md).
...
- [Airspeed](../07.标准配置/12.airspeed.md)

### RC-Setup

[Calibrate your RC Controller](../07.标准配置/radio.md) and setup the [flight mode switches](../07.标准配置/flight_mode.md).

We recommend you assign RC switches for the set of modes defined in [Flight Mode Configuration > What Flight Modes and Switches Should I Set?](../07.标准配置/flight_mode.md#what-flight-modes-and-switches-should-i-set).
In particular you should assign a _VTOL Transition Switch_, _Kill Switch_, and a switch to select [Stabilized mode](../05.基本概念/01.飞行模式/stabilized.md) and [Position mode](../05.基本概念/01.飞行模式/position.md).
...
Motors, control surfaces, and other actuators are configured in the QGroundControl [Actuator Configuration & Testing](../07.标准配置/actuators.md).
...
- Switch into [Manual mode](../05.基本概念/01.飞行模式/stabilized.md)
...
Arm the vehicle with the propellers still detached in [Stabilized mode](../05.基本概念/01.飞行模式/stabilized.md).
...
::: info
increase the throttle a bit if you can't see a reaction since [Airmode](../09.机型调参/11.多旋翼/pid_tuning_guide_multicopter.md#airmode-mixer-saturation) is not enabled for the yaw axis.
:::
...
- Check control surface reactions in [Stabilized mode](../05.基本概念/01.飞行模式/stabilized.md).
...
- If a [kill-switch](../07.标准配置/99.safety.md#kill-switch) is used, make sure it's working properly and will not be activated accidentally in flight!
- Arm in [Stabilized mode](../05.基本概念/01.飞行模式/stabilized.md) and check if motors respond to the commands, e.g. roll left increases throttle on the right motor
- Takeoff in [Stabilized mode](../05.基本概念/01.飞行模式/stabilized.md) and make some basic maneuvers
- If everything went without any issue, takeoff in [Position mode](../05.基本概念/01.飞行模式/position.md) and do a transition at around 50m.
