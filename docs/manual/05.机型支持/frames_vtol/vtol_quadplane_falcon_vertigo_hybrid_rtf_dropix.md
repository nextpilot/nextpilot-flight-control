...
::: info
General information about connecting Dropix can be found in [Dropix Flight Controller](../01.基本概念/dropix.md).
:::
...
Motor and servo wiring is nearly entirely up to you, but should match the [Generic Standard VTOL](../09.机型调参/01.airframe_reference.md#vtol_standard_vtol_generic_standard_vtol) configuration, as shown in the airframe reference.
The geometry and output assignment can be configured in the [Actuators Configuration](../07.标准配置/actuators.md#actuator-outputs)
...
1. Check that the motors turn in the correct directions (as in the QuadX diagram below).
   ...
   ::: info
   If necessary the servo direction can be reversed using the `Rev Range (for servos)` checkbox associated with each servo output in the QGroundControl [Actuator Output](../07.标准配置/actuators.md#actuator-outputs) configuration (for servos only) (this sets the [PWM_AUX_REV](../19.高级调参/parameter_reference.md#PWM_AUX_REV) or [PWM_AUX_MAIN](../19.高级调参/parameter_reference.md#PWM_MAIN_REV) parameter).
   :::
...
Perform the normal [Basic Configuration](../07.标准配置/index.md).

Notes:

1. For [Airframe](../07.标准配置/02.select-airframe.md) select the vehicle group/type as *Standard VTOL* and the specific vehicle as [Generic Standard VTOL](../09.机型调参/01.airframe_reference.md#vtol_standard_vtol_generic_standard_vtol) as shown below.

   ![QCG - Select Generic Standard VTOL](../../assets/qgc/setup/airframe/px4_frame_generic_standard_vtol.png)

1. Set the [Autopilot Orientation](../07.标准配置/05.flight_controller_orientation.md) to `ROTATION_YAW_270` as the autopilot is mounted [sideways](#flight_controller_orientation) with respect to the front of the vehicle. The compass is oriented forward, so you can leave that at the default (`ROTATION_NONE`).
1. Configure the outputs and geometry following the instructions in [Actuators Configuration](../07.标准配置/actuators.md)
