...
There is not much space but it's enough for a [Pixfalcon](../01.基本概念/pixfalcon.md) flight controller with GPS and telemetry.

::: info
The original Horizon Hobby *E-Flite Convergence* frame and [Pixfalcon](../01.基本概念/pixfalcon.md) have been discontinued.
...
- [Pixhawk 4 Mini](../01.基本概念/pixhawk4_mini.md)
- [Holybro Pixhawk Mini](../01.基本概念/pixhawk_mini.md).
...
The outputs are configured in the [Actuators Configuration](../07.标准配置/actuators.md) by following the instructions for VTOL tiltrotor geometry and output configuration.
Note that you will need to start from the [Generic Tiltrotor VTOL](../09.机型调参/01.airframe_reference.md#vtol_vtol_tiltrotor_generic_tiltrotor_vtol) frame.
...
Follow the [Standard Configuration](../07.标准配置/index.md) in *QGroundControl* (radio, sensors, flight modes, etc.).
...
- [Flight Modes/Switches](../07.标准配置/flight_mode.md)
  - As this is a VTOL vehicle, you must [assign an RC controller switch](../07.标准配置/flight_mode.md#what-flight-modes-and-switches-should-i-set) for transitioning between multicopter and fixed-wing modes.
