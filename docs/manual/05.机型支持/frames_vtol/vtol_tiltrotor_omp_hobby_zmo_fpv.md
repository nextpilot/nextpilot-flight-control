...
The OMP Hobby ZMO is a small [tiltrotor VTOL](../09.机型调参/frames_vtol/tiltrotor.md) that is available in an RTF kit.
This build guide shows add a flight controller system (using Auterion Skynode evaluation kit, [Pixhawk 6C](../01.基本概念/pixhawk6c.md) or [Pixhawk 6C mini](../01.基本概念/pixhawk6c_mini.md)) and setup PX4.
...
- [Pixhawk 6C](../01.基本概念/pixhawk6c.md) with [PM02 V3](../21.周边设备/12.电源模块/holybro_pm02.md)
- [Pixhawk 6C mini](../01.基本概念/pixhawk6c_mini.md) with [PM02 V3](../21.周边设备/12.电源模块/holybro_pm02.md)
...
An inexpensive example would be a SiK Telemetry Radio.
:::
...
1. Select [Generic Tiltrotor VTOL](../09.机型调参/01.airframe_reference.md#vtol_vtol_tiltrotor_generic_tiltrotor_vtol) from the _VTOL Tiltrotor_ group, and then and click **Apply and Restart**.
...
The airspeed sensor can be enabled in the [Parameters](../19.高级调参/parameters.md#finding-updating-parameters) tab.
...
- If the Lidar Lightware lw20-c (included in Skynode eval. kit(../21.周边设备/04.传 感 器/sfxx_lidar.md) is used, `SENS_EN_SF1XX` needs to be set to 6 (SF/LW/20c).
...
The direction can't be changed in software because the vehicle does not use [DShot ESC](../21.周边设备/dshot.md).

## First Flight

- Check tilt rotor reactions in [Stabilized mode](../05.基本概念/01.飞行模式/stabilized.md). Keep the throttle stick at the minimum and place the vehicle at the ground. To enable the tilt servos you need to arm the vehicle.
...
- Check control surface reactions in [Stabilized mode](../05.基本概念/01.飞行模式/stabilized.md). Switch the vehicle into forward flight mode.
