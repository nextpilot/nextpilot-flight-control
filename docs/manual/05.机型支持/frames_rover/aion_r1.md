...
For this build this includes an Auterion Skynode, connected to a Pixhawk Adapter Board that interfaces with the RoboClaw motor controllers over serial.
...
1. In the [Basic Configuration](../07.标准配置/index.md) section, select the [Airframe](../07.标准配置/02.select-airframe.md) tab.
...
1. Navigate to the [Parameters](../19.高级调参/parameters.md) section in QGroundControl.

   - Set the [RBCLW_SER_CFG](../19.高级调参/parameter_reference.md#RBCLW_SER_CFG) parameter to the serial port to which the RoboClaw is connected (such as `GPS2`).
   - [RBCLW_COUNTS_REV](../19.高级调参/parameter_reference.md#RBCLW_COUNTS_REV) specifies the number of encoder counts required for one wheel revolution.
   ...
   - RoboClaw motor controllers must be assigned a unique address on the bus.
     The default address is 128 and you should not need to change this (if you do, update the PX4 parameter [RBCLW_ADDRESS](../19.高级调参/parameter_reference.md#RBCLW_ADDRESS) to match).
...
1. Navigate to [Actuators Configuration & Testing](../07.标准配置/actuators.md) in QGroundControl.
