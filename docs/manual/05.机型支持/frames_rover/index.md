...
:::warning
Support for rover is [experimental](../01.airframe_reference.md#experimental-vehicles).
Maintainer volunteers, [contribution]index.md) of new features, new frame configurations, or other improvements would all be very welcome!
:::
...
The supported frames can be seen in [Airframes Reference > Rover](../09.机型调参/01.airframe_reference.md#rover).
...
1. In the [Airframe](../07.标准配置/02.select-airframe.md) configuration, select the _Generic Ground Vehicle_.

   ![Select Ackermann steered airframe](../../assets/config/airframe/airframe_rover_ackermann.png)

   Select the **Apply and Restart** button.

1. Open the [Actuators Configuration & Testing](../07.标准配置/actuators.md) to map the steering and throttle functions to flight controller outputs.

### Differential Steering Configuration

1. In the [Airframe](../07.标准配置/02.select-airframe.md) configuration, select either the _Aion Robotics R1 UGV_ or _NXP Cup car: DF Robot GPX_

   ![Select Differential steered airframe](../../assets/config/airframe/airframe_rover_aion.png)

Select the **Apply and Restart** button.

1. Open the [Actuators Configuration & Testing](../07.标准配置/actuators.md) and map the left and right motor functions to flight controller outputs.
