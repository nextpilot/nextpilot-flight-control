...
::: info
Before filter tuning you should do a first pass at [Basic MC PID tuning](../09.机型调参/11.多旋翼/pid_tuning_guide_multicopter_basic.md).
...
- The maximum gyro publication rate (configured with [IMU_GYRO_RATEMAX](../19.高级调参/parameter_reference.md#IMU_GYRO_RATEMAX)).
...
- A notch filter on the gyro sensor data that is used to filter out narrow band noise, for example harmonics at the rotor blade pass frequency.
  This filter can be configured using [IMU_GYRO_NF0_BW](../19.高级调参/parameter_reference.md#IMU_GYRO_NF0_BW) and [IMU_GYRO_NF0_FRQ](../19.高级调参/parameter_reference.md#IMU_GYRO_NF0_FRQ).
- Low-pass filter on the gyro sensor data.
  It can be configured with the [IMU_GYRO_CUTOFF](../19.高级调参/parameter_reference.md#IMU_GYRO_CUTOFF) parameter.
...
- A separate low-pass filter on the D-term.
  ...
  For this reason the D-term has a separately-configurable low-pass filter, [IMU_DGYRO_CUTOFF](../19.高级调参/parameter_reference.md#IMU_DGYRO_CUTOFF).
- A slewrate filter on the motor outputs ([MOT_SLEW_MAX](../19.高级调参/parameter_reference.md#MOT_SLEW_MAX)).
  Generally not used.
...
First make sure to have the high-rate logging profile activated ([SDLOG_PROFILE](../19.高级调参/parameter_reference.md#SDLOG_PROFILE) parameter).
[Flight Review](../15.数据分析/flight_reporting.md) will then show an FFT plot for the roll, pitch and yaw controls.
...
The performed flight maneuver can simply be hovering in [Manual/Stabilized mode](../05.基本概念/01.飞行模式/manual_stabilized.md) with some rolling and pitching to all directions and some increased throttle periods.
...
First tune the gyro filter [IMU_GYRO_CUTOFF](../19.高级调参/parameter_reference.md#IMU_GYRO_CUTOFF) by increasing it in steps of 10 Hz while using a low D-term filter value ([IMU_DGYRO_CUTOFF](../19.高级调参/parameter_reference.md#IMU_DGYRO_CUTOFF) = 30).
...
In this case you might use the settings: [IMU_GYRO_NF0_FRQ=32](../19.高级调参/parameter_reference.md#IMU_GYRO_NF0_FRQ) and [IMU_GYRO_NF0_BW=5](../19.高级调参/parameter_reference.md#IMU_GYRO_NF0_BW) (note, this spike is narrower than usual).
