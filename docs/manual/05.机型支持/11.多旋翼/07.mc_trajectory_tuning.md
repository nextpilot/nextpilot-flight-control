...
:::tip
Follow the instructions in the [Multicopter PID Tuning Guide](../09.机型调参/11.多旋翼/pid_tuning_guide_multicopter.md) _before_ doing any of the tuning described here.
...
[PID Tuning](../09.机型调参/11.多旋翼/pid_tuning_guide_multicopter.md) ("Lower level tuning") aims to reduce the error between the desired setpoint and the estimate of the vehicle state.
...
:::tip
Poorly tuned [P/PID Gains](../09.机型调参/11.多旋翼/pid_tuning_guide_multicopter.md) can lead to instability.
...
[Mission mode](../05.基本概念/01.飞行模式/mission.md) used the [Jerk-limited](../09.机型调参/11.多旋翼/mc_jerk_limited_type_trajectory.md) trajectory all the time.

[Position mode](../05.基本概念/01.飞行模式/position.md) supports the [implementations](#position-mode-implementations) listed below.
It uses the acceleration based mapping by default; other types can be set using [MPC_POS_MODE](../19.高级调参/parameter_reference.md#MPC_POS_MODE).

[Altitude mode](../05.基本概念/01.飞行模式/altitude.md) similarly supports the [implementations](#altitude-mode-implementations) selected by [MPC_POS_MODE](../19.高级调参/parameter_reference.md#MPC_POS_MODE), but _only_ for smoothing the vertical component (i.e. when controlling the altitude).
...
- [Jerk-limited](../09.机型调参/11.多旋翼/mc_jerk_limited_type_trajectory.md)
