...
This page describes how to setup and configure a racer for optimal performance (in particular for [Acro mode](../05.基本概念/01.飞行模式/acro.md)).
...
- [Return mode](../05.基本概念/01.飞行模式/return.md) can be used, either on a switch or as RC loss/low battery failsafe.
...
If you switch into [position mode](../05.基本概念/01.飞行模式/position.md) during that time, [altitude mode](../05.基本概念/01.飞行模式/altitude.md) will be used instead until the position becomes valid again.
:::

## Hardware Setup

The following paragraphs describe a few important points when building the vehicle.
If you need complete build instructions, you can follow the [QAV-R 5" KISS ESC Racer](../09.机型调参/frames_multicopter/qav_r_5_kiss_esc_racer.md) build log.

### Vibration Setup

There are various mounting approaches to reduce vibrations.
For example, the flight controller can be mounted with vibration dampening foam, or using [O-rings](../09.机型调参/frames_multicopter/qav_r_5_kiss_esc_racer.md#mounting).

While there is no single best method, you will typically have fewer problems with vibrations if you use high-quality components (frame, motors, props) as for example used in the [QAV-R 5" KISS ESC Racer](../09.机型调参/frames_multicopter/qav_r_5_kiss_esc_racer.md).
...
Go through the [Basic Configuration Guide](../07.标准配置/index.md).
In particular, set the [Airframe](../07.标准配置/02.select-airframe.md) that most closely matches your frame (typically you will choose the [Generic 250 Racer](../09.机型调参/01.airframe_reference.md#copter_quadrotor_x_generic_250_racer) airframe, which sets some racer-specific parameters by default).

These parameters are important:

- Enable One-Shot or DShot by selecting the protocol for a group of outputs during [Actuator Configuration](../07.标准配置/actuators.md).
- Set the maximum roll-, pitch- and yaw rates for Manual/Stabilized mode as
  desired: [MC_ROLLRATE_MAX](../19.高级调参/parameter_reference.md#MC_ROLLRATE_MAX), [MC_PITCHRATE_MAX](../19.高级调参/parameter_reference.md#MC_PITCHRATE_MAX) and [MC_YAWRATE_MAX](../19.高级调参/parameter_reference.md#MC_YAWRATE_MAX).
  The maximum tilt angle is configured with [MPC_MAN_TILT_MAX](../19.高级调参/parameter_reference.md#MPC_MAN_TILT_MAX).
- The minimum thrust [MPC_MANTHR_MIN](../19.高级调参/parameter_reference.md#MPC_MANTHR_MIN) should be set to 0.
...
To select it, set [SYS_MC_EST_GROUP](../19.高级调参/parameter_reference.md#SYS_MC_EST_GROUP) to 1, and change the following parameters:

- Set [SYS_HAS_MAG](../19.高级调参/parameter_reference.md#SYS_HAS_MAG) to 0 if the system does not have a magnetometer.
- Set [SYS_HAS_BARO](../19.高级调参/parameter_reference.md#SYS_HAS_BARO) to 0 if the system does not have a barometer.
- Configure the Q estimator: set [ATT_ACC_COMP](../19.高级调参/parameter_reference.md#ATT_ACC_COMP) to 0, [ATT_W_ACC](../19.高级调参/parameter_reference.md#ATT_W_ACC) to 0.4 and [ATT_W_GYRO_BIAS](../19.高级调参/parameter_reference.md#ATT_W_GYRO_BIAS) to 0.
  You can tune these later if you wish.

### Failsafe

Configure [RC loss and low battery failsafe](../07.标准配置/99.safety.md).
...
Make sure to assign a [kill switch](../07.标准配置/99.safety.md#kill-switch) or an [arming switch](../07.标准配置/99.safety.md#arm-disarm-switch).
...
Assuming the vehicle is able to fly using the default settings, we then do a first pass of [Basic MC PID tuning](../09.机型调参/11.多旋翼/pid_tuning_guide_multicopter_basic.md).
...
- [Low-pass filters](../09.机型调参/11.多旋翼/filter_tuning.md) in software and on the sensor chip trade off increased latency for improved noise filtering.
...
- PWM output signal: enable [Dshot](../21.周边设备/dshot.md) by preference to reduce latency (or One-Shot if DShot is not supported).
  The protocol is selected for a group of outputs during [Actuator Configuration](../07.标准配置/actuators.md).

### Filter Tuning

Filters trade off control latency and noise filtering, both of which impact performance.
For information see: [Filter/Control Latency Tuning](../09.机型调参/11.多旋翼/filter_tuning.md)
...
:::tip
You can use the approach described in [Basic MC PID tuning](../09.机型调参/11.多旋翼/pid_tuning_guide_multicopter_basic.md) to tune the frame, but you will need to use the Advanced Multicopter PID Tuning Guide (Advanced/Detailed(../09.机型调参/11.多旋翼/pid_tuning_guide_multicopter.md#thrust-curve) to understand how to tune the thrust curve.

### Airmode

After you have verified that the vehicle flies well at low and high throttle, you can enable [airmode](../09.机型调参/11.多旋翼/pid_tuning_guide_multicopter.md#airmode) with the [MC_AIRMODE](../19.高级调参/parameter_reference.md#MC_AIRMODE) parameter.
