...
This topic explains how to configure a multicopter using selected topics from [Standard Configuration](../07.标准配置/index.md), [Advanced Configuration](../19.高级调参/index.md), and [Flight Controller Peripherals](../21.周边设备/index.md), along with multicopter-specific tuning topics.
...
The first step is to [load PX4 firmware](../07.标准配置/01.upload-firmware.md) onto your [flight controller](../01.基本概念/index.md).
...
- [Loading Firmware](../07.标准配置/01.upload-firmware.md)
...
First [select a multicopter airframe](../07.标准配置/02.select-airframe.md) (options are listed in [Airframe Reference > Copter](../09.机型调参/01.airframe_reference.md#copter)).
...
For example, for a [Quadrotor X](../09.机型调参/01.airframe_reference.md#quadrotor-x) frame you would look for the name of your frame in the list, and if it was not present select the [Generic Quadrotor X](../09.机型调参/01.airframe_reference.md#copter_quadrotor_x_generic_quadcopter) frame.
...
Selecting an airframe applies a [frame configuration file](../dev_airframes/adding_a_new_frame.md#adding-a-frame-configuration) that contains a predefined set of [parameters](../19.高级调参/parameters.md), such as [CA_AIRFRAME=0](../19.高级调参/parameter_reference.md#CA_AIRFRAME) for the vehicle type and [CA_ROTOR_COUNT](../19.高级调参/parameter_reference.md#CA_ROTOR_COUNT) for the number of rotors.
...
The next step is to define your vehicle [geometry](../07.标准配置/actuators.md#motor-geometry-multicopter) (the number of motors and their relative positions) and [assign those motors](../07.标准配置/actuators.md#actuator-outputs) to the physical outputs that they are wired to on your flight controller (both of these are covered in [Actuator Configuration and Testing](../07.标准配置/actuators.md)).

If using PWM ESCs and OneShot ESCs (but not DShot and DroneCAN/Cyphal ESC) you should then perform [ESC Calibration](../19.高级调参/esc_calibration.md) before proceeding to [Motor Configuration](../07.标准配置/actuators.md#motor-configuration).
...
The final step is [Motor Configuration](../07.标准配置/actuators.md#motor-configuration):

- [Reverse any motors](../07.标准配置/actuators.md#reversing-motors) that don't match the spin direction configured in the Geometry.
...
- [Vehicle (Frame) Selection](../07.标准配置/02.select-airframe.md) — Select vehicle type to match your frame.
- [Actuator Configuration and Testing](../07.标准配置/actuators.md) — Vehicle geometry, output mapping, motor configuration, testing.
- [ESC Calibration](../19.高级调参/esc_calibration.md) — Do between output mapping and motor configuration (topic above) for PWM and OneShot ESC.
...
We first need to set the [Sensor Orientation](../07.标准配置/05.flight_controller_orientation.md), informing PX4 how the autopilot (and its inbuilt sensors) and external compasses are oriented relative to the vehicle.
...
- [Sensor Orientation](../07.标准配置/05.flight_controller_orientation.md)
- [Compass](../07.标准配置/06.compass.md)
- [Gyroscope](../07.标准配置/09.gyroscope.md)
- [Accelerometer](../07.标准配置/07.accelerometer.md)

PX4 can use other peripherals, such as distance sensors, optical flow sensors, traffic avoidance alarms, cameras, and so on:

- [Flight Controller Peripherals](../21.周边设备/index.md) - Setup specific sensors, optional sensors, actuators, and so on.

::: info
Sensors that you don't need to calibrate/configure include:

- [Level Horizon](../07.标准配置/11.level_horizon_calibration.md) calibration isn't usually needed if you have mounted the flight controller level.
- Sensors that are not present, or that are not used by PX4 multicopter for flight control, such as [Airspeed sensors](../07.标准配置/12.airspeed.md).
...
- [Radio Controller (RC) Setup](../07.标准配置/radio.md)
- [Flight Mode Configuration](../07.标准配置/flight_mode.md)

Joystick/GamePad:

- [Joystick Setup](../07.标准配置/joystick.md) (includes button/flight mode mapping)
...
- [Battery Estimation Tuning](../07.标准配置/battery.md) — estimate remaining power (needed for low power failsafe).
- Safety Configuration (Failsafes(../07.标准配置/99.safety.md)
...
- [Autotune](../07.标准配置/98.autotune.md) — Automates tuning PX4 rate and attitude controllers (recommended).
...
- MC PID Tuning (Manual/Basic(../09.机型调参/11.多旋翼/pid_tuning_guide_multicopter_basic.md) — Manual tuning basic how to.
- MC PID Tuning Guide (Manual/Detailed(../09.机型调参/11.多旋翼/pid_tuning_guide_multicopter.md) — Manual tuning with detailed explanation.
- [MC Filter/Control Latency Tuning](../09.机型调参/11.多旋翼/filter_tuning.md) — Trade off control latency and noise filtering.
- MC Setpoint Tuning (Trajectory Generator(../09.机型调参/11.多旋翼/mc_trajectory_tuning.md)
  - [MC Jerk-limited Type Trajectory](../09.机型调参/11.多旋翼/mc_jerk_limited_type_trajectory.md)
- [Multicopter Racer Setup](../09.机型调参/11.多旋翼/racer_setup.md)
