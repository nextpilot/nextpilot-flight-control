...
This tutorial explains how to _manually_ tune the PID loops on PX4 for all [multicopter setups](../09.机型调参/01.airframe_reference.md#copter) (Quads, Hexa, Octo etc).
...
Generally if you're using an appropriate [supported frame configuration](../09.机型调参/01.airframe_reference.md#copter), the default tuning should allow you to fly the vehicle safely.
...
- Use [Position mode](../05.基本概念/01.飞行模式/position.md) to tune the _Velocity Controller_ and the _Position Controller_.
...
- You have selected the closest matching [default frame configuration](../07.标准配置/02.select-airframe.md) for your vehicle.
  This should give you a vehicle that already flies.
- You should have done an [ESC calibration](../19.高级调参/esc_calibration.md).
- If using PWM outputs their minimum values should be set correctly in the [Actuator Configuration](../07.标准配置/actuators.md).
  These need to be set low, but such that the **motors never stop** when the vehicle is armed.

  This can be tested in [Acro mode](../05.基本概念/01.飞行模式/acro.md) or in [Manual/Stabilized mode](../05.基本概念/01.飞行模式/manual_stabilized.md):
...
- Disable [MC_AIRMODE](../19.高级调参/parameter_reference.md#MC_AIRMODE) before tuning a vehicle (there is an options for this in the PID tuning screen).

:::warning
Poorly tuned vehicles are likely to be unstable, and easy to crash.
Make sure to have assigned a [Kill switch](../07.标准配置/99.safety.md#emergency-switches).
:::
...
1. Arm the vehicle, takeoff, and hover (typically in [Position mode](../05.基本概念/01.飞行模式/position.md)).
...
1. Set the _Thrust curve_ value to: 0.3 (PWM, power-based controllers) or 1 (RPM-based ESCs)
   ...
   - For PWM controllers, 0.3 is a good default (which may benefit from [further tuning](../09.机型调参/11.多旋翼/pid_tuning_guide_multicopter.md#thrust-curve)).
   - For RPM-based controllers, use 1 (no further tuning is required as these have a quadratic thrust curve).

   For more information see the [detailed PID tuning guide](../09.机型调参/11.多旋翼/pid_tuning_guide_multicopter.md#thrust-curve).
   :::

1. Set the _Select Tuning_ radio button to: **Roll**.
1. (Optionally) Select the **Automatic Flight Mode Switching** checkbox.
   This will _automatically_ switch from [Position mode](../05.基本概念/01.飞行模式/position.md) to [Stabilised mode](../05.基本概念/01.飞行模式/manual_stabilized.md) when you press the **Start** button
