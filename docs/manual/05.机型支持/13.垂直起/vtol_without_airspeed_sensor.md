...
Fixed-wing vehicles use [airspeed sensors](../21.周边设备/04.传 感 器/airspeed.md) to determine the speed at which the airplane is moving through the air.
...
After the reference flight download the log and use [FlightPlot](../15.数据分析/flight_log_analysis.md#flightplot) (or another analysis tool) to examine the log.
...
To bypass the airspeed preflight check you need to set [SYS_HAS_NUM_ASPD](../19.高级调参/parameter_reference.md#SYS_HAS_NUM_ASPD) to 0.

To prevent an installed airspeed sensor being used for feedback control set [FW_USE_AIRSPD](../19.高级调参/parameter_reference.md#FW_USE_AIRSPD) to `False`.
...
Set the trim throttle ([FW_THR_TRIM](../19.高级调参/parameter_reference.md#FW_THR_TRIM)) to the percentage as determined from the log of the reference flight.
...
Set the minimum front transition time ([VT_TRANS_MIN_TM](../19.高级调参/parameter_reference.md#VT_TRANS_MIN_TM)) to the number of seconds determined from the reference flight and add +- 30% for safety.

### Optional Recommended Parameters

Because the risk of stalling is real, it is recommended to set the 'fixed-wing minimum altitude' (a.k.a. 'quad-chute') threshold ([VT_FW_MIN_ALT](../19.高级调参/parameter_reference.md#VT_FW_MIN_ALT)).
...
The position estimator tested for this mode is EKF2, you can set this by changing the [SYS_MC_EST_GROUP](../19.高级调参/parameter_reference.md#SYS_MC_EST_GROUP).

## First Flight Without Airspeed Sensor

The values apply to a position controlled flight (like [Hold mode](../05.基本概念/01.飞行模式/hold.md) or [Mission mode](../flight_modes_vtol/mission.md) or Mission mode).
...
- [FW_USE_AIRSPD](../19.高级调参/parameter_reference.md#FW_USE_AIRSPD)
- [SYS_HAS_NUM_ASPD](../19.高级调参/parameter_reference.md#SYS_HAS_NUM_ASPD)
- [SYS_MC_EST_GROUP](../19.高级调参/parameter_reference.md#SYS_MC_EST_GROUP): EKF2 (2)
- [FW_THR_TRIM](../19.高级调参/parameter_reference.md#FW_THR_TRIM): determined (e.g. 70%)
- [VT_TRANS_MIN_TM](../19.高级调参/parameter_reference.md#VT_TRANS_MIN_TM): determined (e.g. 10 seconds)
- [VT_FW_MIN_ALT](../19.高级调参/parameter_reference.md#VT_FW_MIN_ALT): 15
