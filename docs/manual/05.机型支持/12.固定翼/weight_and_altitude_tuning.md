...
:::warning
This topic requires that you have already performed [basic TECS tuning](../09.机型调参/12.固定翼/position_tuning_guide_fixedwing.md#tecs-tuning-altitude-and-airspeed).
:::

[Basic TECS tuning](../09.机型调参/12.固定翼/position_tuning_guide_fixedwing.md#tecs-tuning-altitude-and-airspeed) established the key performance limitations of the vehicle that are required for the altitude and airspeed controller to function properly.
...
- [WEIGHT_BASE](../19.高级调参/parameter_reference.md#WEIGHT_BASE) — the weight of the vehicle at which the [Basic TECS tuning](../09.机型调参/12.固定翼/position_tuning_guide_fixedwing.md#tecs-tuning-altitude-and-airspeed) was performed.
- [WEIGHT_GROSS](../19.高级调参/parameter_reference.md#WEIGHT_BASE) — the actual weight of the vehicle at any given time, for example when using a larger battery, or with a payload that was not present during tuning.
...
In PX4 the service ceiling [FW_SERVICE_CEIL](../19.高级调参/parameter_reference.md#FW_SERVICE_CEIL) specifies the altitude in standard atmospheric conditions at which the vehicle is still able to achieve a maximum climb rate of 0.5 m/s at maximum throttle and weight equal to [WEIGHT_BASE](../19.高级调参/parameter_reference.md#WEIGHT_BASE).
...
The minimum sink rate is set in [FW_T_SINK_MIN](../19.高级调参/parameter_reference.md#FW_T_SINK_MIN).

If the [Basic TECS tuning](../09.机型调参/12.固定翼/position_tuning_guide_fixedwing.md#tecs-tuning-altitude-and-airspeed) was not done in standard sea level conditions then the [FW_T_SINK_MIN](../19.高级调参/parameter_reference.md#FW_T_SINK_MIN) parameter must be modified by multiplying with correction factor $P$ (where $\rho$ is the air density during tuning):
...
The trim throttle is set using [FW_THR_TRIM](../19.高级调参/parameter_reference.md#FW_THR_TRIM).

If basic tuning was not done in standard sealevel conditions then the value for [FW_THR_TRIM](../19.高级调参/parameter_reference.md#FW_THR_TRIM) must be modified by multiplying with correction factor $P$:
...
By calibrated we mean the value of that variable measured at sea level in standard atmospheric conditions, and when vehicle weight was equal to [WEIGHT_BASE](../19.高级调参/parameter_reference.md#WEIGHT_BASE).

E.g. by $\hat{\dot{h}}_{max}$ we specify the maximum climb rate the vehicle can achieve at [WEIGHT_BASE](../19.高级调参/parameter_reference.md#WEIGHT_BASE) at sea level in standard atmospheric conditions.

### Effect of Weight on Maximum Climb Rate

The maximum climb rate ([FW_T_CLMB_MAX](../19.高级调参/parameter_reference.md#FW_T_CLMB_MAX)) is scaled as a function of the weight ratio.
...
The minimum sink rate ([FW_T_SINK_MIN](../19.高级调参/parameter_reference.md#FW_T_SINK_MIN)) is scaled as a function of weight ratio
...
The minimum airspeed ([FW_AIRSPD_MIN](../19.高级调参/parameter_reference.md#FW_AIRSPD_MIN)), the stall airspeed ([FW_AIRSPD_STALL](../19.高级调参/parameter_reference.md#FW_AIRSPD_STALL)) and trim airspeed ([FW_AIRSPD_TRIM](../19.高级调参/parameter_reference.md#FW_AIRSPD_TRIM)) are adjusted based on the weight ratio specified by [WEIGHT_BASE](../19.高级调参/parameter_reference.md#WEIGHT_BASE) and [WEIGHT_GROSS](../19.高级调参/parameter_reference.md#WEIGHT_GROSS).
...
The maximum climb rate is set using [FW_T_CLMB_MAX](../19.高级调参/parameter_reference.md#FW_T_CLMB_MAX).
...
The minimum sink rate is set using [FW_T_SINK_MIN](../19.高级调参/parameter_reference.md#FW_T_SINK_MIN).
