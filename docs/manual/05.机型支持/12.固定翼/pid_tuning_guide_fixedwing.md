...
::: info
[Autotune](../07.标准配置/98.autotune.md) is recommended for most users, as it is far faster, easier and provides good tuning for most frames.
...
- Trims must be configured first (before PID turning).
  The [Fixed-Wing Trimming Guide](../09.机型调参/12.固定翼/trimming_guide_fixedwing.md) explains how.
- Incorrectly set gains during tuning can make attitude control unstable.
  A pilot tuning gains should therefore be able to fly and land the plane in [manual](../05.基本概念/01.飞行模式/manual.md) (override) control.
...
- Roll and pitch tuning follow the same sequence.
  The only difference is that pitch is more sensitive to trim offsets, so [trimming](../09.机型调参/12.固定翼/trimming_guide_fixedwing.md) has to be done carefully and integrator gains need more attention to compensate this.
...
- [FW_RR_I](../19.高级调参/parameter_reference.md#FW_RR_I)
- [FW_RR_P](../19.高级调参/parameter_reference.md#FW_RR_P)

#### Gains to tune

- [FW_RR_FF](../19.高级调参/parameter_reference.md#FW_RR_FF) - start with a value of 0.4.
...
- [FW_RR_P](../19.高级调参/parameter_reference.md#FW_RR_P) - start with a value of 0.06.
...
- [FW_RR_I](../19.高级调参/parameter_reference.md#FW_RR_I) - start with a value of 0.01.
...
- [FW_PR_I](../19.高级调参/parameter_reference.md#FW_PR_I)
- [FW_PR_P](../19.高级调参/parameter_reference.md#FW_PR_I)

#### Gains to tune

- [FW_PR_FF](../19.高级调参/parameter_reference.md#FW_PR_FF) - start with a value of 0.4.
  Increase this value (doubling each time) until the plane pitches satisfactory and reaches the setpoint.
  Back down the gain 20% at the end of the process.

### Tuning the Rate Gain

- [FW_PR_P](../19.高级调参/parameter_reference.md#FW_PR_P) - start with a value of 0.04.
  Increase this value (doubling each time) until the system starts to wobble / twitch.
  Then reduce value by 50%.

### Tuning the Trim Offsets with the Integrator Gain

- [FW_PR_I](../19.高级调参/parameter_reference.md#FW_PR_I) - start with a value of 0.01.
...
- [FW_P_TC](../19.高级调参/parameter_reference.md#FW_P_TC) - set to a default of 0.5 seconds, increase to make the Pitch response softer, decrease to make the response harder.
- [FW_R_TC](../19.高级调参/parameter_reference.md#FW_R_TC) - set to a default of 0.5 seconds, increase to make the Roll response softer, decrease to make the response harder.

## Other Tuning Parameters

The most important parameters are covered in this guide.
Additional tuning parameters are documented in the [Parameter Reference](../19.高级调参/parameter_reference.md).
