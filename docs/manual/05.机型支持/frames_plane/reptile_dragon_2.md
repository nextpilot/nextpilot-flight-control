...
::: info
[DSHOT ESC](../21.周边设备/dshot.md#wiring-connections) were used (not PWM as for the servos).
To make efficient use of the [DSHOT output port restrictions](../21.周边设备/dshot.md#wiring-connections), the two ESCs must be wired to FMU output channels 3 and 4.
:::
...
- [MSP_OSD_CONFIG](../19.高级调参/parameter_reference.md#MSP_OSD_CONFIG) param must match serial port which is connected to the Caddx Vista (in this build, `/dev/ttyS7`).
- [RC_CRSF_PRT_CFG](../19.高级调参/parameter_reference.md#RC_CRSF_PRT_CFG) param must match the serial port which is connected to the ELRS RX (in this build, `Telem 1`).

### Radio Setup

You should enable Manual, Acro, and Position modes on your controller (at least for the first flight).
For instructions see [Flight mode Configuration](../07.标准配置/flight_mode.md)

We also recommend configuring an [autotuning switch](../07.标准配置/98.autotune.md#enable-disable-autotune-switch-fixed-wing) for the first flight, as this makes it easier to enable/disable autotuning while flying.
...
Motors and flight control surface setup done in the [Actuator](../07.标准配置/actuators.md) section.
...
With the propellers removed, power the airplane up and use the [Actuator](../07.标准配置/actuators.md) test in QGC to spin up the motors.
...
Once the airplane is successfully airborne, cruise up to an altitude of a few hundred feet and switch to [Acro mode](../05.基本概念/01.飞行模式/acro.md).
This is a good time to use [Autotuning](../07.标准配置/98.autotune.md) to tune the airframe.

If the airplane is well behaved in _Acro mode_, switch to [Position mode](../05.基本概念/01.飞行模式/position.md).
