...
1. Select a helicopter [Airframe](../07.标准配置/02.select-airframe.md) in QGroundControl.
   At time of writing there is only _Generic Helicopter (Tail ESC)_ in the Helicopter group.
   This will configure the helicopter frame with a mechanically uncoupled tail ([CA_AIRFRAME](../19.高级调参/parameter_reference.md#CA_AIRFRAME): `10: Helicopter (tail ESC)`).
   ...
   To select this configuration, set the parameter [CA_AIRFRAME](../19.高级调参/parameter_reference.md#CA_AIRFRAME) to _Helicopter (tail Servo)_.
...
1. Configure helicopter actuator geometry in **Vehicle Setup > Actuators**.
   ...
   ::: info
   Actuator setup and testing is covered for most frames in [Actuators](../07.标准配置/actuators.md).
   While that is referenced below, this is the main topic for helicopter setup information.
   :::

   The geometry for a [Generic Helicopter - with Tail ESC](../09.机型调参/01.airframe_reference.md#copter_helicopter_generic_helicopter_%28tail_esc%29) is shown below.
...
1. Assign motors and servos to outputs and test (also in [Actuator configuration](../07.标准配置/actuators.md)):

   1. Assign the [motors and servos to the outputs](../07.标准配置/actuators.md#actuator-outputs).
   1. Power the vehicle with a battery and use the [actuator testing sliders](../07.标准配置/actuators.md#actuator-testing) to validate correct servo and motor assignment and direction.

1. Using an RC in [Acro mode](../05.基本概念/01.飞行模式/acro.md), verify the correct movement of the swash-plate. With most airframes you need to see the following:
...
1. Arm the vehicle and check the main rotor spins up slowly.
   Adjust the throttle spoolup time as needed using the parameter [COM_SPOOLUP_TIME](../19.高级调参/parameter_reference.md#COM_SPOOLUP_TIME).
   You can also adjust the throttle curve with the parameters [CA_HELI_THR_Cx](../19.高级调参/parameter_reference.md#CA_HELI_THR_C0).
...
1. Configure the collective pitch curve using the parameters [CA_HELI_PITCH_Cx](../19.高级调参/parameter_reference.md#CA_HELI_PITCH_C0).
...
Attitude, velocity, and position controller tuning is then performed in the [same as for multicopters](../09.机型调参/11.多旋翼/index.md).
...
Most importantly set the rotation direction of your main rotor which is by default clockwise when seen from above the airframe. In case yours turns counter-clockwise set [CA_HELI_YAW_CCW](../19.高级调参/parameter_reference.md#CA_HELI_YAW_CCW) to 1.

There are two parameters to compensate yaw for the main rotor's collective and throttle:
[CA_HELI_YAW_CP_S](../19.高级调参/parameter_reference.md#CA_HELI_YAW_CP_S)
[CA_HELI_YAW_TH_S](../19.高级调参/parameter_reference.md#CA_HELI_YAW_TH_S)
...
The rate controller should be tuned in [Acro mode](../05.基本概念/01.飞行模式/acro.md), but can also be done in [Stabilized mode](../05.基本概念/01.飞行模式/manual_stabilized.md) if you cannot fly Acro mode.
...
2. Take off slowly and provide some roll and stick movements.
   ...
   Increase the roll and pitch feed forward gains [MC_ROLLRATE_FF](../19.高级调参/parameter_reference.md#MC_ROLLRATE_FF), [MC_PITCHRATE_FF](../19.高级调参/parameter_reference.md#MC_PITCHRATE_FF) until the response reaches the setpoint when giving a step input.

3. Then enable the PID gains.
   Start off with following values:

   - [MC_ROLLRATE_P](../19.高级调参/parameter_reference.md#MC_ROLLRATE_P), [MC_PITCHRATE_P](../19.高级调参/parameter_reference.md#MC_PITCHRATE_P) a quarter of the value you found to work well as the corresponding feed forward value in the previous step. `P = FF / 4`
