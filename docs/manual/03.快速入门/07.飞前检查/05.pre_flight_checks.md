...
PX4 performs a number of preflight sensor quality and estimator checks to determine if there is a good enough position estimate to arm and fly the vehicle (these checks are controlled by the [COM\_ARM\_](../19.高级调参/parameter_reference.md#commander) parameters).

:::tip
Any preflight errors are reported in *QGroundControl* as `PREFLIGHT FAIL` messages.
The `estimator_status.gps_check_fail_flags` message [in the logs](../15.数据分析/flight_reporting.md) also shows which GPS quality checks are failing.
:::
...
The following errors (with associated checks and parameters) are reported by the [EKF](../19.高级调参/tuning_the_ecl_ekf.md) (and propagate to *QGroundControl*):
...
* The check is controlled by the [COM_ARM_EKF_HGT](../19.高级调参/parameter_reference.md#COM_ARM_EKF_HGT) parameter.
...
* The check is controlled by the [COM_ARM_EKF_VEL](../19.高级调参/parameter_reference.md#COM_ARM_EKF_VEL) parameter.
...
* The check is controlled by the [COM_ARM_EKF_POS](../19.高级调参/parameter_reference.md#COM_ARM_EKF_POS) parameter.
...
* The check is controlled by the [COM_ARM_EKF_YAW](../19.高级调参/parameter_reference.md#COM_ARM_EKF_YAW) parameter
...
* If you *sometimes* get the warning: [re-calibrate the accelerometer](../07.标准配置/07.accelerometer.md).
* If you get *regularly* get the warning: Perform a [thermal calibration](../19.高级调参/sensor_thermal_calibration.md).
* If you still get the warning after thermal calibration (or you can't perform thermal calibration):
  * Verify that the issues do not come from the sensor or autopilot hardware:
    ...
    * Alternatively, [log and compare](../15.数据分析/logging.md#configuration) all accelerometers across a number of bench test runs with `6: Sensor comparison` enabled in [SDLOG_PROFILE](../19.高级调参/parameter_reference.md#SDLOG_PROFILE).
...
* The check is controlled by the [COM_ARM_IMU_ACC](../19.高级调参/parameter_reference.md#COM_ARM_IMU_ACC) parameter.

#### PREFLIGHT FAIL: GYRO SENSORS INCONSISTENT - CHECK CALIBRATION

* This error message is produced when the angular rate measurements from different IMU units are inconsistent.
* This check only applies to boards with more than one IMU.
* The check is controlled by the [COM_ARM_IMU_GYR](../19.高级调参/parameter_reference.md#COM_ARM_IMU_GYR) parameter.
...
* The check is controlled by the [COM_ARM_MAG_ANG](../19.高级调参/parameter_reference.md#COM_ARM_MAG_ANG) parameter.
...
The [COM_ARM_WO_GPS](../19.高级调参/parameter_reference.md#COM_ARM_WO_GPS) parameter controls whether or not arming is allowed without a global position estimate.
