...
The [Geofence Failsafe](../07.标准配置/99.safety.md#geofence-failsafe) defines a cylinder centered on the home position, with a specified maximum radius and altitude.

The settings also include a "failsafe action" in the event that the fence is breached.
This may simply be a warning notification, but more commonly a vehicle will immediately [Return](../05.基本概念/01.飞行模式/return.md) to a safe location.

For more information see: [Safety > Geofence Failsafe](../07.标准配置/99.safety.md#geofence-failsafe).
...
1. Set the breach action in the [Geofence Failsafe](../07.标准配置/99.safety.md#geofence-failsafe).
