...
The compass should ideally be oriented so that it is upright and the direction marker is pointing towards the front of the vehicle (the default orientation), but if needed can be oriented at multiples of 45° from this attitude (in any axis) as defined in the [standard MAVLink orientations](https://mavlink.io/en/messages/common.html#MAV_SENSOR_ORIENTATION) (these follow the same frame convention as when [orienting the flight controller](../../07.标准配置/05.flight_controller_orientation.md#calculating-orientation)).

The diagram below shows the heading marker on the Pixhawk 4 flight controller and compass.

![Connect compass/GPS to Pixhawk 4](/assets/flight_controller/pixhawk4/pixhawk4_compass_gps.jpg)

PX4 will automatically detect the orientation for any of these standard orientations during [compass calibration](../../07.标准配置/06.compass.md) ([by default](../../19.高级调参/parameter_reference.md#SENS_MAG_AUTOROT)).
...
For more information see [Setting the Compass Orientation](../../07.标准配置/05.flight_controller_orientation.md#setting-the-compass-orientation) in _Flight Controller/Sensor Orientation_.

## Position

In order to compensate for the relative motion between the receiver and the CoG, you should [configure](../../19.高级调参/parameters.md) the following parameters to set the offsets: [EKF2_GPS_POS_X](../../19.高级调参/parameter_reference.md#EKF2_GPS_POS_X), [EKF2_GPS_POS_Y](../../19.高级调参/parameter_reference.md#EKF2_GPS_POS_Y) and [EKF2_GPS_POS_Z](../../19.高级调参/parameter_reference.md#EKF2_GPS_POS_Z).
...
It is particularly important if using [RTK GNSS](../../19.高级调参/) which has centimeter-level accuracy, because if the offsets are not set then GNSS measurements will often be rejected as inconsistent with the current EFK estimate.
