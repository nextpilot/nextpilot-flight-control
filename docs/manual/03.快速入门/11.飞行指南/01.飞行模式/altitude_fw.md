...
* Centered Roll/Pitch/Yaw inputs (inside deadband):
  ...
  * Throttle stick controls the airspeed of the aircraft if an airspeed sensor is connected. Without an airspeed sensor the user cannot control throttle (in which case the vehicle will fly level at trim throttle ([FW_THR_TRIM](../19.高级调参/parameter_reference.md#FW_THR_TRIM)), increasing or decreasing throttle as needed to climb or descend).
* Outside center:
  ...
  * Roll stick controls roll angle. Autopilot will maintain [coordinated flight](https://en.wikipedia.org/wiki/Coordinated_flight).
    This is same as in [Stabilized mode](../05.基本概念/01.飞行模式/stabilized_fw.md).
...
--- | ---
<a id="FW_AIRSPD_MIN"></a>[FW_AIRSPD_MIN](../19.高级调参/parameter_reference.md#FW_AIRSPD_MIN) | Min airspeed/throttle. Default: 10 m/s.
<a id="FW_AIRSPD_MAX"></a>[FW_AIRSPD_MAX](../19.高级调参/parameter_reference.md#FW_AIRSPD_MAX) | Max airspeed/throttle. Default: 20 m/s.
<a id="FW_AIRSPD_TRIM"></a>[FW_AIRSPD_TRIM](../19.高级调参/parameter_reference.md#FW_AIRSPD_TRIM) | Cruise speed. Default: 15 m/s.
<a id="FW_MAN_P_MAX"></a>[FW_MAN_P_MAX](../19.高级调参/parameter_reference.md#FW_MAN_P_MAX) | Max pitch for manual control in attitude stabilized mode. Default: 45 degrees.
<a id="FW_MAN_R_MAX"></a>[FW_MAN_R_MAX](../19.高级调参/parameter_reference.md#FW_MAN_R_MAX) | Max roll for manual control in attitude stabilized mode. Default: 45 degrees.
<a id="FW_NPFG_CONTROL"></a>[FW NPFG Control](../19.高级调参/parameter_reference.md#fw-npfg-control) | The roll/yaw needed to maintain the commanded altitude and airspeed are also affected by the FW NPFG Control parameters.
