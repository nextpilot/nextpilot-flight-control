...
The setpoints may be provided using MAVLink (or a MAVLink API such as [MAVSDK](https://mavsdk.mavlink.io/)) or by ROS 2.
...
- RC control is disabled except to change modes (you can also fly without any manual controller at all by setting the parameter [COM_RC_IN_MODE](../19.高级调参/parameter_reference.md#COM_RC_IN_MODE) to 4: Stick input disabled).
...
:::note
Before using offboard mode with ROS 2, please spend a few minutes understanding the different [frame conventions]#ros-2-px4-frame-conventions) that PX4 and ROS 2 use.
:::
...
--- | ---
<a id="COM_OF_LOSS_T"></a>[COM_OF_LOSS_T](../19.高级调参/parameter_reference.md#COM_OF_LOSS_T) | Time-out (in seconds) to wait when offboard connection is lost before triggering offboard lost failsafe (`COM_OBL_RC_ACT`)
<a id="COM_OBL_RC_ACT"></a>[COM_OBL_RC_ACT](../19.高级调参/parameter_reference.md#COM_OBL_RC_ACT) | Mode to switch to if offboard control is lost (Values are - 0: *Position*, 1: [Altitude](../05.基本概念/01.飞行模式/altitude_mc.md), 2: *Manual*, 3: [Return](../05.基本概念/01.飞行模式/return.md), 4: [Land](../05.基本概念/01.飞行模式/land.md)).
<a id="COM_RC_OVERRIDE"></a>[COM_RC_OVERRIDE](../19.高级调参/parameter_reference.md#COM_RC_OVERRIDE) | Controls whether stick movement on a multicopter (or VTOL in MC mode) causes a mode change to [Position mode](../05.基本概念/01.飞行模式/position_mc.md). This is not enabled for offboard mode by default.
...
<a id="COM_RCL_EXCEPT"></a>[COM_RCL_EXCEPT](../19.高级调参/parameter_reference.md#COM_RCL_EXCEPT) | Specify modes in which RC loss is ignored and the failsafe action not triggered. Set bit `2` to ignore RC loss in Offboard mode.
