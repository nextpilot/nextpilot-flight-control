...
[<img src="/assets/site/difficulty_easy.png" title="Easy to fly" width="30px" />](../README.md#key_difficulty)&nbsp;[<img src="/assets/site/remote_control.svg" title="Manual/Remote control required" width="30px" />](../README.md#key_manual)&nbsp;[<img src="/assets/site/position_fixed.svg" title="Position fix required (e.g. GPS)" width="30px" />](../README.md#key_position_fixed)
...
Unlike [Altitude](../05.基本概念/01.飞行模式/altitude_mc.md) and [Manual/Stabilized](../05.基本概念/01.飞行模式/manual_stabilized_mc.md) modes the vehicle will stop when the sticks are centered rather than continuing until slowed by wind resistance.
:::
...
1. The vehicle will lower propeller thrust, detect the ground and [automatically disarm](../19.高级调参/prearm_arm_disarm.md#auto-disarming) (by default).
...
- If the vehicle does not stop moving horizontally:
  - You can still land under control in [Altitude mode](../05.基本概念/01.飞行模式/altitude_mc.md).
...
- If the vehicle does not detect the ground/landing and disarm:
  - After the vehicle is on the ground switch to [Manual/Stabilized mode](../05.基本概念/01.飞行模式/manual_stabilized_mc.md) keeping the throttle stick low, and manually disarm using a gesture or other command.
...
- Centered roll, pitch, throttle sticks (within RC deadzone [MPC_HOLD_DZ](../19.高级调参/parameter_reference.md#MPC_HOLD_DZ)) hold x, y, z position steady against any disturbance like wind.
- Outside center:
...
All the parameters in the [Multicopter Position Control](../19.高级调参/parameter_reference.md#multicopter-position-control) group are relevant. A few parameters of particular note are listed below.

Parameter | Description
--- | ---
<a id="MPC_HOLD_DZ"></a>[MPC_HOLD_DZ](../19.高级调参/parameter_reference.md#MPC_HOLD_DZ) | Deadzone of sticks where position hold is enabled. Default: 0.1 (10% of full stick range).
...
<a id="MPC_LAND_ALT1"></a>[MPC_LAND_ALT1](../19.高级调参/parameter_reference.md#MPC_LAND_ALT1) | Altitude for triggering first phase of slow landing. Below this altitude descending velocity gets limited to a value between [MPC_Z_VEL_MAX_DN](#MPC_Z_VEL_MAX_DN) (or `MPC_Z_V_AUTO_DN`) and [MPC_LAND_SPEED](#MPC_LAND_SPEED). Value needs to be higher than [MPC_LAND_ALT2](#MPC_LAND_ALT2). Default 10m.
<a id="MPC_LAND_ALT2"></a>[MPC_LAND_ALT2](../19.高级调参/parameter_reference.md#MPC_LAND_ALT2) | Altitude for second phase of slow landing. Below this altitude descending velocity gets limited to [`MPC_LAND_SPEED`](#MPC_LAND_SPEED). Value needs to be lower than "MPC_LAND_ALT1". Default 5m.
...
<a id="MPC_POS_MODE"></a>[MPC_POS_MODE](../19.高级调参/parameter_reference.md#MPC_POS_MODE) | Stick input to movement translation strategy. From PX4 v1.12 the default (4) is that stick position controls acceleration (in a similar way to a car accelerator pedal). Other options allow stick deflection to directly control speed over ground, with and without smoothing and acceleration limits.
<a id="MPC_ACC_HOR_MAX"></a>[MPC_ACC_HOR_MAX](../19.高级调参/parameter_reference.md#MPC_ACC_HOR_MAX) | Maximum horizontal acceleration.
<a id="MPC_VEL_MANUAL"></a>[MPC_VEL_MANUAL](../19.高级调参/parameter_reference.md#MPC_VEL_MANUAL) | Maximum horizontal velocity.
<a id="MPC_LAND_SPEED"></a>[MPC_LAND_SPEED](../19.高级调参/parameter_reference.md#MPC_LAND_SPEED) | Landing descend rate. Default 0.7 m/s.
...
If the estimate falls below acceptable levels, for example due to GPS loss, this may trigger a [Position (GPS) Loss Failsafe](../07.标准配置/99.safety.md#position-gps-loss-failsafe).
