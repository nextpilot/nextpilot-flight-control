...
[<img src="/assets/site/position_fixed.svg" title="Global position fix required (e.g. GPS)" width="30px" />](../README.md#key_position_fixed)

*Mission mode* causes the vehicle to execute a predefined autonomous [mission](../03.飞行指南/missions.md) (flight plan) that has been uploaded to the flight controller.
...
- RC stick movement in a multicopter (or VTOL in multicopter mode) will [by default](#COM_RC_OVERRIDE) change the vehicle to [Position mode](../05.基本概念/01.飞行模式/position_mc.md) unless handling a critical battery failsafe.
...
1. If a mission is stored and PX4 is flying it will execute the [mission/flight plan](../03.飞行指南/missions.md) from the current step.
...
1. If a mission is stored and PX4 is landed:
   - On copters PX4 will execute the [mission/flight plan](../03.飞行指南/missions.md).
...
Missions can be paused by switching out of mission mode to any other mode (such as [Hold mode](../05.基本概念/01.飞行模式/hold.md) or [Position mode](../05.基本概念/01.飞行模式/position_mc.md).
When you switch back to mission mode the vehicle will continue the mission, heading from the *current vehicle position* to the current active mission item (the same waypoint it as heading towards originally).
...
- [Mission Planning](../03.飞行指南/missions.md)
...
Mission behaviour is affected by a number of parameters, most of which are documented in [Parameter Reference > Mission](../19.高级调参/parameter_reference.md#mission).
...
--- | ---
<a id="NAV_RCL_ACT"></a>[NAV_RCL_ACT](../19.高级调参/parameter_reference.md#NAV_RCL_ACT) | RC loss failsafe mode (what the vehicle will do if it looses RC connection) - e.g. enter hold mode, return mode, terminate etc.
<a id="NAV_LOITER_RAD"></a>[NAV_LOITER_RAD](../19.高级调参/parameter_reference.md#NAV_RCL_ACT) | Fixed-wing loiter radius.
<a id="COM_RC_OVERRIDE"></a>[COM_RC_OVERRIDE](../19.高级调参/parameter_reference.md#COM_RC_OVERRIDE) | Controls whether stick movement on a multicopter (or VTOL in MC mode) gives control back to the pilot in [Position mode](../05.基本概念/01.飞行模式/position_mc.md). This can be separately enabled for auto modes and for offboard mode, and is enabled in auto modes by default.
<a id="COM_RC_STICK_OV"></a>[COM_RC_STICK_OV](../19.高级调参/parameter_reference.md#COM_RC_STICK_OV) | The amount of stick movement that causes a transition to [Position mode](../05.基本概念/01.飞行模式/position_mc.md) (if [COM_RC_OVERRIDE](#COM_RC_OVERRIDE) is enabled).

Parameters related to [mission feasibility checks](#mission-feasibility-checks):

Parameter | Description
--- | ---
<a id="MIS_DIST_1WP"></a>[MIS_DIST_1WP](../19.高级调参/parameter_reference.md#MIS_DIST_1WP) | The mission will not be started if the current waypoint is more distant than this value from the home position. Disabled if value is 0 or less.
<a id="FW_LND_ANG"></a>[FW_LND_ANG](../19.高级调参/parameter_reference.md#FW_LND_ANG) | Maximum landing slope angle.
<a id="MIS_TKO_LAND_REQ"></a>[MIS_TKO_LAND_REQ](../19.高级调参/parameter_reference.md#MIS_TKO_LAND_REQ) | Mission takeoff/landing requirement configuration. FW and VTOL both have it set to 2 by default, which means that the mission has to contain a landing.

<a id="mission_commands"></a>

## Supported Mission Commands

PX4 "accepts" the following MAVLink mission commands in Mission mode (with some *caveats*, given after the list).
...
MC vehicles will change the *speed* when approaching or leaving a waypoint based on the [jerk-limited](../09.机型调参/11.多旋翼/mc_jerk_limited_type_trajectory.md#auto-mode) tuning.
The vehicle will follow a smooth rounded curve towards the next waypoint (if one is defined) defined by the acceptance radius ([NAV_ACC_RAD](../19.高级调参/parameter_reference.md#NAV_ACC_RAD)).
...
- For MC this radius is defined by [NAV_ACC_RAD](../19.高级调参/parameter_reference.md#NAV_ACC_RAD).
- For FW the acceptance radius is defined by the "L1 distance".
  - The L1 distance is computed from two parameters: [NPFG_DAMPING](../19.高级调参/parameter_reference.md#NPFG_DAMPING) and [NPFG_PERIOD](../19.高级调参/parameter_reference.md#NPFG_PERIOD), and the current ground speed.
...
During mission execution this will cause the vehicle to ascend vertically to the minimum takeoff altitude defined in the [MIS_TAKEOFF_ALT](../19.高级调参/parameter_reference.md#MIS_TAKEOFF_ALT) parameter, then head towards the 3D position defined in the mission item.
...
:::note
A more detailed description of mission mode fixed-wing takeoff can be found in [Takeoff mode > Fixed-wing](../05.基本概念/01.飞行模式/takeoff.md#fixed-wing-fw) (covering fixed wing takeoff in both mission mode and takeoff mode).
:::
...
Both runway and hand-launched takeoff are supported — for configuration information see [Takeoff mode > Fixed-wing](../05.基本概念/01.飞行模式/takeoff.md#fixed-wing-fw).
...
:::note
Note fixed wing landing using [Land mode](../05.基本概念/01.飞行模式/land.md#fixed-wing-fw) is currently broken, so mission landing is the *only* way to autonomously land a plane.
:::
...
Parameter | Description
--- | ---
<a id="FW_LND_ANG"></a>[FW_LND_ANG](../19.高级调参/parameter_reference.md#FW_LND_ANG) | The maximum achievable landing approach slope angle. Note that smaller angles may still be commanded via the landing pattern mission item.
[FW_LND_EARLYCFG](../19.高级调参/parameter_reference.md#FW_LND_EARLYCFG) | Optionally deploy landing configuration during the landing descent orbit (e.g. flaps, spoilers, landing airspeed).
[FW_LND_AIRSPD](../19.高级调参/parameter_reference.md#FW_LND_AIRSPD) | Calibrated airspeed setpoint during landing.
[FW_FLAPS_LND_SCL](../19.高级调参/parameter_reference.md#FW_FLAPS_LND_SCL) | Flaps setting during landing.
[FW_LND_THRTC_SC](../19.高级调参/parameter_reference.md#FW_LND_THRTC_SC) | Altitude time constant factor for landing (overrides default [TECS tuning](../09.机型调参/12.固定翼/advanced_tuning_guide_fixedwing.md)).
...
Parameter | Description
--- | ---
<a id="FW_LND_FL_TIME"></a>[FW_LND_FL_TIME](../19.高级调参/parameter_reference.md#FW_LND_FL_TIME) | Time before impact (at current descent rate) at which the vehicle should flare.
<a id="FW_LND_FL_SINK"></a>[FW_LND_FL_SINK](../19.高级调参/parameter_reference.md#FW_LND_FL_SINK) | A shallow sink rate the aircraft will track during the flare.
<a id="FW_LND_FLALT"></a>[FW_LND_FLALT](../19.高级调参/parameter_reference.md#FW_LND_FLALT) | Minimum altitude above ground the aircraft must flare. This is only used when the time-based flare altitude is too low.
<a id="FW_LND_FL_PMAX"></a>[FW_LND_FL_PMAX](../19.高级调参/parameter_reference.md#FW_LND_FL_PMAX) | Maximum allowed pitch during the flare.
<a id="FW_LND_FL_PMIN"></a>[FW_LND_FL_PMIN](../19.高级调参/parameter_reference.md#FW_LND_FL_PMIN) | Minimum allowed pitch during the flare (often necessary to avoid negative pitch angles commanded to increase airspeed, as the throttle is reduced to idle setting.)
<a id="FW_LND_TD_TIME"></a>[FW_LND_TD_TIME](../19.高级调参/parameter_reference.md#FW_LND_TD_TIME) | The time after flare start when the vehicle should pitch the nose down.
<a id="RWTO_PSP"></a>[RWTO_PSP](../19.高级调参/parameter_reference.md#RWTO_PSP) | Pitch setpoint while on the runway. For tricycle gear, typically near zero. For tail draggers, positive.
<a id="FW_THR_IDLE"></a>[FW_THR_IDLE](../19.高级调参/parameter_reference.md#FW_THR_IDLE) | Idle throttle setting. The vehicle will retain this setting throughout the flare and roll out.
...
On *QGroundControl* a popup button appears during landing to enable this.
...
Operators may still manually abort the landing by switching to any manual mode, such as [Stabilized mode](../05.基本概念/01.飞行模式/stabilized_fw.md)), though it should be noted that this is risky!
...
Parameter | Description
--- | ---
<a id="MIS_LND_ABRT_ALT"></a>[MIS_LND_ABRT_ALT](../19.高级调参/parameter_reference.md#MIS_LND_ABRT_ALT) | The minimum altitude above the land point an abort orbit can be commanded.
<a id="FW_LND_ABORT"></a>[FW_LND_ABORT](../19.高级调参/parameter_reference.md#FW_LND_ABORT) | Determines which automatic abort criteria are enabled.
<a id="FW_LND_USETER"></a>[FW_LND_USETER](../19.高级调参/parameter_reference.md#FW_LND_USETER) | Enables use of the distance sensor during the final approach.

### Nudging

In the case of minor GNSS or map discrepancies causing an offset approach, small manual adjustments to the landing approach and roll out can be made by the operator (via yaw stick) when [FW_LND_NUDGE](../19.高级调参/parameter_reference.md#FW_LND_NUDGE) is enabled.
...
[FW_LND_TD_OFF](../19.高级调参/parameter_reference.md#FW_LND_TD_OFF) allows determination of how far to the left or right of the landing waypoint the projected touchdown point may be nudged.
...
If the vehicle is regularly showing poor tracking peformance on a defined path, please refer to the [fixed-wing control tuning guide](../05.基本概念/01.飞行模式/position_fw.md) for instruction.
:::

Parameter | Description
--- | ---
<a id="FW_LND_NUDGE"></a>[FW_LND_NUDGE](../19.高级调参/parameter_reference.md#FW_LND_NUDGE) | Enable nudging behavior for fixed-wing landing.
<a id="FW_LND_TD_OFF"></a>[FW_LND_TD_OFF](../19.高级调参/parameter_reference.md#FW_LND_TD_OFF) | Configure the allowable touchdown lateral offset from the commanded landing point.
<a id="FW_W_EN"></a>[FW_W_EN](../19.高级调参/parameter_reference.md#FW_W_EN) | Enable the nose wheel steering controller.
...
Parameter | Description
--- | ---
[FW_WING_SPAN](../19.高级调参/parameter_reference.md#FW_WING_SPAN) | Wing span of the airframe.
[FW_WING_HEIGHT](../19.高级调参/parameter_reference.md#FW_WING_HEIGHT) | Height of wing from bottom of gear (or belly if no gear).

## VTOL Mission Takeoff

Plan a VTOL mission takeoff by adding a `VTOL Takeoff` mission item to the map.

During mission execution the vehicle will ascend vertically to the minimum takeoff altitude defined in the [MIS_TAKEOFF_ALT](../19.高级调参/parameter_reference.md#MIS_TAKEOFF_ALT) parameter, then transition to fixed-wing mode with the heading defined in the mission item.
