...
[Pixhawk-series flight controllers](../01.基本概念/pixhawk_series.md) use audible tones/tunes and [LEDs](led_meanings.md) to indicate vehicle state and events (e.g. arming success and failure, low battery warnings).

The set of standard sounds are listed below.

> **Developers:** Tunes are defined in [/lib/tunes/tune_definition.desc](https://github.com/PX4/PX4-Autopilot/blob/main/src/lib/tunes/tune_definition.desc) and can be tested using the tune-control module. You can search for tune use using the string `TUNE_ID_name`(e.g. `TUNE_ID_PARACHUTE_RELEASE`)
...
- Low battery warning ([failsafe](../07.标准配置/99.safety.md#low-battery-failsafe)).
...
- Critical low battery warning ([failsafe](../07.标准配置/99.safety.md#low-battery-failsafe)).
