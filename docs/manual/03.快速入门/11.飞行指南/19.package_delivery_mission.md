...
A package delivery mission allows users to plan and execute cargo delivery using a gripper.
...
- [Gripper > Package Delivery Configuration]#package-delivery-configuration)

## Mission Planning

A package delivery mission is planned in much the same as any other [waypoint mission](../03.飞行指南/missions.md), with mission start, takeoff waypoint, various path waypoints, and possibly a return waypoint.
...
A gripper can be [manually controlled using a joystick button]#qgc-joystick-configuration) (if configured) in any mode, including during a mission.
...
The mission will resume after the payload delivery mission item timeout ([MIS_PD_TO](../19.高级调参/parameter_reference.md#MIS_PD_TO) expires, even if it has not released the package.
