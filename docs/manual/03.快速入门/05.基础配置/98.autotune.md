...
!!! info
    During [Airframe Setup](./02.select-airframe.md) you should have selected the frame that most closely matches your vehicle.
...
2. Takeoff and prepare for the test
   - **Multicopters:** Take off and hover at 1m above ground in [Altitude mode](../05.基本概念/01.飞行模式/altitude.md) or Stabilized mode.
   - **Fixed-wing:** Take off and fly at cruise speed in [Position mode](../05.基本概念/01.飞行模式/position.md) or [Altitude mode](../05.基本概念/01.飞行模式/altitude.md).
...
The recommended modes for autotuning are [Hold mode](../05.基本概念/01.飞行模式/hold.md) (FW) and [Altitude mode](../05.基本概念/01.飞行模式/altitude.md) (MC), but any other flight mode can be used.
...
2. Takeoff using RC control and prepare for test:
   - **Multicopters:** Takeoff using the remote controller in [Altitude mode](../05.基本概念/01.飞行模式/altitude.md).
     Hover the vehicle at a safe distance and at a few meters above ground (between 4 and 20m).
   - **Fixed-wing:** Once flying at cruise speed, activate [Hold mode](../05.基本概念/01.飞行模式/hold.md).
...
- **Multicopter:** The instructions above tune the vehicle in [Altitude mode](../05.基本概念/01.飞行模式/altitude.md).
  You can instead takeoff in [Takeoff mode](../05.基本概念/01.飞行模式/takeoff.md) and tune in [Position mode](../05.基本概念/01.飞行模式/position.md) if the vehicle is is _known_ to be stable in these modes.
- **Fixed-wing:** Autotuning can also be run in [Altitude mode](../05.基本概念/01.飞行模式/altitude.md) or [Position mode](../05.基本概念/01.飞行模式/position.md).
...
- slow oscillations (1 oscillation per second or slower): this often occurs on large platforms and means that the attitude loop is too fast compared to the rate loop.
  - **Multicopter:** decrease [MC_ROLL_P](../19.高级调参/parameter_reference.md#MC_ROLL_P) and [MC_PITCH_P](../19.高级调参/parameter_reference.md#MC_PITCH_P) by steps of 1.0.
  - **Fixed-wing:** increase [FW_R_TC](../19.高级调参/parameter_reference.md#FW_R_TC) and [FW_P_TC](../19.高级调参/parameter_reference.md#FW_P_TC) by steps of 0.1.
- fast oscillations (more than 1 oscillation per second): this is because the gain of the rate loop is too high.
  ...
  - **Fixed-wing:** decrease [FW_RR_P](../19.高级调参/parameter_reference.md#FW_RR_P), [FW_PR_P](../19.高级调参/parameter_reference.md#FW_PR_P), [FW_YR_P](../19.高级调参/parameter_reference.md#FW_YR_P) by steps of 0.01.
...
Increase the [FW_AT_SYSID_AMP](../19.高级调参/parameter_reference.md#FW_AT_SYSID_AMP), [MC_AT_SYSID_AMP](../19.高级调参/parameter_reference.md#MC_AT_SYSID_AMP) by steps of 1 and trigger the auto-tune again.
...
- [Multicopter PID Tuning Guide](../09.机型调参/11.多旋翼/pid_tuning_guide_multicopter_basic.md) (Manual/Simple)
- [Multicopter PID Tuning Guide](../09.机型调参/11.多旋翼/pid_tuning_guide_multicopter.md) (Advanced/Detailed)
- [Fixed-Wing PID Tuning Guide](../09.机型调参/12.固定翼/pid_tuning_guide_fixedwing.md)
...
This behaviour can be configured using the [MC_AT_APPLY](../19.高级调参/parameter_reference.md#MC_AT_APPLY) and [FW_AT_APPLY](../19.高级调参/parameter_reference.md#FW_AT_APPLY) parameters respectively:
...
1. Set [RC_MAP_AUX1](../19.高级调参/parameter_reference.md#RC_MAP_AUX1) to match the RC channel for your switch (you can use any of `RC_MAP_AUX1` to `RC_MAP_AUX6`).
1. Set [FW_AT_MAN_AUX](../19.高级调参/parameter_reference.md#FW_AT_MAN_AUX) to the selected channel (i.e. `1: Aux 1` if you mapped `RC_MAP_AUX1`).
...
Fixed-wing vehicles (only) can select which axes are tuned using the [FW_AT_AXES](../19.高级调参/parameter_reference.md#FW_AT_AXES) bitmask parameter:
...
PX4 uses PID controllers (rate, attitude, velocity, and position) to calculate the outputs required to move a vehicle from its current estimated state to match a desired setpoint.
...
Generally if you use a [frame configuration](./02.select-airframe.md) that is similar to your vehicle then the vehicle will be able to fly.
...
The vehicle must be flying in an altitude-stabilized mode (such as [Altitude mode](../05.基本概念/01.飞行模式/altitude.md), [Hold mode](../05.基本概念/01.飞行模式/hold.md), or [Position mode](../05.基本概念/01.飞行模式/position.md)).
