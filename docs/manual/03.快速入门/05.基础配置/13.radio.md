...
::: info
A [Joystick](./joystick.md) can be used instead of RC for manual control.
The [COM_RC_IN_MODE](../19.高级调参/parameter_reference.md#COM_RC_IN_MODE) parameter [can be set](../19.高级调参/parameters.md) to define what kind of manual controller(s) are enabled.
:::
...
PX4 needs to be able to detect when the signal from the RC controller has been lost in order to be able to take [appropriate safety measures](./99.safety.md#manual-control-loss-failsafe).
...
If your RC receiver does not support outputting no signal on RC loss, you must configure it to set throttle low instead, and set the corresponding value in [RC_FAILS_THR](../19.高级调参/parameter_reference.md#RC_FAILS_THR).
...
1. Set the [transmitter mode](../21.周边设备/01.遥 控 器/index.md#transmitter_modes) radio button that matches your transmitter (this ensures that _QGroundControl_ displays the correct stick positions for you to follow during calibration).
...
:::tip
This feature is provided to enable manual in-flight tuning: [Multicopter PID Tuning Guide](../09.机型调参/11.多旋翼/pid_tuning_guide_multicopter.md), [Fixed-wing PID Tuning Guide](../09.机型调参/12.固定翼/pid_tuning_guide_fixedwing.md).
:::
...
- [RC System Selection](../21.周边设备/01.遥 控 器/index.md) - Choose a compatible RC system.
