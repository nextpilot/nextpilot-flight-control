...
The compass calibration process configures all connected internal and external [magnetometers](../21.周边设备/04.传 感 器/01.卫星导航/ub482.md).
...
::: info
Compass calibration also auto-detects the compass orientation ([by default](../19.高级调参/parameter_reference.md#SENS_MAG_AUTOROT)).
If you have [mounted the compass](../02.快速入门/03.设备安装/mount_gps_compass.md#compass-orientation) at a non-standard angle you will need to [manually set the compass orientation](./05.flight_controller_orientation.md#setting-the-compass-orientation) before calibrating.
:::
...
1. If using an external compass (or a combined GPS/compass module), make sure it is [mounted](../02.快速入门/03.设备安装/mount_gps_compass.md) as far as possible from other electronics in order to reduce magnetic interference, and in a _supported orientation_.
...
1. Click the **Compass** sensor button.
   ...
   ::: info
   You should already have set the [Autopilot Orientation](./05.flight_controller_orientation.md). If not, you can also set it here.
   :::
...
The process above will autodetect, [set default rotations](../19.高级调参/parameter_reference.md#SENS_MAG_AUTOROT), calibrate, and prioritise, all connected magnetometers.
...
These are prefixed with [CAL*MAGx*](../19.高级调参/parameter_reference.md#CAL_MAG0_ID) (where `x=0-3`).

To disable an internal compass:

- Use [CAL_MAGn_ROT](../19.高级调参/parameter_reference.md#CAL_MAG0_ROT) to determine which compasses are internal.
  A compass is internal if `CAL_MAGn_ROT==1`.
- Then use [CAL_MAGx_PRIO](../19.高级调参/parameter_reference.md#CAL_MAG0_PRIO) to disable the compass.
  This can also be used to change the relative priority of a compass.

## Debugging

Raw comparison data for magnetometers (in fact, for all sensors) can be logged by setting [SDLOG_MODE=1](../19.高级调参/parameter_reference.md#SDLOG_MODE) and [SDLOG_PROFILE=64](../19.高级调参/parameter_reference.md#SDLOG_PROFILE).
See [Logging](../15.数据分析/logging.md) for more information.

## Further Information

- [Peripherals > GPS & Compass](../21.周边设备/04.传 感 器/01.卫星导航/ub482.md)
- [Basic Assembly](../02.快速入门/03.设备安装/index.md) (setup guides for each flight controller)
- [Compass Power Compensation](../19.高级调参/compass_power_compensation.md) (Advanced Configuration)
