...
A [computer joystick](https://en.wikipedia.org/wiki/Joystick) or gamepad connected through _QGroundControl_ can be used to manually control the vehicle (_instead_ of using an [RC Transmitter](./radio.md)).
...
:::tip
[Radio Setup](./radio.md) is not required if using only a joystick (because a joystick is not an RC controller)!
:::
...
- Set the parameter [COM_RC_IN_MODE=1](../19.高级调参/parameter_reference.md#COM_RC_IN_MODE) - `Joystick`
