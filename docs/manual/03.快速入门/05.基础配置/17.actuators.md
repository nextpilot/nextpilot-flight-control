...
The displayed elements depend on the [selected frame](./02.select-airframe.md), with outputs mapped by default as shown in the [Airframe Reference](../09.机型调参/01.airframe_reference.md).

The view has three sections:

- [Geometry](#geometry): Configure the geometry for the [selected airframe](./02.select-airframe.md).
...
The geometry section is used to set any configurable geometry-related parameters for the selected [airframe](./02.select-airframe.md).
...
See the [Airframe Reference](../09.机型调参/01.airframe_reference.md) for an broad understanding of the motor positions for other frames.
...
The motor geometry for a [VTOL Quad Tailsitter](../09.机型调参/01.airframe_reference.md#vtol-tailsitter) is shown below (the approach for configuring other tailsitter VTOL vehicles will be similar).
...
The motor geometry for a [Generic Quadplane VTOL Tiltrotor](../09.机型调参/01.airframe_reference.md#vtol_vtol_tiltrotor_generic_quadplane_vtol_tiltrotor) is shown below (the approach for configuring other [VTOL tiltrotors](../09.机型调参/01.airframe_reference.md#vtol_vtol_tiltrotor_generic_quadplane_vtol_tiltrotor) will be similar).
...
The motor geometry for a [Generic Standard VTOL](../09.机型调参/01.airframe_reference.md#vtol_standard_vtol_generic_standard_vtol) is shown below (the approach for configuring other "Standard VTOL" will be similar).
...
Note that you will need to also ensure that the ESC associated with bidirectional motors is configured appropriately (e.g. 3D mode enabled for DShot ESCs, which can be achieved via [DShot commands]#commands)).
...
[VTOL tiltrotor vehicles](../09.机型调参/frames_vtol/tiltrotor.md) can tilt their motors to transition between hover and forward flight.
...
The example below shows the tilt servo setup for the [tiltrotor motor geometry shown above](./actuators.md#motor-geometry-vtol-tiltrotor).
...
The PWM AUX tab has CAP outputs that are generally used as the camera capture/trigger input.
...
- `Peripheral via Acutator Set 1` to `Peripheral via Acutator Set 6`: Generic Actuator Control with MAVLink.
...
- `RC Roll`: Output is passthrough roll from RC ([RC_MAP_ROLL](../19.高级调参/parameter_reference.md#RC_MAP_ROLL) maps an RC channel to this output).
  An RC channel is mapped to the output using .
- `RC Pitch`: Output is passthrough pitch from RC ([RC_MAP_PITCH](../19.高级调参/parameter_reference.md#RC_MAP_PITCH) maps an RC channel to this output).
- `RC Throttle`: Output is passthrough throttle from RC ([RC_MAP_THROTTLE](../19.高级调参/parameter_reference.md#RC_MAP_THROTTLE) maps an RC channel to this output).
- `RC Yaw`: Output is yaw from RC ([RC_MAP_YAW](../19.高级调参/parameter_reference.md#RC_MAP_YAW) maps an RC channel to this output).
- `RC Flaps`: Output is flaps from RC ([RC_MAP_FLAPS](../19.高级调参/parameter_reference.md#RC_MAP_FLAPS) maps an RC channel to this output).
- `RC AUXn` to `RC AUX1`: Outputs used for arbitrary payloads triggered by RC passthrough.
...
- `Camera_Trigger`: Output to trigger camera.
  Enabled when [`TRIG_MODE==0`](../19.高级调参/parameter_reference.md#TRIG_MODE).
  Configured via `TRIG_*` parameters.
- `Camera_Capture`: Input to get image capture notification.
  Enabled when [CAM_CAP_FBACK==0](../19.高级调参/parameter_reference.md#CAM_CAP_FBACK).
  Configured via `CAM_CAP_*` parameters.
- `PPS_Input`: Pulse-per-second input capture.
  Used for GPS synchronisation.
  Enabled when [`PPS_CAP_ENABLE==0`](../19.高级调参/parameter_reference.md#PPS_CAP_ENABLE)
...
1. Check which actuator moves on the vehicle.
   This should match the actuator positions for your geometry (the [airframe reference](../09.机型调参/01.airframe_reference.md) shows motor positions for a number of standard airframes).
...
::: info
If using PWM or OneShot ESCs, you should first perform [ESC Calibration](../19.高级调参/esc_calibration.md) (this topic also covers PWM specific motor configuration).
...
::: info
Another way to test without using the sliders would be to set the [`COM_PREARM_MODE`](../19.高级调参/parameter_reference.md#COM_PREARM_MODE) parameter to `Always`:
...
- The parameter [COM_MOT_TEST_EN](../19.高级调参/parameter_reference.md#COM_MOT_TEST_EN) can be used to completely disable actuator testing.
...
- If the ESCs are configured as DShot you can permanently reverse the direction via UI.
