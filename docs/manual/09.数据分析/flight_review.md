...
The [Flight Review](https://review.px4.io) plots for a flight can be used to analyze general vehicle condition.
...
The [Multicopter PID Tuning Guide](../09.机型调参/11.多旋翼/pid_tuning_guide_multicopter.md) contains example plots and information about analysing tracking performance.

:::tip
For the rate controller in particular, it is useful to enable the high-rate logging profile ([SDLOG_PROFILE](../19.高级调参/parameter_reference.md#SDLOG_PROFILE)) to get more details when zooming in.
:::
...
:::note
You need to enable the high-rate logging profile ([SDLOG_PROFILE](../19.高级调参/parameter_reference.md#SDLOG_PROFILE)) to see this plot.
:::
...
[QAV-R 5" Racer](../09.机型调参/frames_multicopter/qav_r_5_kiss_esc_racer.md) frame (excellent vibration).

![Low vibration QAV-R 5 Racer - FFT plot](/assets/flight_log_analysis/flight_review/vibrations_good_actuator_controls_fft.png)

:::note
The excellent vibration characteristics of the above frame mean that we can considerably increase the cutoff frequency of the [software filters](../09.机型调参/11.多旋翼/filter_tuning.md) (reducing control latency).
:::
...
- Set [IMU_GYRO_RATEMAX](../19.高级调参/parameter_reference.md#IMU_GYRO_RATEMAX) to 400.
...
- Use a good SD card, as the IMU data requires a high logging bandwidth (Flight Review will show dropouts if the logging rate gets too high).
  ...
  :::tip
  See [Logging > SD Cards](../15.数据分析/logging.md#sd-cards) for a comparison of popular SD card.
  :::

- Enable either the gyro or accel high-rate FIFO profile in [SDLOG_PROFILE](../19.高级调参/parameter_reference.md#SDLOG_PROFILE) and disable the rest of the entries.
...
Vibration Isolation explains some basic things you can check (and do) to reduce vibration levels.
...
(see [here](../15.数据分析/logging.md#sd-cards) for good SD cards):
...
These charts are all from the same flight of a [QAV-R 5" Racer](../09.机型调参/frames_multicopter/qav_r_5_kiss_esc_racer.md).
<!-- https://logs.px4.io/plot_app?log=cd88b091-ec89-457c-85f6-e63e4fa0f51d -->
