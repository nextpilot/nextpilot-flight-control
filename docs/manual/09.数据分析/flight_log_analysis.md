...
:::note
[Flight Reporting](../15.数据分析/flight_reporting.md) explains how to download a log and report/discuss issues about a flight with the development team.
:::
...
On autopilots based on the [STM32 series](https://www.st.com/en/microcontrollers/stm32-32-bit-arm-cortex-mcus.html), hard faults are logged to the SD card.
...
[Flight Review](https://review.px4.io) is the successor of _Log Muncher_.
It is used in combination with the new [ULog](../15.数据分析/ulog_file_format.md) logging format.
...
See [Log Analysis using Flight Review](../15.数据分析/flight_review.md) for an introduction.
...
See [Log Analysis using Plot Juggler](../15.数据分析/plotjuggler_log_analysis.md) for an introduction.
