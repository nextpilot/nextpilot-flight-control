...
PX4 behaviour can be configured/tuned using [parameters](../19.高级调参/parameter_reference.md) (e.g. [Multicopter PID gains](../09.机型调参/11.多旋翼/pid_tuning_guide_multicopter.md), calibration information, etc.).
...
::: info
Most of the more commonly used parameters are more conveniently set using the dedicated setup screens, as described in the [Standard Configuration](../07.标准配置/index.md) section.
...
Note that the groups at the bottom named _Component X_ are attached DroneCAN peripherals ("X" is the node id).
QGC can set the parameters of these peripherals if they are attached to the Flight Controller when QGC is started.
...
You can usually find out what parameters are conditional by searching the [full parameter reference](../19.高级调参/parameter_reference.md) and other documentation.
...
You can check whether a parameter _should_ be present by reviewing the [full parameter reference](../19.高级调参/parameter_reference.md) for the version you're targeting.
...
- Check if you can update your board to run FMUv3 firmware, which includes all modules: [Firmware > FMUv2 Bootloader Update](../07.标准配置/01.upload-firmware.md#bootloader)
...
For more information see: [Radio Setup > Param Tuning Channels](../07.标准配置/radio.md#param-tuning-channels).
