...
DShot and [CAN](../can/index.md) ESCs (DroneCAN/Cyphal) do not require this kind of calibration.
:::
...
This should be verified using [Actuator Testing](../07.标准配置/actuators.md#actuator-testing).
...
1. Map the ESCs you're calibrating as motors in the vehicle's [Actuator Configuration](../07.标准配置/actuators.md).
...
1. Go back to the [Actuator Configuration](../07.标准配置/actuators.md) section.
   ...
   ::: info
   The steps below are similar to those described in [Actuator Configuration > Motor Configuration](../07.标准配置/actuators.md#motor-configuration).
   :::
   ...
   - The minimum value for a motor (default: `1100us`) should make the motor spin slowly but reliably, and also spin up reliably after it was stopped.
     ...
     You can confirm that a motor spins at minimum (still without propellers) in [Actuator Testing](../07.标准配置/actuators.md#actuator-testing), by enabling the sliders, and then moving the test output slider for the motor to the first snap position from the bottom.
   ...
   - The maximum value for a motor (default: `1900us`) should be chosen such that increasing the value doesn't make the motor spin any faster.
     ...
     You can confirm that the motor spins quickly at the maximum setting in [Actuator Testing](../07.标准配置/actuators.md#actuator-testing), by moving the associated test output slider to the top position.
   ...
   - The disarmed value for a motor (default: `1000us`) should make the motor stop and stay stopped.
     ...
     You can confirm this in [Actuator Testing](../07.标准配置/actuators.md#actuator-testing) by moving the test output slider to the snap position at the bottom of the slider and observing that the motor does not spin.
