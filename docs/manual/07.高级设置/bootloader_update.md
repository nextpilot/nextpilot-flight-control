...
The _PX4 Bootloader_ is used to load firmware for Pixhawk boards (PX4FMU, PX4IO).
...
- On FMUv6X-RT you can [install bootloader/unbrick boards via USB](bootloader_update_v6xrt.md).
...
After the bootloader has updated you can [Load PX4 Firmware](../07.标准配置/01.upload-firmware.md) using _QGroundControl_.
...
You can then initiate bootloader update on next restart by setting the parameter: [SYS_BL_UPDATE](../19.高级调参/parameter_reference.md#SYS_BL_UPDATE).

This approach can only be used if [SYS_BL_UPDATE](../19.高级调参/parameter_reference.md#SYS_BL_UPDATE) is present in firmware.
...
1. [Update the Firmware](../07.标准配置/01.upload-firmware.md#custom) with an image containing the new/desired bootloader.
...
1. [Find and enable](../19.高级调参/parameters.md) the parameter [SYS_BL_UPDATE](../19.高级调参/parameter_reference.md#SYS_BL_UPDATE).
1. Reboot (disconnect/reconnect the board).
   The bootloader update will only take a few seconds.

Generally at this point you may then want to [update the firmware](../07.标准配置/01.upload-firmware.md) again using the correct/newly installed bootloader.
...
::: info
Early FMUv2 [Pixhawk-series]#fmu_versions) flight controllers had a hardware issue that restricted them to using 1MB of flash memory.
...
1. Insert an SD card (enables boot logging to debug any problems).
1. [Update the Firmware](../07.标准配置/01.upload-firmware.md) to PX4 _master_ version (when updating the firmware, check **Advanced settings** and then select **Developer Build (master)** from the dropdown list).
...
1. Reboot (disconnect/reconnect the board).
   The bootloader update will only take a few seconds.
1. Then [Update the Firmware](../07.标准配置/01.upload-firmware.md) again.
