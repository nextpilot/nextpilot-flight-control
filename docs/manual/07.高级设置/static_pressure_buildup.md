...
This might manifest as the vehicle losing altitude when it stops moving in [Altitude](../05.基本概念/01.飞行模式/altitude.md), [Position](../05.基本概念/01.飞行模式/position.md) or [Mission](../05.基本概念/01.飞行模式/mission.md) modes (when the vehicle stops moving the static pressure drops, the sensor reports a higher altitude, and the vehicle compensates by descending).
...
After modifying the hardware, you can then use the [EKF2_PCOEF\_\*](../19.高级调参/parameter_reference.md#EKF2_PCOEF_XN) parameters to tune for expected barometer variation based on relative air velocity.
For more information see [ECL/EKF Overview & Tuning > Correction for Static Pressure Position Error](../19.高级调参/tuning_the_ecl_ekf.md#correction-for-static-pressure-position-error).
