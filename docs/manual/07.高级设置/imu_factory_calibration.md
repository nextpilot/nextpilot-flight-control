...
This procedure will write the following parameters to `/fs/mtd_caldata`: [CAL_ACC\*](../19.高级调参/parameter_reference.md#CAL_ACC0_ID), [CAL_GYRO\*](../19.高级调参/parameter_reference.md#CAL_GYRO0_ID), [CAL_MAG\*](../19.高级调参/parameter_reference.md#CAL_MAG0_ID).
...
::: info
These values cannot be stored in the [frame configuration](../dev_airframes/adding_a_new_frame.md) because they vary from device to device (the frame configuration defines the set of parameters that are applicable across all vehicles of the same type, such as the enabled sensors, [autopilot rotation](../07.标准配置/05.flight_controller_orientation.md) and PID tuning).
:::

## Performing the Factory Calibration

1. Set the parameter [SYS_FAC_CAL_MODE](../19.高级调参/parameter_reference.md#SYS_FAC_CAL_MODE) to 1.
1. Perform all IMU calibrations: [accelerometer](../07.标准配置/07.accelerometer.md#performing-the-calibration), [gyroscope](../07.标准配置/09.gyroscope.md#performing-the-calibration) and [magnetometer](../07.标准配置/06.compass.md#performing-the-calibration).
...
::: info
If you only want to factory calibrate the accelerometer and the gyroscope you can set [SYS_FAC_CAL_MODE](../19.高级调参/parameter_reference.md#SYS_FAC_CAL_MODE) to 2, in which case the magnetometer is omitted.
:::
