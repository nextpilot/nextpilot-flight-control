...
The delay compensation for each sensor is controlled by the [EKF2\_\*\_DELAY](../19.高级调参/parameter_reference.md#ekf2) parameters.
...
The time constant for this filter is controlled by the [EKF2_TAU_VEL](../19.高级调参/parameter_reference.md#EKF2_TAU_VEL) and [EKF2_TAU_POS](../19.高级调参/parameter_reference.md#EKF2_TAU_POS) parameters.
...
- [EKF2_MULTI_IMU](../19.高级调参/parameter_reference.md#EKF2_MULTI_IMU) = 0
- [EKF2_MULTI_MAG](../19.高级调参/parameter_reference.md#EKF2_MULTI_MAG) = 0
- [SENS_IMU_MODE](../19.高级调参/parameter_reference.md#SENS_IMU_MODE) = 1
- [SENS_MAG_MODE](../19.高级调参/parameter_reference.md#SENS_MAG_MODE) = 1
...
The total number of EKF instances is the product of the number of IMU's and number of magnetometers selected by [EKF2_MULTI_IMU](../19.高级调参/parameter_reference.md#EKF2_MULTI_IMU) and [EKF2_MULTI_MAG](../19.高级调参/parameter_reference.md#EKF2_MULTI_MAG) and is given by the following formula:

> N_instances = MAX([EKF2_MULTI_IMU](../19.高级调参/parameter_reference.md#EKF2_MULTI_IMU) , 1) x MAX([EKF2_MULTI_MAG](../19.高级调参/parameter_reference.md#EKF2_MULTI_MAG) , 1)
...
If [EKF2_MULTI_IMU](../19.高级调参/parameter_reference.md#EKF2_MULTI_IMU) >= 3, then the failover time for large rate gyro errors is further reduced because the EKF selector is able to apply a median select strategy for faster isolation of the faulty IMU.

The setup for multiple EKF instances is controlled by the following parameters:

- [SENS_IMU_MODE](../19.高级调参/parameter_reference.md#SENS_IMU_MODE):
  Set to 0 if running multiple EKF instances with IMU sensor diversity, ie [EKF2_MULTI_IMU](../19.高级调参/parameter_reference.md#EKF2_MULTI_IMU) > 1.
...
- [SENS_MAG_MODE](../19.高级调参/parameter_reference.md#SENS_MAG_MODE):
  Set to 0 if running multiple EKF instances with magnetometer sensor diversity, ie [EKF2_MULTI_MAG](../19.高级调参/parameter_reference.md#EKF2_MULTI_MAG) > 1.

  When set to 1 (default for single EKF operation) the sensor module selects Magnetometer data used by the EKF.
  This provides protection against loss of data from the sensor but does not protect against bad sensor data.
  When set to 0, the sensor module does not make a selection.

- [EKF2_MULTI_IMU](../19.高级调参/parameter_reference.md#EKF2_MULTI_IMU):
  ...
  When [SENS_IMU_MODE](../19.高级调参/parameter_reference.md#SENS_IMU_MODE) = 1, this will be the sensor selected by the sensor module.
...
- [EKF2_MULTI_MAG](../19.高级调参/parameter_reference.md#EKF2_MULTI_MAG):
  ...
  When [SENS_MAG_MODE](../19.高级调参/parameter_reference.md#SENS_MAG_MODE) = 1, this will be the sensor selected by the sensor module.
...
The logic used to select these modes is set by the [EKF2_MAG_TYPE](../19.高级调参/parameter_reference.md#EKF2_MAG_TYPE) parameter.
...
- [GNSS/GPS](#gnss-gps): [EKF2_GPS_CTRL](../19.高级调参/parameter_reference.md#EKF2_GPS_CTRL)
- [Barometer](#barometer): [EKF2_BARO_CTRL](../19.高级调参/parameter_reference.md#EKF2_BARO_CTRL)
- [Range finder](#range-finder): [EKF2_RNG_CTRL](../19.高级调参/parameter_reference.md#EKF2_RNG_CTRL)
- [External vision](#external-vision-system): Enabled when [EKF2_HGT_REF](../19.高级调参/parameter_reference.md#EKF2_HGT_REF) is set to "Vision"

Over the long term the height estimate follows the "reference source" of height data.
This reference is defined by the [EKF2_HGT_REF](../19.高级调参/parameter_reference.md#EKF2_HGT_REF) parameter.

#### Typical configurations

|                           | [EKF2_GPS_CTRL](../19.高级调参/parameter_reference.md#EKF2_GPS_CTRL) | [EKF2_BARO_CTRL](../19.高级调参/parameter_reference.md#EKF2_BARO_CTRL) | [EKF2_RNG_CTRL](../19.高级调参/parameter_reference.md#EKF2_RNG_CTRL) | [EKF2_HGT_REF](../19.高级调参/parameter_reference.md#EKF2_HGT_REF) |
| ------------------------- | ------------------------------------------------------------------------ | -------------------------------------------------------------------------- | ------------------------------------------------------------------------ | ---------------------------------------------------------------------- |
...
Enable/disable using [EKF2_BARO_CTRL](../19.高级调参/parameter_reference.md#EKF2_BARO_CTRL) as a source for [Height](#height) data.

Note that data from only one barometer is fused, even if multiple barometers are available.
The barometer with the highest [CAL_BAROx_PRIO](../19.高级调参/parameter_reference.md#CAL_BARO0_PRIO) priority value is selected first, falling back to the next highest priority barometer if a sensor fault is detected.
...
1. Fly once in [Position mode](../05.基本概念/01.飞行模式/position.md) repeatedly forwards/backwards/left/right/up/down between rest and maximum speed (best results are obtained when this testing is conducted in still conditions).
...
- [EKF2_PCOEF_XP](../19.高级调参/parameter_reference.md#EKF2_PCOEF_XP)
- [EKF2_PCOEF_XN](../19.高级调参/parameter_reference.md#EKF2_PCOEF_XN)
- [EKF2_PCOEF_YP](../19.高级调参/parameter_reference.md#EKF2_PCOEF_YP)
- [EKF2_PCOEF_YN](../19.高级调参/parameter_reference.md#EKF2_PCOEF_YN)
- [EKF2_PCOEF_Z](../19.高级调参/parameter_reference.md#EKF2_PCOEF_Z)
...
- GPS use is enabled via setting of the [EKF2_GPS_CTRL](../19.高级调参/parameter_reference.md#EKF2_GPS_CTRL) parameter.
- GPS quality checks have passed.
  These checks are controlled by the [EKF2_GPS_CHECK](../19.高级调参/parameter_reference.md#EKF2_GPS_CHECK) and `EKF2_REQ_*` parameters.
...
Use of GPS yaw measurements is enabled by setting bit position 3 to 1 (adding 8) in the [EKF2_GPS_CTRL](../19.高级调参/parameter_reference.md#EKF2_GPS_CTRL) parameter.
...
To use this feature, set [EKF2_MAG_TYPE](../19.高级调参/parameter_reference.md#EKF2_MAG_TYPE) to `none` (5) to disable magnetometer use.
...
This is controlled by the [SENS_GPS_MASK](../19.高级调参/parameter_reference.md#SENS_GPS_MASK) parameter.

The [SENS_GPS_MASK](../19.高级调参/parameter_reference.md#SENS_GPS_MASK) parameter is set by default to disable blending and always use the first receiver, so it will have to be set to select which receiver accuracy metrics are used to decide how much each receiver output contributes to the blended solution.
Where different receiver models are used, it is important that the [SENS_GPS_MASK](../19.高级调参/parameter_reference.md#SENS_GPS_MASK) parameter is set to a value that uses accuracy metrics that are supported by both receivers.
...
- Verify that data for the second receiver is present.
  ...
  The [GPS_2_CONFIG](../19.高级调参/parameter_reference.md#GPS_2_CONFIG) parameter will need to be set correctly.
- Check the `s_variance_m_s`, `eph` and `epv` data from each receiver and decide which accuracy metrics can be used.
  If both receivers output sensible `s_variance_m_s` and `eph` data, and GPS vertical position is not being used directly for navigation, then setting [SENS_GPS_MASK](../19.高级调参/parameter_reference.md#SENS_GPS_MASK) to 3 is recommended.
  Where only `eph` data is available and both receivers do not output `s_variance_m_s` data, set [SENS_GPS_MASK](../19.高级调参/parameter_reference.md#SENS_GPS_MASK) to 2.
  Bit position 2 would only be set if the GPS had been selected as the reference height source with the [EKF2_HGT_REF](../19.高级调参/parameter_reference.md#EKF2_HGT_REF) parameter and both receivers output sensible `epv` data.
...
For the ECL to accept GNSS data for navigation, certain minimum requirements need to be satisfied over a period of time, defined by [EKF2_REQ_GPS_H](../19.高级调参/parameter_reference.md#EKF2_REQ_GPS_H) (10 seconds by default).

Minima are defined in the [EKF2*REQ*\*](../19.高级调参/parameter_reference.md#EKF2_REQ_EPH) parameters and each check can be enabled/disabled using the [EKF2_GPS_CHECK](../19.高级调参/parameter_reference.md#EKF2_GPS_CHECK) parameter.
...
| -------------------- | ----------------------------------------------------------------------------------------- | ------------- | ----- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| eph                  | <&nbsp;3 ([EKF2_REQ_EPH](../19.高级调参/parameter_reference.md#EKF2_REQ_EPH))         | 0.8           | m     | Standard deviation of horizontal position error                                                                                             |
| epv                  | <&nbsp;5 ([EKF2_REQ_EPV](../19.高级调参/parameter_reference.md#EKF2_REQ_EPV))         | 1.5           | m     | Standard deviation of vertical position error                                                                                               |
| Number of satellites | ≥6&nbsp;([EKF2_REQ_NSATS](../19.高级调参/parameter_reference.md#EKF2_REQ_NSATS))      | 14            | -     |
| sacc                 | <&nbsp;0.5 ([EKF2_REQ_SACC](../19.高级调参/parameter_reference.md#EKF2_REQ_SACC))     | 0.2           | m/s   | Standard deviation of horizontal speed error                                                                                                |
...
| PDOP                 | <&nbsp;2.5 ([EKF2_REQ_PDOP](../19.高级调参/parameter_reference.md#EKF2_REQ_PDOP))     | 1.0           | -     | Position dilution of precision                                                                                                              |
| hpos drift rate      | <&nbsp;0.1 ([EKF2_REQ_HDRIFT](../19.高级调参/parameter_reference.md#EKF2_REQ_HDRIFT)) | 0.01          | m/s   | Drift rate calculated from reported GNSS position (when stationary).                                                                        |
| vpos drift rate      | <&nbsp;0.2 ([EKF2_REQ_VDRIFT](../19.高级调参/parameter_reference.md#EKF2_REQ_VDRIFT)) | 0.02          | m/s   | Drift rate calculated from reported GNSS altitude (when stationary).                                                                        |
| hspd                 | <&nbsp;0.1 ([EKF2_REQ_HDRIFT](../19.高级调参/parameter_reference.md#EKF2_REQ_HDRIFT)) | 0.01          | m/s   | Filtered magnitude of reported GNSS horizontal velocity.                                                                                    |
| vspd                 | <&nbsp;0.2 ([EKF2_REQ_VDRIFT](../19.高级调参/parameter_reference.md#EKF2_REQ_VDRIFT)) | 0.02          | m/s   | Filtered magnitude of reported GNSS vertical velocity.                                                                                      |
...
The fusion modes of operation are controlled by [EKF2_RNG_CTRL](../19.高级调参/parameter_reference.md#EKF2_RNG_CTRL):
...
If the range finder is set as the reference height source (using [EKF2_HGT_REF](../19.高级调参/parameter_reference.md#EKF2_HGT_REF)), the other active height sources such as baro and GNSS altitude will adjust their measurement to match the readings of the range finder over time.
...
:::tip
[Terrain Hold](../03.飞行指南/terrain_following_holding.md#terrain_hold) is recommended over _Range Aid_ for terrain holding.
...
_Conditional range aid_ is enabled by setting [EKF2_RNG_CTRL](../19.高级调参/parameter_reference.md#EKF2_RNG_CTRL) = "Enabled (conditional mode)" (1).

It is further configured using the `EKF2_RNG_A_` parameters:

- [EKF2_RNG_A_VMAX](../19.高级调参/parameter_reference.md#EKF2_RNG_A_VMAX): Maximum horizontal speed, above which range aid is disabled.
- [EKF2_RNG_A_HMAX](../19.高级调参/parameter_reference.md#EKF2_RNG_A_HMAX): Maximum height, above which range aid is disabled.
- [EKF2_RNG_A_IGATE](../19.高级调参/parameter_reference.md#EKF2_RNG_A_IGATE): Range aid consistency checks "gate" (a measure of the error before range aid is disabled).
...
The feature is enabled by setting [EKF2_RNG_CTRL](../19.高级调参/parameter_reference.md#EKF2_RNG_CTRL) to "Enabled" (2).
To make the range finder the height reference when active, set: [EKF2_HGT_REF](../19.高级调参/parameter_reference.md#EKF2_HGT_REF) to "Range sensor".

:::tip
To enable the range finder fusion only when the drone is stationary (in order to benefit from a better altitude estimate during takeoff and landing) but not fuse the range finder the rest of the time, use the [conditional mode](#conditional-range-aiding) (1) of [EKF2_RNG_CTRL](../19.高级调参/parameter_reference.md#EKF2_RNG_CTRL).
:::
...
- [EKF2_RNG_NOISE](../19.高级调参/parameter_reference.md#EKF2_RNG_NOISE)
- [EKF2_RNG_K_GATE](../19.高级调参/parameter_reference.md#EKF2_RNG_K_GATE)

### Airspeed

Equivalent Airspeed (EAS) data can be used to estimate wind velocity and reduce drift when GPS is lost by setting [EKF2_ARSP_THR](../19.高级调参/parameter_reference.md#EKF2_ARSP_THR) to a positive value.
Airspeed data will be used when it exceeds the threshold set by a positive value for [EKF2_ARSP_THR](../19.高级调参/parameter_reference.md#EKF2_ARSP_THR) and the vehicle type is not rotary wing.
...
This is enabled by setting the [EKF2_FUSE_BETA](../19.高级调参/parameter_reference.md#EKF2_FUSE_BETA) parameter to 1.
...
This can be enabled using [EKF2_DRAG_CTRL](../19.高级调参/parameter_reference.md#EKF2_DRAG_CTRL).

The relationship between airspeed and specific force (IMU accelerometer measurements) along the X and Y body axes is controlled by the [EKF2_BCOEF_X](../19.高级调参/parameter_reference.md#EKF2_BCOEF_X), [EKF2_BCOEF_Y](../19.高级调参/parameter_reference.md#EKF2_BCOEF_Y) and [EKF2_MCOEF](../19.高级调参/parameter_reference.md#EKF2_MCOEF) parameters which set the ballistic coefficients for flight in the X and Y directions, and the momentum drag produced by the propellers, respectively.
The amount of specific force observation noise is set by the [EKF2_DRAG_NOISE](../19.高级调参/parameter_reference.md#EKF2_DRAG_NOISE) parameter.
...
- [EKF2_OF_CTRL](../19.高级调参/parameter_reference.md#EKF2_OF_CTRL) is set.
- The quality metric returned by the flow sensor is greater than the minimum requirement set by the [EKF2_OF_QMIN](../19.高级调参/parameter_reference.md#EKF2_OF_QMIN) parameter.

### External Vision System

Position, velocity or orientation measurements from an external vision system, e.g. Vicon, can be used.

The measurements that are fused are configured by setting the appropriate bits of [EKF2_EV_CTRL](../19.高级调参/parameter_reference.md#EKF2_EV_CTRL) to `true`:

- `0`: Horizontal position data
- `1`: Vertical position data.
  Height sources may additionally be configured using [EKF2_HGT_REF](../19.高级调参/parameter_reference.md#EKF2_HGT_REF) (see section [Height](#height)).
- `2`: Velocity data
- `3`: Yaw data
...
This uncertainty information can be sent via the covariance fields in the MAVLink [ODOMETRY](https://mavlink.io/en/messages/common.html#ODOMETRY) message or it can be set through the parameters [EKF2_EVP_NOISE](../19.高级调参/parameter_reference.md#EKF2_EVP_NOISE), [EKF2_EVV_NOISE](../19.高级调参/parameter_reference.md#EKF2_EVV_NOISE) and [EKF2_EVA_NOISE](../19.高级调参/parameter_reference.md#EKF2_EVA_NOISE).
You can choose the source of the uncertainty with [EKF2_EV_NOISE_MD](../19.高级调参/parameter_reference.md#EKF2_EV_NOISE_MD).

## How do I use the 'ecl' library EKF?

Set the [SYS_MC_EST_GROUP](../19.高级调参/parameter_reference.md#SYS_MC_EST_GROUP) parameter to 2 to use the ecl EKF.
...
- [1] Velocity tracking error magnitude (m/s).
  The velocity tracking time constant can be adjusted using the [EKF2_TAU_VEL](../19.高级调参/parameter_reference.md#EKF2_TAU_VEL) parameter.
...
- [2] Position tracking error magnitude \(m\).
  The position tracking time constant can be adjusted using the [EKF2_TAU_POS](../19.高级调参/parameter_reference.md#EKF2_TAU_POS) parameter.
...
- Double the value of the innovation gate for the primary height sensor.
  If using barometric height this is [EKF2_BARO_GATE](../19.高级调参/parameter_reference.md#EKF2_BARO_GATE).
- Increase the value of [EKF2_ACC_NOISE](../19.高级调参/parameter_reference.md#EKF2_ACC_NOISE) to 0.5 initially.
...
- High vibration levels.
  ...
  - Increasing the value of [EKF2_ACC_NOISE](../19.高级调参/parameter_reference.md#EKF2_ACC_NOISE) and [EKF2_GYR_NOISE](../19.高级调参/parameter_reference.md#EKF2_GYR_NOISE) can help, but does make the EKF more vulnerable to GPS glitches.
...
- Then set the parameter [EKF2_GND_EFF_DZ](../19.高级调参/parameter_reference.md#EKF2_GND_EFF_DZ) to that value and add a 10 percent margin. Therefore, in this case a value of 6.6 meters would be a good starting point.

If a terrain estimate is available (e.g. the vehicle is equipped with a range finder) then you can additionally specify [EKF2_GND_MAX_HGT](../19.高级调参/parameter_reference.md#EKF2_GND_MAX_HGT), the above ground-level altitude below which ground effect compensation should be activated.
