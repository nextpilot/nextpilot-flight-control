...
1. Perform the [standard compass calibration](../07.标准配置/06.compass.md#compass-calibration).
1. Set the parameter [SDLOG_MODE](../19.高级调参/parameter_reference.md#SDLOG_MODE) to 2 to enable logging of data from boot.
1. Set the parameter [SDLOG_PROFILE](../19.高级调参/parameter_reference.md#SDLOG_PROFILE) checkbox for _Sensor comparison_ (bit 6) to get more data points.
...
1. Power the vehicle and switch into [ACRO flight mode](../05.基本概念/01.飞行模式/acro.md) (using this mode ensures the vehicle won't attempt to compensate for movement resulting from the straps).
...
1. Once the parameters are identified, the power compensation must be enabled by setting [CAL_MAG_COMP_TYP](../19.高级调参/parameter_reference.md#CAL_MAG_COMP_TYP) to 1 (when using thrust parameters) or 2 (when using current parameters).
