...
Most flight controllers on which PX4 runs include a barometer.
By default PX4 will select the barometer with the highest priority (if any are present), and configure it as a data source for [Height estimation](../../19.高级调参/tuning_the_ecl_ekf.md#height).
...
- Enable/Disable barometers as data source for [Height estimation](../../19.高级调参/tuning_the_ecl_ekf.md#height) using the [EKF2_BARO_CTRL](../../19.高级调参/parameter_reference.md#EKF2_BARO_CTRL) parameter.
- Change the selection order of barometers using the [CAL_BAROx_PRIO](../../19.高级调参/parameter_reference.md#CAL_BARO0_PRIO) parameters for each barometer.
- Disable a barometer by setting its [CAL_BAROx_PRIO](../../19.高级调参/parameter_reference.md#CAL_BARO0_PRIO) value to `0`.
