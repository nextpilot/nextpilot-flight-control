...
Nextpilot can run on many flight controller boards.
...
Pixhawk Series open-hardware flight controllers run Nextpilot on RT-Thread OS.
With many form factors, there are versions targeted towards many use cases and market segments.

Pixhawk Standard Autopilots are used as the Nextpilot reference platform.
...
Other flight controllers are manufacturer-supported.
...
For more computing power, the most common approach is to run those applications on a separate onboard Companion Computer.
