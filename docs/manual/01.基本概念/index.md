...
If you are already familiar with the basic concepts, you can move on to [Basic Assembly](../02.快速入门/03.设备安装/README.md) to learn how to wire your specific autopilot hardware.
To load firmware and set up the vehicle with _QGroundControl_, see [Basic Configuration](../07.标准配置/README.md).
...
- Controls [many different vehicle frames/types](../09.机型调参/01.airframe_reference.md), including: aircraft (multicopters, fixed-wing aircraft and VTOLs), ground vehicles and underwater vehicles.
...
- Robust and deep integration with [companion computers](#offboard-companion-computer) and robotics APIs (ROS 2), [MAVSDK](https://mavsdk.mavlink.io)).

PX4 is a core part of a broader drone platform that includes the [QGroundControl](#qgroundcontrol) ground station, [Pixhawk hardware](https://pixhawk.org/), and [MAVSDK](https://mavsdk.mavlink.io) for integration with companion computers, cameras and other hardware using the MAVLink protocol.
PX4 is supported by the [Dronecode Project](https://www.dronecode.org/).

## QGroundControl

The Dronecode Ground Control Station (GCS) is called [QGroundControl](https://qgroundcontrol.com/).
...
Download and install it from [here](https://qgroundcontrol.com/downloads/).

![QGC Main Screen](/assets/concepts/qgc_main_screen.jpg)

## Flight Controller Board

Nextpilot was initially designed to run on Pixhawk Series controllers, but can now run on Linux computers and other hardware.
...
- [Sensors](../01.基本概念/sensor_selection.md)
...
The outputs may be PWM ports or DroneCAN nodes (e.g. DroneCAN motor controllers).
The images below show the PWM output ports for Pixhawk 4 and Pixhawk 4 mini.
...
!!! warning
    ...
    Ensure that you select a controller that has enough ports/outputs for your [airframe](../09.机型调参/01.airframe_reference.md).

You can connect almost any output to any motor or other actuator, by assigning the associated function ("Motor 1") to the desired output ("AUX1") in QGroundControl: [Actuator Configuration and Testing](../07.标准配置/actuators.md).
Note that the functions (motor and control surface actuator positions) for each frame are given in the [Airframe Reference](../09.机型调参/01.airframe_reference.md).
...
- The FMU output ports can use D-shot or _One-shot_ protocols (as well as PWM), which provide much lower-latency behaviour.
...
The battery is typically connected to the system using a [Power Module](../21.周边设备/12.电源模块/README.md) or _Power Management Board_, which provide separate power for the flight controller and to the ESCs (for the motors).

Information about batteries and battery configuration can be found in [Battery Estimation Tuning](../07.标准配置/battery.md) and the guides in [Basic Assembly](../02.快速入门/03.设备安装/README.md) (e.g. [Pixhawk 4 Wiring Quick Start > Power](../02.快速入门/03.设备安装/quick_start_pixhawk4.md#power)).

## Manual Control

Pilots can control a vehicle manually using either a [Radio Control (RC) System](../21.周边设备/01.遥 控 器/index.md) or a [Joystick/Gamepad](../07.标准配置/joystick.md) controller connected via QGroundControl.
...
[Data/Telemetry Radios](../21.周边设备/03.数 据 链/microhard-p900.md) can provide a wireless MAVLink connection between a ground control station like _QGroundControl_ and a vehicle running PX4.
...
The companion computer typically communicates with PX4 using a high level Robotics API such as [MAVSDK](https://mavsdk.mavlink.io/) or ROS 2.

Relevant topics include:

- Companion Computers
- [Off-board Mode](../05.基本概念/01.飞行模式/offboard.md) - Flight mode for offboard control of PX4 from a GCS or companion computer.
- Robotics APIs

## SD Cards (Removable Memory)

PX4 uses SD memory cards for storing [flight logs](../15.数据分析/flight_reporting.md), and they are also required in order to use UAVCAN peripherals and fly [missions](../03.飞行指南/missions.md).

By default, if no SD card is present PX4 will play the format failed (2-beep(../05.基本概念/03.声光告警/tunes.md#format-failed) tune twice during boot (and none of the above features will be available).

!!! tip
    The maximum supported SD card size on Pixhawk boards is 32GB.
    The _SanDisk Extreme U3 32GB_ and _Samsung EVO Plus 32_ are [highly recommended](../15.数据分析/logging.md#sd-cards).

SD cards are never-the-less optional.
Flight controllers that do not include an SD Card slot may:

- Disable notification beeps are disabled using the parameter [CBRK_BUZZER](../19.高级调参/parameter_reference.md#CBRK_BUZZER).
- [Stream logs](../15.数据分析/logging.md#log-streaming) to another component (companion).
...
- [Payloads & Cameras](../21.周边设备/99.任务载荷/index.md)
...
- Arming is prevented if a VTOL vehicle is in fixed-wing mode ([by default](../19.高级调参/parameter_reference.md#CBRK_VTOLARMING)).
...
A detailed overview of arming and disarming configuration can be found here: [Prearm, Arm, Disarm Configuration](../19.高级调参/prearm_arm_disarm.md).
...
- Flight Modes (Multicopter(../05.基本概念/01.飞行模式/README.md)
- Flight Modes (Fixed-Wing(../05.基本概念/01.飞行模式/README.md)
- Flight Modes (VTOL(../05.基本概念/01.飞行模式/README.md)
- Flight Modes (Rover(../05.基本概念/01.飞行模式/README.md)

Instructions for how to set up your remote control switches to enable different flight modes is provided in [Flight Mode Configuration](../07.标准配置/flight_mode.md).
...
For more information see: [Safety](../07.标准配置/99.safety.md) (Basic Configuration).
...
You can read in depth about heading in [Flight Controller Orientation](../07.标准配置/05.flight_controller_orientation.md)
